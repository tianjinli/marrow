...
private:
  ...
  BYTE m_cbBuffer[64];  // 加密变量
  BYTE m_cbPadding[64]; // 加密变量
...
VOID CMD5Aide::Decode(ULONG* pcbOutput, BYTE* pcbInput, UINT nLen) {
  UINT i, j;
  for (i = 0, j = 0; j < nLen; i++, j += 4) {
    pcbOutput[i] = ((ULONG)pcbInput[j]) | (((ULONG)pcbInput[j + 1]) << 8) | (((ULONG)pcbInput[j + 2]) << 16) | (((ULONG)pcbInput[j + 3]) << 24);
  }
}
...
bool CWHEncrypt::MD5Encrypt(LPCTSTR pszSourceData, TCHAR szMD5Result[LEN_MD5]) {
  ...
  CMD5Aide MD5Aide;
  BYTE szResult[16];
  std::string strSrcData = ToSimpleUtf8(pszSourceData);

  // 加密密文
  MD5Aide.MD5Update((BYTE*)(strSrcData.c_str()), (UINT) strSrcData.length() * sizeof(char));
  MD5Aide.MD5Final(szResult);

  // 输出结果
  szMD5Result[0] = 0;
  for (INT i = 0; i < 16; i++) {
    SnprintfT(&szMD5Result[i * 2], LEN_MD5 - i * 2, TEXT("%02X"), szResult[i]); // 第二个参数也可以写死 3 (包含终止符)
  }
  return true;
}
...
bool CWHEncrypt::XorEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncryptData, WORD wMaxCount) {
  // 变量定义
  StringT strSrcData = (pszSourceData);
  TCHAR szEncryptData[MAX_ENCRYPT_LEN + 1] = TEXT("");

  // 生成密钥
  WORD wRandKey[ENCRYPT_KEY_LEN];
  wRandKey[0] = (WORD) strSrcData.length();
  for (WORD i = 1; i < std::size(wRandKey); i++) {
  ...
  for (WORD i = 0; i < wTimes; i++) {
    ...
    SnprintfT(szEncryptData + i * 8, MAX_ENCRYPT_LEN + 1 - i * 8, TEXT("%04X%04X"), wRandKey[i % ENCRYPT_KEY_LEN],
              wTempCode); // 第二个参数也可以写死 9 (包含终止符)
  }

  // 字符转换
  LSTRCPYN(pszEncryptData, szEncryptData, wMaxCount);
...
bool CWHEncrypt::XorCrevasse(LPCTSTR pszEncryptData, LPTSTR pszSourceData, WORD wMaxCount) {
  ...
  // 变量定义
  StringT strEncrypData = (pszEncryptData);
  TCHAR szSrcData[MAX_SOURCE_LEN] = TEXT("");

  // 效验长度
  WORD wEncryptPassLen = (WORD) strEncrypData.length();
  if (wEncryptPassLen < ENCRYPT_KEY_LEN * 8) {
    return false;
  }

  // 提取长度
  TCHAR szTempBuffer[5] = TEXT("");
  szTempBuffer[std::size(szTempBuffer) - 1] = 0;
  CopyMemory(szTempBuffer, strEncrypData.c_str(), sizeof(TCHAR) * 4);

  // 获取长度
  size_t pEnd = 0;
  WORD wSourceLength = (WORD)std::stol(szTempBuffer, &pEnd, 16);

  // 长度效验
  ASSERT(wEncryptPassLen == (((wSourceLength + ENCRYPT_KEY_LEN - 1) / ENCRYPT_KEY_LEN) * ENCRYPT_KEY_LEN * 8));
  if (wEncryptPassLen != (((wSourceLength + ENCRYPT_KEY_LEN - 1) / ENCRYPT_KEY_LEN) * ENCRYPT_KEY_LEN * 8)) {
    return false;
  }

  // 长度效验
  ASSERT((wSourceLength + 1) <= wMaxCount);
  if ((wSourceLength + 1) > wMaxCount) {
    return false;
  }

  // 解开密码
  for (INT i = 0; i < wSourceLength; i++) {
    // 获取密钥
    TCHAR szKeyBuffer[5] = TEXT("");
    szKeyBuffer[std::size(szKeyBuffer) - 1] = 0;
    szTempBuffer[std::size(szTempBuffer) - 1] = 0;
    CopyMemory(szKeyBuffer, strEncrypData.c_str() + i * 8, sizeof(TCHAR) * 4);
    CopyMemory(szTempBuffer, strEncrypData.c_str() + i * 8 + 4, sizeof(TCHAR) * 4);

    // 提取密钥
    TCHAR wKey = (TCHAR)std::stol(szKeyBuffer, &pEnd, 16);
    TCHAR wEncrypt = (TCHAR)std::stol(szTempBuffer, &pEnd, 16);

    // 生成原文
    szSrcData[i] = (TCHAR)((TCHAR)wKey ^ (TCHAR)wEncrypt);
  }

  // 终止字符
  szSrcData[wSourceLength] = 0;

  // 字符转换
  LSTRCPYN(pszSourceData, szSrcData, wMaxCount);
...
bool CWHEncrypt::MapEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncrypData, WORD wMaxCount) {
  // 效验参数
  ASSERT(wMaxCount > StrLenT(pszEncrypData));
  ASSERT((pszEncrypData != nullptr) && (pszSourceData != nullptr));

  // 变量定义
  UINT nLength = (UINT) StrLenT(pszSourceData);
  BYTE* pcbEncryptData = (BYTE*)pszEncrypData;
...
bool CWHEncrypt::MapCrevasse(LPCTSTR pszEncrypData, LPTSTR pszSourceData, WORD wMaxCount) {
  ...
  // 变量定义
  UINT nLength = (UINT) StrLenT(pszEncrypData);
  BYTE* pcbEncryptData = (BYTE*)pszEncrypData;
  BYTE* pcbSourceData = (BYTE*)pszSourceData;
