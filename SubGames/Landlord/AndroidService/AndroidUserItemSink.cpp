  ...
  switch (nTimerID) {
    ...
    case IDI_OUT_CARD: // 用户出牌
    {
      ...
      try {
        ...
        m_GameLogic.SearchOutCard(m_cbHandCardData, m_cbHandCardCount[wMeChairID], m_cbTurnCardData, m_cbTurnCardCount, m_wOutCardUser,
                                  m_pIAndroidUserItem->GetChairID(), OutCardResult);
      } catch (...) {
        // 这里的设置，使得进入下面的if处理
