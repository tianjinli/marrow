...
CTableFrameSink::CTableFrameSink() {
  ...
  m_hControlInst = DYNLIB_LOAD(GAME_CONTROL_DLL_NAME);
  if (m_hControlInst) {
    typedef void* (*CREATE)();
    CREATE ServerControl = (CREATE) DYNLIB_GETSYM(m_hControlInst, "CreateServerControl");
    if (ServerControl) {
...
//
  ...
  //     0x11, 0x11, 0x1D, 0x0C, 0x0B, 0x0A, 0x0A, 0x09, 0x07, 0x07, 0x07, 0x07, 0x06, 0x06, 0x06, 0x05, 0x05, 0x15,
  //     0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A,
  //     0x2B, 0x2C, 0x2D, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x05, 0x05, 0x04,
  // };
...
bool CTableFrameSink::OnEventGameConclude(WORD wChairID, IServerUserItem* pIServerUserItem, BYTE cbReason) {
  switch (cbReason) {
    case GER_NORMAL: // 常规结束
    {
      ...
      // 调整倍数
      lScoreTimes = std::min((LONG)m_pGameCustomRule->wMaxScoreTimes, lScoreTimes);
    ...
    case GER_NETWORK_ERROR: // 网络中断
    {
      ...
      // 调整倍数
      lScoreTimes = std::min(m_pGameCustomRule->wMaxScoreTimes, lScoreTimes);
      ...
      // 变量定义
      SCORE lCellScore = m_pITableFrame->GetCellScore();
      SCORE lUserScore = lCellScore * std::max(lScoreTimes, m_pGameCustomRule->wFleeScoreTimes);

      // 金币平衡
      if ((m_pGameServiceOption->wServerType & SCORE_GENRE_POSITIVE) != 0 ||
          (m_pGameServiceOption->wServerType & (GAME_GENRE_GOLD | GAME_GENRE_EDUCATE)) != 0) {
        lUserScore = std::min(pIServerUserItem->GetUserScore(), lUserScore);
      }
      ...
      switch (cbGameStatus) {
        ...
        case GAME_SCENE_PLAY: {
          if (m_cbTurnCardCount == 0) {
            ...
            if (SearchCardResult.cbCardCount[0] > 0) {
