...
bool CMatchInfoBuffer::ResetMatchOption() {
  // 添加对象
  m_GameMatchOptionBuffer.insert(m_GameMatchOptionBuffer.end(), m_GameMatchOptionBuffer.begin(), m_GameMatchOptionArray.begin());
...
tagGameMatchOption* CMatchInfoBuffer::CreateMatchOption() {
  ...
  // 创建对象
  // INT_PTR nArrayCount = m_GameMatchOptionBuffer.size();
  // if (nArrayCount > 0) {
...
CRewardInfoBuffer::~CRewardInfoBuffer() {
  ...
  // 删除数组
  m_MatchRewardInfoBuffer.insert(m_MatchRewardInfoBuffer.end(), m_MatchRewardInfoArray.begin(), m_MatchRewardInfoArray.end());
  for (INT_PTR i = 0; i < m_MatchRewardInfoBuffer.size(); i++) {
    pMatchRewardInfo = m_MatchRewardInfoBuffer[i];
    SafeDelete(pMatchRewardInfo);
  }

  // 删除引用
  m_MatchRewardInfoArray.clear();
  m_MatchRewardInfoBuffer.clear();
...
bool CRewardInfoBuffer::DeleteMatchRewardInfo(WORD wRankID) {
  ...
  // 查找对象
  for (INT_PTR i = 0; i < m_MatchRewardInfoArray.size(); i++) {
    pMatchRewardInfo = m_MatchRewardInfoArray[i];
    if (pMatchRewardInfo->wRankID == wRankID) {
      m_MatchRewardInfoBuffer.emplace_back(pMatchRewardInfo);
      // 删除数据
      m_MatchRewardInfoArray.erase(m_MatchRewardInfoArray.begin() + i);
...
tagMatchRewardInfo* CRewardInfoBuffer::InsertMatchRewardInfo(tagMatchRewardInfo& MatchRewardInfo) {
  ...
  if (pMatchRewardInsert == NULL) {
    ...
    // 添加数据
    m_MatchRewardInfoArray.emplace_back(pMatchRewardInsert);
  }
...
DWORD CRewardInfoBuffer::GetMatchRewardCount() {
  return (DWORD) (m_MatchRewardInfoArray.size());
}
...
tagMatchRewardInfo* CRewardInfoBuffer::CreateMatchRewardInfo() {
  ...
  // 创建对象
  // INT_PTR nArrayCount = m_MatchRewardInfoBuffer.size();
  // if (nArrayCount > 0) {
  //   pMatchRewardInfo = m_MatchRewardInfoBuffer[nArrayCount - 1];
  if (!m_MatchRewardInfoBuffer.empty()) {
    pMatchRewardInfo = m_MatchRewardInfoBuffer.back();
    m_MatchRewardInfoBuffer.pop_back();
    } else {
...
bool CMatchInfoManager::InsertGameMatchOption(tagCreateMatchInfo* pCreateMatchInfo, tagGameMatchOption& GameMatchResult) {
  ...
  try {
    ...
    for (INT i = 0; i < CountArray(pCreateMatchInfo->cbMatchRule); i++) {
      SnprintfT(&szMatchRule[i * 2], 3, TEXT("%02X"), pCreateMatchInfo->cbMatchRule[i]);
    }
...
bool CMatchInfoManager::ConnectGameMatchDB(CDataBaseHelper& GameMatchDBModule) {
  ...
  if ((GameMatchDBModule.GetInterface() == NULL) && (GameMatchDBModule.CreateInstance() == false)) {
    CLogger::Error(TEXT("ConnectGameMatchDB 创建 GameMatchDBModule 对象失败"));
    return false;
  }

  // 设置连接
  GameMatchDBModule->SetConnectionInfo(pDataBaseParameter->szDataBaseAddr, pDataBaseParameter->wDataBasePort, pDataBaseParameter->szDataBaseName,
                                       pDataBaseParameter->szDataBaseUser, pDataBaseParameter->szDataBasePass);
...
bool CMatchInfoManager::ReadGameMatchOption(CDataBaseAide& GameMatchDBAide, tagGameMatchOption& GameMatchResult) {
  ...
  // 奖励人数
  GameMatchResult.wRewardCount = (WORD) std::min((size_t) RewardInfoBuffer.GetMatchRewardCount(), CountArray(GameMatchResult.MatchRewardInfo));
