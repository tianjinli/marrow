...
CServerInfoBuffer::~CServerInfoBuffer() {
  // 删除容器
  for (auto& pGameServerInfo: m_GameServerInfoMap | std::views::values) {
    SafeDelete(pGameServerInfo);
  }

  // 删除数组
  for (auto& pGameServerInfo: m_GameServerInfoArray) {
    SafeDelete(pGameServerInfo);
  }

  // 删除引用
  m_GameServerInfoMap.clear();
  m_GameServerInfoArray.clear();
}

// 重置数据
bool CServerInfoBuffer::ResetServerInfo() {
  for (auto& pGameServerInfo: m_GameServerInfoMap | std::views::values) {
    m_GameServerInfoArray.emplace_back(pGameServerInfo);
  }
  // 删除索引
  m_GameServerInfoMap.clear();
...
bool CServerInfoBuffer::DeleteServerInfo(WORD wServerID) {
  // 查找类型
  auto itr = m_GameServerInfoMap.find(wServerID);
  if (itr == m_GameServerInfoMap.end()) {
    return false;
  }

  tagGameServerInfo* pGameServerInfo = itr->second;
  // 删除数据
  m_GameServerInfoArray.emplace_back(pGameServerInfo);
  m_GameServerInfoMap.erase(itr);
...
DWORD CServerInfoBuffer::GetServerInfoCount() {
  return (DWORD) (m_GameServerInfoMap.size());
}
...
tagGameServerInfo* CServerInfoBuffer::CreateServerInfo() {
  ...
  if (!m_GameServerInfoArray.empty()) {
    pGameServerInfo = m_GameServerInfoArray.back();
    m_GameServerInfoArray.pop_back();
  } else {
    pGameServerInfo = new (std::nothrow) tagGameServerInfo;
...
CMatchOptionBuffer::~CMatchOptionBuffer() {
  ...
  // 删除数组
  m_GameMatchOptionBuffer.insert(m_GameMatchOptionBuffer.end(), m_GameMatchOptionArray.begin(), m_GameMatchOptionArray.end());
  for (INT_PTR i = 0; i < m_GameMatchOptionBuffer.size(); i++) {
    pGameMatchOption = m_GameMatchOptionBuffer[i];
    SafeDelete(pGameMatchOption);
  }

  // 删除引用
  m_GameMatchOptionArray.clear();
  m_GameMatchOptionBuffer.clear();
}

// 重置数据
bool CMatchOptionBuffer::ResetMatchOption() {
  // 添加对象
  m_GameMatchOptionBuffer.insert(m_GameMatchOptionBuffer.end(), m_GameMatchOptionBuffer.begin(), m_GameMatchOptionBuffer.end());
...
bool CMatchOptionBuffer::DeleteMatchOption(DWORD dwMatchID, DWORD wMatchNO) {
  ...
  // 查找对象
  for (size_t i = 0; i < m_GameMatchOptionArray.size(); i++) {
    pGameMatchOption = m_GameMatchOptionArray[i];
    if (pGameMatchOption->dwMatchID == dwMatchID && pGameMatchOption->lMatchNo == wMatchNO) {
      // 删除数据
      m_GameMatchOptionBuffer.emplace_back(pGameMatchOption);
      m_GameMatchOptionArray.erase(m_GameMatchOptionArray.begin() + i);
...
bool CMatchOptionBuffer::InsertMatchOption(tagGameMatchOption* pGameMatchOption) {
  ...
  // 设置数据
  m_GameMatchOptionArray.emplace_back(pGameMatchInsert);
...
DWORD CMatchOptionBuffer::GetMatchOptionCount() {
  return (DWORD) (m_GameMatchOptionArray.size());
}
...
tagGameMatchOption* CMatchOptionBuffer::SearchMatchOption(DWORD dwMatchID, LONGLONG wMatchNO) {
  ...
  // 查找对象
  for (INT_PTR i = 0; i < m_GameMatchOptionArray.size(); i++) {
    pGameMatchOption = m_GameMatchOptionArray[i];
    if (pGameMatchOption->dwMatchID == dwMatchID && pGameMatchOption->lMatchNo == wMatchNO) {
...
tagGameMatchOption* CMatchOptionBuffer::CreateMatchOption() {
  ...
  // 创建对象
  if (!m_GameMatchOptionBuffer.empty()) {
    pGameMatchOption = m_GameMatchOptionBuffer.back();
    m_GameMatchOptionBuffer.pop_back();
  } else {
    pGameMatchOption = new (std::nothrow) tagGameMatchOption;
    if (pGameMatchOption == nullptr) {
      return nullptr;
    }
  }
  // 设置变量
...
bool CServerInfoManager::InsertGameServer(tagGameServerCreate* pGameServerCreate, tagGameServerInfo& GameServerResult) {
  ...
  try {
    ...
    for (INT i = 0; i < CountArray(pGameServerCreate->cbCustomRule); i++) {
      SnprintfT(&szCustomRule[i * 2], 3, TEXT("%02X"), pGameServerCreate->cbCustomRule[i]);
    }
    TCHAR szPersonalRule[CountArray(pGameServerCreate->cbPersonalRule) * 2 + 1] = TEXT("");
    for (INT i = 0; i < CountArray(pGameServerCreate->cbPersonalRule); i++) {
      SnprintfT(&szPersonalRule[i * 2], 3, TEXT("%02X"), pGameServerCreate->cbPersonalRule[i]);
    }
    ...
    if (pGameServerCreate->wServerType == GAME_GENRE_PERSONAL) {
      InsertPersonalRoomParameter(PlatformDBAide, m_PersonalRoomOption, m_PersonalRoomOptionRightAndFee, GameServerResult.wServerID,
                                  GameServerResult.wKindID);
    }
...
bool CServerInfoManager::ModifyGameServer(tagGameServerCreate* pGameServerCreate, tagGameServerInfo& GameServerResult) {
  ...
  try {
    ...
    if (pGameServerCreate->wServerType == GAME_GENRE_PERSONAL) {
      // 修改私人房间参数
      ModifyPersonalRoomParameter(PlatformDBAide, m_PersonalRoomOption, m_PersonalRoomOptionRightAndFee, GameServerResult.wServerID,
                                  GameServerResult.wKindID);
    }
    return true;
  } catch (IDataBaseException* pIException) {
...
bool CServerInfoManager::ConnectPlatformDB(CDataBaseHelper& PlatformDBModule) {
  ...
  if ((PlatformDBModule.GetInterface() == NULL) && (PlatformDBModule.CreateInstance() == false)) {
    CLogger::Error(TEXT("CreateGameServer 创建 PlatformDBModule 对象失败"));
    return false;
  }
...
bool CServerInfoManager::ReadGameServerInfo(CDataBaseAide& PlatformDBAide, tagGameServerInfo& GameServerResult) {
  ...
  if (szPersonalRule[0] != 0) {
    ...
    for (INT i = 0; i < nPersonalRuleSize; i++) {
      ...
      // 生成结果
      if ((cbChar2 >= TEXT('0')) && (cbChar2 <= TEXT('9'))) {
        GameServerResult.cbPersonalRule[i] += (cbChar2 - TEXT('0'));
      }
      if ((cbChar2 >= TEXT('A')) && (cbChar2 <= TEXT('F'))) {
        GameServerResult.cbPersonalRule[i] += (cbChar2 - TEXT('A') + 0x0A);
      }

      // 生成结果
      if ((cbChar1 >= TEXT('0')) && (cbChar1 <= TEXT('9'))) {
        GameServerResult.cbPersonalRule[i] += (cbChar1 - TEXT('0')) * 0x10;
      }
      if ((cbChar1 >= TEXT('A')) && (cbChar1 <= TEXT('F'))) {
        GameServerResult.cbPersonalRule[i] += (cbChar1 - TEXT('A') + 0x0A) * 0x10;
      }
    }
  }
...
// 加载私人房间配置
bool CServerInfoManager::LoadPersonalRoomParameter(tagPersonalRoomOption& pesronalRoomOption,
                                                   tagPersonalRoomOptionRightAndFee& personalRoomOptionRightAndFee, WORD wServerID) {
  // 变量定义
  CDataBaseAide PlatformDBAide;
  CDataBaseHelper PlatformDBModule;
...
// 插入私人房间参数
bool CServerInfoManager::InsertPersonalRoomParameter(CDataBaseAide PlatformDBAide, tagPersonalRoomOption PersonalRoomOption,
                                                     tagPersonalRoomOptionRightAndFee PersonalRoomOptionRightAndFee, WORD dwServerID, WORD dwKindID) {
  try {
    // 插入参数
    PlatformDBAide.ResetParameter();
    ...
    if (PersonalRoomOptionRightAndFee.stCreateRoomPayFee[0].dwPlayTimeLimit == 0 &&
        PersonalRoomOptionRightAndFee.stCreateRoomPayFee[0].dwPlayTurnLimit == 0) {
    ...
    for (int i = 0; i < FEE_OPTION_COUNT; i++) {
      if (PersonalRoomOptionRightAndFee.stCreateRoomPayFee[i].dwPlayTimeLimit != 0 ||
          PersonalRoomOptionRightAndFee.stCreateRoomPayFee[i].dwPlayTurnLimit != 0) {
        // 插入参数
        PlatformDBAide.ResetParameter();
        PlatformDBAide.AddParameter(TEXT("@wKindID"), dwKindID);
...
// 设置私人房间参数
void CServerInfoManager::SetPersonalRoomOption(tagPersonalRoomOption PersonalRoomOption,
...
// 修改私人房间参数
bool CServerInfoManager::ModifyPersonalRoomParameter(CDataBaseAide PlatformDBAide, tagPersonalRoomOption PersonalRoomOption,
                                                     tagPersonalRoomOptionRightAndFee PersonalRoomOptionRightAndFee, WORD dwServerID, WORD dwKindID) {
  try {
    // 插入参数
    PlatformDBAide.ResetParameter();
...
inline void CServerInfoManager::WriteFileValue(LPCTSTR pszKeyName, LPCTSTR pszItemName, UINT nValue, CSimpleIni& ini_reader) {
  ini_reader.SetLongValue(pszKeyName, pszItemName, nValue);
}

bool CServerInfoManager::WritePersonalRoomInfo(tagPersonalRoomOption PersonalRoomOption,
                                               tagPersonalRoomOptionRightAndFee PersonalRoomOptionRightAndFee, WORD dwKindID) {
  // 获取路径
  TCHAR szWorkDir[MAX_PATH] = TEXT("");
  CWHService::GetWorkDirectory(szWorkDir, CountArray(szWorkDir));

  // 构造路径
  const auto szIniFile = std::filesystem::path(StringT(szWorkDir)) / std::format(TEXT("{}.ini"), dwKindID);

  // 读取配置
  CSimpleIni ini_reader;
  if (ini_reader.LoadFile(szIniFile.c_str()) != SI_OK) {
    return false;
  }

  WriteFileValue(TEXT("Enable"), TEXT("Enabled"), 1, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("CardOrBean"), PersonalRoomOption.cbCardOrBean, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("FeeCardOrBeanCount"), PersonalRoomOption.lFeeCardOrBeanCount, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("IsJoinGame"), PersonalRoomOption.cbIsJoinGame, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("MinPeople"), PersonalRoomOption.cbMinPeople, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("MaxCellScore"), PersonalRoomOption.cbMaxPeople, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("MaxCellScore"), PersonalRoomOption.lMaxCellScore, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("PersonalRoomTax"), PersonalRoomOption.lPersonalRoomTax, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("CanCreateCount"), PersonalRoomOption.wCanCreateCount, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("PlayTurnCount"), PersonalRoomOption.dwPlayTurnCount, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("PlayTimeLimit"), PersonalRoomOption.dwPlayTimeLimit, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("TimeAfterBeginCount"), PersonalRoomOption.dwTimeAfterBeginCount, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("TimeOffLineCount"), PersonalRoomOption.dwTimeOffLineCount, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("TimeNotBeginGame"), PersonalRoomOption.dwTimeNotBeginGame, ini_reader);
  WriteFileValue(TEXT("Option"), TEXT("TimeAfterCreateRoom"), PersonalRoomOption.dwTimeAfterCreateRoom, ini_reader);

  // 创建私人房间的权限及费用
  WriteFileValue(TEXT("CreateRoomRight"), TEXT("CreateRight"), PersonalRoomOptionRightAndFee.cbCreateRight, ini_reader);
  ...
  for (int i = 0; i < FEE_OPTION_COUNT; i++) {
    SnprintfT(szPlayTurnLimit, CountArray(szPlayTurnLimit), TEXT("PlayTurnLimit_%d"), i);
    WriteFileValue(TEXT("FeeParameter"), szPlayTurnLimit, PersonalRoomOptionRightAndFee.stCreateRoomPayFee[i].dwPlayTurnLimit, ini_reader);

    SnprintfT(szPayFee, CountArray(szPayFee), TEXT("PayFee_%d"), i);
    WriteFileValue(TEXT("FeeParameter"), szPayFee, PersonalRoomOptionRightAndFee.stCreateRoomPayFee[i].dwPayFee, ini_reader);

    SnprintfT(szIniScore, CountArray(szIniScore), TEXT("IniScore_%d"), i);
    WriteFileValue(TEXT("FeeParameter"), szIniScore, PersonalRoomOptionRightAndFee.stCreateRoomPayFee[i].dwIniScore, ini_reader);
  }
