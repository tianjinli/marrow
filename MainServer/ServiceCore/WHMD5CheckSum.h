...
public:
  virtual ~CWHMD5Checksum() = default;

  // 静态函数
  static std::string GetMD5(BYTE* buf, UINT length);
  static std::string GetMD5(const std::filesystem::path& file_path);

private:
  // 构造函数
  CWHMD5Checksum() = default;

  // 辅助函数
  std::string Final();
  VOID Transform(BYTE block[64]);
  VOID Update(BYTE* input, ULONG input_len);
  ...
  // 存储变量
  BYTE buffer_[64] = {}; // 输入缓冲
  DWORD count_[2] = {}; // 位数计数
  DWORD md5_sum_[4] = {MD5_INIT_STATE_0, MD5_INIT_STATE_1, MD5_INIT_STATE_2, MD5_INIT_STATE_3}; // MD5校验和
};
