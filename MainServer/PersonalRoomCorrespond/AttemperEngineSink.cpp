...
bool CAttemperEngineSink::OnEventTCPNetworkShut(DWORD dwSocketID, DWORD dwClientAddr, time_t dwActiveTime) {
  ...
  if (pBindParameter->ServiceKind == ServiceKind_Game) {
    ...
    if (wBindIndex == m_wCollectItem) {
      ...
      // 汇总切换
      if (m_WaitCollectItemArray.size() > 0) {
        // 提取变量
      ...
      // 删除等待
      if (auto itr = std::find(m_WaitCollectItemArray.begin(), m_WaitCollectItemArray.end(), wBindIndex); itr != m_WaitCollectItemArray.end()) {
        m_WaitCollectItemArray.erase(itr);
      }
    }
...
bool CAttemperEngineSink::OnTCPNetworkMainRegister(WORD wSubCmdID, VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  switch (wSubCmdID) {
    case SUB_CS_C_REGISTER_SERVER: // 注册房间
    {
      ...
      if (m_GlobalInfoManager.SearchServerItem(pRegisterServer->wServerID) != NULL) {
        ...
        lstrcpyn(RegisterFailure.szDescribeString, TEXT("已经存在相同标识的游戏房间服务，房间服务注册失败"),
                 CountArray(RegisterFailure.szDescribeString));
      ...
      if (m_wCollectItem == INVALID_WORD) {
        ...
        m_WaitCollectItemArray.emplace_back(wBindIndex);
      }
    ...
    case SUB_CS_C_REGISTER_PERSONAL: {
      ...
      if (wDataSize != sizeof(CMD_CS_S_RegisterPersonal)) {
        return false;
      }
...
bool CAttemperEngineSink::OnTCPNetworkMainServiceInfo(WORD wSubCmdID, VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  switch (wSubCmdID) {
    ...
    case SUB_CS_C_SEARCH_TABLE: // 查找桌子
    {
      ...
      if (wDataSize != sizeof(CMD_CS_C_SearchTable)) {
        return false;
      }
      ...
      // 查找桌子
      StringT szPersonalTableID = ToSimpleString(pSearchTable->szPersonalTableID);
      tagPersonalTableInfo* pPersonalTable = m_GlobalInfoManager.SearchTableByID(szPersonalTableID.c_str());
    ...
    case SUB_CS_C_DISSUME_SEARCH_TABLE: // 查找桌子
    {
      ...
      m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_SERVICE_INFO, SUB_CS_S_DISSUME_SEARCH_TABLE_RESULT, &SearchTableResult,
                                     sizeof(SearchTableResult));
    ...
    case SUB_CS_C_QUERY_GAME_SERVER: // 查询可用房间
    {
      ...
      if (wDataSize != sizeof(CMD_CS_C_QueryGameServer)) {
        return false;
      }
      ...
      if (m_GlobalInfoManager.GetFreeServerCount(pQueryGameServer->dwKindID) > 0) {
        if (m_GlobalInfoManager.CanCreatePersonalRoom(pQueryGameServer->dwKindID, pQueryGameServer->dwUserID)) {
          ...
          if (dwServerID == 0) {
            lstrcpyn(QueryGameServerResult.szErrDescrybe, TEXT("房主必须参与游戏模式下只能创建一个房间，或游戏服务器创建约战房间已满！"),
                     sizeof(QueryGameServerResult.szErrDescrybe));
          }
        } else {
          lstrcpyn(QueryGameServerResult.szErrDescrybe, TEXT("已经达到创建约战房间的最大数，不能再创建房间！"),
                   sizeof(QueryGameServerResult.szErrDescrybe));
        }
      } else {
        lstrcpyn(QueryGameServerResult.szErrDescrybe, TEXT("当前游戏的约战房服务器不存在，请选择其他游戏！"),
                 sizeof(QueryGameServerResult.szErrDescrybe));
      }
      ...
      m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_SERVICE_INFO, SUB_CS_S_QUERY_GAME_SERVER_RESULT, &QueryGameServerResult,
                                     sizeof(CMD_CS_S_QueryGameServerResult));
    ...
    case SUB_CS_C_CREATE_TABLE: // 创建桌子
    {
      ...
      if (wDataSize != sizeof(CMD_CS_C_CreateTable)) {
        return false;
      }
      ...
      lstrcpyn(CreateTableResult.PersonalTable.szPassword, pCreateTable->PersonalTable.szPassword,
               CountArray(CreateTableResult.PersonalTable.szPassword));
      ...
      m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_SERVICE_INFO, SUB_CS_S_CREATE_TABLE_RESULT, &CreateTableResult,
                                     sizeof(CMD_CS_S_CreateTableResult));
    ...
    case SUB_CS_C_DISMISS_TABLE: // 解散桌子
    {
      ...
      if (wDataSize != sizeof(CMD_CS_C_DismissTable)) {
        return false;
      }
      ...
      tagPersonalTableInfo* pPersonalTableInfo =
          m_GlobalInfoManager.SearchTableByTableIDAndServerID(pDismissTable->dwServerID, pDismissTable->dwTableID);
      ...
      m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_SERVICE_INFO, SUB_CS_C_DISMISS_TABLE_RESULT, &DismissTableResult,
                                     sizeof(CMD_CS_C_DismissTableResult));
    ...
    case SUB_CS_S_QUERY_PERSONAL_ROOM_LIST: // 请求房间列表
    {
      ...
      if (wDataSize != sizeof(CMD_MB_SC_QeuryPersonalRoomList)) {
        return false;
      }
      ...
      m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_SERVICE_INFO, SUB_CS_C_QUERY_PERSONAL_ROOM_LIST_RESULT, &cmdHostCreatRoomInfo,
  ...
  switch (wSubCmdID) {
    case SUB_CS_C_USER_ENTER: // 用户进入
    {
      ...
      do {
        ...
        // 发送状态
        m_pITCPNetworkEngine->SendData(pGlobalChatItem->m_ChatServer.dwSocketID, MDM_CS_USER_COLLECT, SUB_CS_S_USER_GAMESTATE, &UserGameStatus,
  ...
  switch (wSubCmdID) {
    case SUB_CS_C_SEARCH_CORRESPOND: // 协调查找
    {
      ...
      if (pSearchCorrespond->szNickName[0] != 0) {
        ...
        StringT nick_name = StringUtils::ToString(pSearchCorrespond->szNickName);
        std::for_each(nick_name.begin(), nick_name.end(), [](TCHAR& c) {
          c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
        });
        ResultUserItem[1] = m_GlobalInfoManager.SearchUserItemByNickName(nick_name.c_str());
      }
