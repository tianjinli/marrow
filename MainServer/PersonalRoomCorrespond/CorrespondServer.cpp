...
// 启动服务
VOID CCorrespondServer::StartService() {
  // 启动服务
  try {
    m_ServiceUnits.StartService();
  } catch (const std::exception& e) {
    CLogger::Error("StartService exception: {}", e.what());
    ASSERT(FALSE);
  } catch (...) {
    ASSERT(FALSE);
  }
...
// 停止服务
VOID CCorrespondServer::ConcludeService() {
  // 停止服务
  try {
    m_ServiceUnits.ConcludeService();
  } catch (const std::exception& e) {
    CLogger::Error("ConcludeService exception: {}", e.what());
    ASSERT(FALSE);
  } catch (...) {
    ASSERT(FALSE);
  }
