...
INT CGlobalInfoManager::GetHostCreatePersonalRoomCount(DWORD dwUserID, DWORD dwKindID) {
  return std::ranges::count_if(m_MapPersonalTableInfo | std::views::values, [dwUserID, dwKindID](const auto& info) {
    return info != nullptr && info->dwUserID == dwUserID && info->dwKindID == dwKindID;
  });
}
