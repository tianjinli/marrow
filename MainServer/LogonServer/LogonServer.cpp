...
// 启动服务
VOID CLogonServer::StartService() {
  // 启动服务
  try {
    m_ServiceUnits.StartService();
  } catch (const std::exception& e) {
    CLogger::Error("StartService failed: %s", e.what());
    ASSERT(FALSE);
  } catch (...) {
    ASSERT(FALSE);
  }
...
// 停止服务
VOID CLogonServer::ConcludeService() {
  // 停止服务
  try {
    m_ServiceUnits.ConcludeService();
  } catch (const std::exception& e) {
    CLogger::Error("ConcludeService failed: %s", e.what());
    ASSERT(FALSE);
  } catch (...) {
    ASSERT(FALSE);
  }
