...
bool CServiceUnits::ConcludeService() {
  ...
  if (m_PsrnlRmTCPSocketService.GetInterface() != NULL) {
    m_PsrnlRmTCPSocketService->ConcludeService();
  }
...
bool CServiceUnits::InitializeService() {
  ...
  if ((m_PsrnlRmTCPSocketService.GetInterface() == NULL) && (m_PsrnlRmTCPSocketService.CreateInstance() == false)) {
    return false;
  }
  ...
  if (m_DataBaseEngine->SetDataBaseEngineSink(&m_DataBaseEngineSink) == false) {
    return false;
  }
  ...
  // 约战服务
  if (m_PsrnlRmTCPSocketService->SetServiceID(NETWORK_PERSONAL_ROOM_CORRESPOND) == false) {
    return false;
  }
  if (m_PsrnlRmTCPSocketService->SetTCPSocketEvent(&m_AttemperEngineSink) == false) {
    return false;
  }
  ...
  m_DataBaseEngineSink.m_pIDataBaseEngineEvent = &m_AttemperEngineSink;
...
bool CServiceUnits::StartKernelService() {
  ...
  // 约战引擎
  if (m_PsrnlRmTCPSocketService->StartService(io_context_) == false) {
    ASSERT(FALSE);
    return false;
  }
