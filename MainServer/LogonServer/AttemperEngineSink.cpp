...
CAttemperEngineSink::~CAttemperEngineSink() {
  // 清理资源
  for (auto& pValidateCode: m_VerifyCodeArray) {
...
bool CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx* pIUnknownEx) {
  ...
  {
    // 构造路径
    const auto szIniFile = std::filesystem::path(StringT(szWorkDir)) / TEXT("PlazaOptionConfig.ini");
    ...
    // 读取配置
    m_bShowServerStatus = IniData.ReadInt(TEXT("ServerStatus"), TEXT("ShowServerStatus"), m_bShowServerStatus);
  }
  {
    // 构造路径
    const auto szIniFile = std::filesystem::path(StringT(szWorkDir)) / TEXT("VideoOption.ini");
    ...
    m_wAVServerPort = IniData.ReadInt(TEXT("VideoOption"), TEXT("ServerPort"), m_wAVServerPort);
    IniData.ReadString(TEXT("VideoOption"), TEXT("ServerAddr"), TEXT("127.0.0.1"), szServerAddr, LEN_SERVER);
...
bool CAttemperEngineSink::OnEventControl(WORD wIdentifier, VOID* pData, WORD wDataSize) {
  switch (wIdentifier) {
    ...
    case CT_CONNECT_CORRESPOND: // 连接协调
    {
      ...
      CLogger::Info(TEXT("正在连接协调服务器 [ {}:{} ]..."), pCorrespondAddress->szAddress, m_pInitParameter->m_wCorrespondPort);
      return true;
    }
    case CT_CONNECT_PERSONAL_ROOM_CORRESPOND: // 连接约战
    {
      ...
      CLogger::Info(TEXT("正在连接约战服务器 [ {}:{} ]..."), pPrsnlRmCorrespondAddress->szAddress, m_pInitParameter->m_wPrsnlRmCorrespondPort);
      return true;
    }
  }
...
bool CAttemperEngineSink::OnEventTCPSocketShut(WORD wServiceID, BYTE cbShutReason) {
  ...
  if (wServiceID == NETWORK_CORRESPOND) {
    ...
    if (m_bNeekCorrespond == true) {
      // 提示消息
      CLogger::Warn(TEXT("与协调服务器的连接关闭了，{} 秒后将重新连接"), m_pInitParameter->m_wConnectTime);
  ...
  // 约战连接
    ...
    if (m_bNeekPrsnlRmCorrespond == true) {
      // 提示消息
      CLogger::Warn(TEXT("与约战服务器的连接关闭了，{} 秒后将重新连接"), m_pInitParameter->m_wConnectTime);
...
bool CAttemperEngineSink::OnEventTCPSocketLink(WORD wServiceID, INT nErrorCode) {
  ...
  if (wServiceID == NETWORK_CORRESPOND) {
    ...
    if (nErrorCode != 0) {
      // 提示消息
      CLogger::Warn(TEXT("协调服务器连接失败 [ {} ]，{} 秒后将重新连接"), nErrorCode, m_pInitParameter->m_wConnectTime);
    ...
    if (nErrorCode != 0) {
      // 提示消息
      CLogger::Warn(TEXT("约战服务器连接失败 [ {} ]，{} 秒后将重新连接"), nErrorCode, m_pInitParameter->m_wConnectTime);
...
bool CAttemperEngineSink::OnTCPSocketMainRegister(WORD wSubCmdID, VOID* pData, WORD wDataSize) {
  switch (wSubCmdID) {
    case SUB_CS_S_REGISTER_FAILURE: // 注册失败
    {
      ...
      // 显示消息
      StringT describe_string = ToSimpleString(pRegisterFailure->szDescribeString);
      if (!describe_string.empty()) {
        CLogger::Error(TEXT("{} → {}"), TEXT(__FUNCTION__), describe_string);
      }
...
bool CAttemperEngineSink::OnTCPSocketPersonalServiceInfo(WORD wSubCmdID, VOID* pData, WORD wDataSize) {
  switch (wSubCmdID) {
    case SUB_CS_S_QUERY_GAME_SERVER_RESULT: // 查询结果
    {
      ...
      if ((m_pBindParameter + LOWORD(pQueryGameServerResult->dwSocketID))->dwSocketID != pQueryGameServerResult->dwSocketID) {
        return true;
      }
      ...
      // 发送数据
      m_pITCPNetworkEngine->SendData(pQueryGameServerResult->dwSocketID, MDM_MB_PERSONAL_SERVICE, SUB_MB_QUERY_GAME_SERVER_RESULT,
                                     &QueryGameServerResult, sizeof(CMD_MB_QueryGameServerResult));
    ...
    case SUB_CS_S_SEARCH_TABLE_RESULT: {
      ...
      if ((m_pBindParameter + LOWORD(pSearTableResult->dwSocketID))->dwSocketID != pSearTableResult->dwSocketID) {
        return true;
      }
      ...
      // 发送消息
      m_pITCPNetworkEngine->SendData(pSearTableResult->dwSocketID, MDM_MB_PERSONAL_SERVICE, SUB_MB_SEARCH_RESULT, &SearchResult,
    ...
    case SUB_CS_S_DISSUME_SEARCH_TABLE_RESULT: {
      ...
      // 发送消息
      m_pITCPNetworkEngine->SendData(pSearTableResult->dwSocketID, MDM_MB_PERSONAL_SERVICE, SUB_MB_DISSUME_SEARCH_RESULT, &SearchResult,
    ...
    case SUB_CS_C_QUERY_PERSONAL_ROOM_LIST_RESULT: {
      ...
      // 向数据可发送查询每个房间信息的请求
      m_pIDataBaseEngine->PostDataBaseRequest(DBR_MB_QUERY_PERSONAL_ROOM_INFO, pHostCreatRoomInfo->wSocketID, pHostCreatRoomInfo,
...
bool CAttemperEngineSink::OnTCPSocketMainRemoteService(WORD wSubCmdID, VOID* pData, WORD wDataSize) {
  switch (wSubCmdID) {
    case SUB_CS_S_SEARCH_CORRESPOND: // 协调查找
    {
      ...
      m_pITCPNetworkEngine->SendData(pSearchCorrespond->dwSocketID, MDM_GP_REMOTE_SERVICE, SUB_GP_S_SEARCH_CORRESPOND, &SearchCorrespond,
                                     wHeadSize + wItemSize);
    ...
    case SUB_CS_S_SEARCH_ALLCORRESPOND: {
      ...
      m_pITCPNetworkEngine->SendData(pSearchAllCorrespond->dwSocketID, MDM_GP_REMOTE_SERVICE, SUB_GP_S_SEARCH_ALLCORRESPOND, pSearchCorrespond,
                                     cbDataSize);
      return true;
    }
  }
    ...
    case SUB_GP_GET_ONLINE: // 获取在线
    {
      ...
      for (WORD i = 0; i < pGetOnline->wServerCount; i++) {
        ...
        if (pGameServerItem != NULL) {
          ...
          ServerOnline.OnLineInfoServer[wServerIndex].dwOnLineCount =
              pGameServerItem->m_GameServer.dwOnLineCount + pGameServerItem->m_GameServer.dwAndroidCount;
        }
...
bool CAttemperEngineSink::OnTCPNetworkMainPCUserService(WORD wSubCmdID, VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  switch (wSubCmdID) {
    ...
    case SUB_GP_USER_SAVE_SCORE: // 存入游戏币
    {
      ...
      if (pUserSaveScore->lSaveScore <= 0) {
        return false;
      }
      // 构造数据
  ...
  switch (wSubCmdID) {
    ...
    case SUB_GP_C_SEARCH_ALLCORRESPOND: {
      ...
      if (wDataSize >= sizeof(CMD_GP_C_SearchAllCorrespond)) {
        ...
        m_pITCPSocketService->SendData(MDM_CS_REMOTE_SERVICE, SUB_CS_C_SEARCH_ALLCORRESPOND, pSearchCorrespond,
...
bool CAttemperEngineSink::OnTCPNetworkSubPCQueryVerifyCode(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 查找是否已申请
  INT i = 0;
  for (i = 0; i < m_VerifyCodeArray.size(); i++) {
    tagVerifyCode* pVerifyCode = m_VerifyCodeArray[i];
    if (pVerifyCode == NULL) {
      continue;
    }

    StringT machine_id = ToSimpleString(pQueryVerifyCode->szMachineID);
    if (pVerifyCode->dwClientAddr == dwClientAddr && machine_id == pVerifyCode->szMachineID) {
      if (dwCurrentTime - pVerifyCode->dwUpdateTime < TIME_VALID_VERIFY_CODE) {
        ...
        lstrcpyn(VerifyCodeResult.szDescString, TEXT("抱歉，您在短时间内查询次数过于频繁，请稍后重试！"), CountArray(VerifyCodeResult.szDescString));
  ...
  // 新增请求项
  if (i == m_VerifyCodeArray.size()) {
    tagVerifyCode* pNewVerifyCode = new (std::nothrow) tagVerifyCode;
    ...
    m_VerifyCodeArray.emplace_back(pNewVerifyCode);
...
bool CAttemperEngineSink::OnTCPNetworkSubMBDissumeSearchServerTable(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  // if (m_pIPersonalRoomServiceManager)
  //{
  //	return m_pIPersonalRoomServiceManager->OnTCPNetworkSubMBDissumeSearchServerTable(pData, wDataSize, dwSocketID, m_pBindParameter,
  // m_pITCPSocketService);
  // }

  // 校验数据
  ASSERT(wDataSize == sizeof(CMD_MB_DissumeSearchServerTable));
  if (wDataSize != sizeof(CMD_MB_DissumeSearchServerTable)) {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubMBQueryPersonalRoomList(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 构造数据
  if (m_pPrsnlRmITCPSocketService) {
    m_pPrsnlRmITCPSocketService->SendData(MDM_CS_SERVICE_INFO, SUB_CS_S_QUERY_PERSONAL_ROOM_LIST, &SC_QeuryPersonalRoomList,
...
bool CAttemperEngineSink::OnDBPCUserInsureUserInfo(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  // 发送数据
  m_pITCPNetworkEngine->SendData(dwContextID, MDM_GP_USER_SERVICE, SUB_GP_QUERY_USER_INFO_RESULT, &UserTransferUserInfo,
                                 sizeof(UserTransferUserInfo));
...
bool CAttemperEngineSink::OnDBPCTaskList(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  // 拷贝数据
  WORD wTaskCount = std::min((size_t) pTaskListInfo->wTaskCount, CountArray(m_TaskParameterBuffer) - m_wTaskCountBuffer);
...
bool CAttemperEngineSink::OnDBMBPersonalRoomListInfo(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_PERSONAL_SERVICE, SUB_MB_QUERY_PERSONAL_ROOM_LIST_RESULT, &PersonalRoomInfoList,
...
bool CAttemperEngineSink::OnDBPCGameListResult(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  if (pGameListResult->cbSuccess == TRUE) {
    ...
    // 提示消息
    CLogger::Warn(TEXT("服务器列表加载失败，{} 秒后将重新加载"), m_pInitParameter->m_wReLoadListTime);
...
VOID CAttemperEngineSink::SendGamePropertyRelatInfo(DWORD dwSocketID) {
  ...
  for (DWORD i = 0; i < dwCount; i++) {
    ...
    if (pGamePropertyRelatItem == NULL) {
      break;
    }
...
VOID CAttemperEngineSink::SendMemberConfig(DWORD dwContextID) {
  ...
  m_pITCPNetworkEngine->SendData(dwContextID, MDM_GP_LOGON, SUB_GP_MEMBER_PARAMETER_RESULT, &MemberParameterResult,
                                 wConfigMemberHead + wConfigMemberInfo);
}
