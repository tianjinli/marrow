...
bool CDataBaseEngineSink::OnRequestVerifyIndividual(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  try {
    ...
    StringT DBVarValue;
    m_AccountsDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
    lstrcpyn(VerifyIndividualResult.szErrorMsg, DBVarValue.c_str(), CountArray(VerifyIndividualResult.szErrorMsg));
...
bool CDataBaseEngineSink::OnRequestModifyMachine(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  try {
    ...
    if (m_AccountsDBAide.GetReturnValue() == DB_SUCCESS) {
      ...
      lstrcpyn(OperateSuccess.szDescribeString, DBVarValue.c_str(), CountArray(OperateSuccess.szDescribeString));
      ...
      lstrcpyn(OperateFailure.szDescribeString, DBVarValue.c_str(), CountArray(OperateFailure.szDescribeString));
...
bool CDataBaseEngineSink::OnRequestModifyCustomFace(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  try {
    ...
    // 头像信息
    std::vector<uint8_t> VarChunk(pcbCustomFace, pcbCustomFace + CustomFaceSize);
    pIDataBase->AddParameter(TEXT("@dwCustomFace"), adParamInput, VarChunk);
...
bool CDataBaseEngineSink::OnRequsetDissumeRoomTest(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  cbPersonalRoomInfo.reserve(PERSONAL_ROOM_CHAIR * sizeof(tagPersonalUserScoreInfo));
  ...
  try {
    ...
    for (int i = 0; i < MAX_CREATE_PERSONAL_ROOM; i++) {
      ...
      if (DB_SUCCESS == lResultCode) {
        ...
        lstrcpyn(PersonalRoomInfoList.PersonalRoomInfo[i].szRoomID, pGetParameter->HostCreatRoomInfo.szRoomID[i],
                 CountArray(PersonalRoomInfoList.PersonalRoomInfo[i].szRoomID));
    ...
    for (int i = 0; i <= iCount; i++) {
      for (int j = i + 1; j <= iCount; j++) {
        if (DiffSystemTime(&PersonalRoomInfoList.PersonalRoomInfo[i].sysCreateTime, &PersonalRoomInfoList.PersonalRoomInfo[j].sysCreateTime) > 0) {
  ...
  try {
    ...
    while (m_PlatformDBModule->IsRecordsetEnd() == false) {
      ...
      m_PlatformDBAide.GetValue_String(TEXT("RegulationsInfo"), pGamePropertyItem->szRegulationsInfo,
                                       CountArray(pGamePropertyItem->szRegulationsInfo));
      // 设置位移
    ...
    if (lResultCode == DB_SUCCESS) {
      ...
      while (true) {
        ...
        if (dwDataBufferSize >= SOCKET_TCP_PACKET - sizeof(PropertyInfo)) {
          // 构造提示
          CLogger::Error(TEXT("{} → 发送道具数量: {}"), TEXT(__FUNCTION__), pQueryBackpackResult->dwCount);
...
bool CDataBaseEngineSink::OnRequestPropertyBuff(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  try {
    ...
    for (WORD i = 0; i < CountArray(GamePropertyBuff.PropertyBuff); i++) {
      ...
      SYSTEMTIME st;
      m_PlatformDBAide.GetValue_SystemTime(TEXT("UseTime"), st);
      GamePropertyBuff.PropertyBuff[i].tUseTime = GetUnixTime(&st);
  ...
  try {
    ...
    if (lResultCode == 0) {
      ...
      for (WORD i = 0;; i++) {
        ...
        QuerySendPresenResult.Present[i].tSendTime = GetUnixTime(&st);
  ...
  try {
    ...
    if (lResultCode == 0) {
      ...
      for (WORD i = 0;; i++) {
        ...
        SYSTEMTIME st;
        m_PlatformDBAide.GetValue_SystemTime(TEXT("SendTime"), st);
        GetSendPresenResult.Present[i].tSendTime = GetUnixTime(&st);
...
bool CDataBaseEngineSink::OnRequestOnLineCountInfo(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  try {
    ...
    for (WORD i = 0; i < pOnLineCountInfo->wKindCount; i++) {
      ...
      SnprintfT(&szOnLineCountKind[nLength], CountArray(szOnLineCountKind) - nLength, TEXT("%d:%ld;"), pOnLineCountInfo->OnLineCountKind[i].wKindID,
    ...
    for (WORD j = 0; j < pOnLineCountInfo->wKindCount; j++) {
      ...
      SnprintfT(&szAndroidCountKind[nLength], CountArray(szAndroidCountKind) - nLength, TEXT("%d:%ld;"), pOnLineCountInfo->OnLineCountKind[j].wKindID,
                pOnLineCountInfo->OnLineCountKind[j].dwAndroidCount);
    }
  ...
  if (bMobileClient == false) {
    if (dwErrorCode == DB_SUCCESS) {
      ...
      try {
        ...
        CLogger::Error(TEXT("{} {} 出现异常"), TEXT(__FUNCTION__), TEXT("GSP_GP_LoadAgentGameKindItem"));
      }
      //////////////////////////////////////////////////////////////////////////
