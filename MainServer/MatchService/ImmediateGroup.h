...
// 数组定义
using CUserMatchTimerArray = std::vector<tagMatchTimer*>; // 比赛定时器数组
using CUserMatchScoreArray = std::vector<tagMatchScore*>; // 比赛分数数组
using CMatchUserItemArray = std::vector<IServerUserItem*>; // 比赛用户数组
using CTableFrameMananerArray = std::vector<tagTableFrameInfo*>; // 比赛桌数组

// 字典定义
using CUserItemMap = std::unordered_map<DWORD, IServerUserItem*>;
using CUserSeatMap = std::unordered_map<IServerUserItem*, DWORD>;
...
public:
  // 开始比赛
  virtual bool OnEventMatchStart(CImmediateGroup * pMatch) = 0;
  // 比赛结束
  virtual bool OnEventMatchOver(CImmediateGroup * pMatch) = 0;

  // 定时器接口
public:
  // 删除定时器
  virtual bool KillGameTimer(DWORD dwTimerID, CImmediateGroup * pMatch) = 0;
  // 设置定时器
  virtual bool SetGameTimer(DWORD dwTimerID, DWORD dwElapse, DWORD dwRepeat, WPARAM dwBindParameter, CImmediateGroup * pMatch) = 0;

  // 发送函数
public:
  // 发送数据
  virtual bool SendMatchInfo(IServerUserItem * pIServerUserItem) = 0;
  // 发送信息
  virtual bool SendGroupUserMessage(LPCTSTR pStrMessage, CImmediateGroup * pMatch) = 0;
  // 发送分数
  virtual bool SendGroupUserScore(IServerUserItem * pIServerUserItem, CImmediateGroup * pMatch) = 0;
  // 发送状态
  virtual bool SendGroupUserStatus(IServerUserItem * pIServerUserItem, CImmediateGroup * pMatch) = 0;
  // 发送消息
  virtual bool SendGameMessage(IServerUserItem * pIServerUserItem, LPCTSTR lpszMessage, WORD wMessageType) = 0;
  // 发送消息
  virtual bool SendRoomMessage(IServerUserItem * pIServerUserItem, LPCTSTR lpszMessage, WORD wMessageType) = 0;
  // 发送数据
  virtual bool SendGroupData(WORD wMainCmdID, WORD wSubCmdID, void* pData, WORD wDataSize, CImmediateGroup* pMatch) = 0;
  // 发送数据
...
public:
  // 插入空的桌子
  virtual void InsertNullTable() = 0;
  // 获取机器人
  virtual IAndroidUserItem* GetFreeAndroidUserItem() = 0;
};
...
protected:
  ...
  // 加入比赛桌子
  VOID AddMatchTable(tagTableFrameInfo* pTableFrameInfo) { m_MatchTableArray.emplace_back(pTableFrameInfo); }
