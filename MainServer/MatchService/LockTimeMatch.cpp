...
#include "../GameServer/DataBasePacket.h"

////////////////////////////////////////////////////////////////////////////////////////////////////////////

// 比赛状态
enum emMatchStatus { MatchStatus_Free = 0, MatchStatus_WaitPlay, MatchStatus_Playing, MatchStatus_WaitEnd, MatchStatus_End };
...
CLockTimeMatch::CLockTimeMatch() {
  ...
  ZeroMemory(&m_TimeLastMatch, sizeof(SYSTEMTIME));
}
  ...
  switch (dwTimerID) {
    ...
    case IDI_SWITCH_STATUS: // 切换状态
    {
      ...
      if (m_MatchStatus == MatchStatus_End) {
        ...
        if (m_TimeLastMatch.wDay != CurTime.wDay) {
          if (m_pLockTimeMatch->cbMatchMode == MATCH_MODE_ROUND) {
            ...
            SYSTEMTIME CurTime;
            GetLocalTime(&CurTime);
            SYSTEMTIME MatchStartTime = m_pLockTimeMatch->MatchStartTime;

            // 计算时间
            int nCurrSeconds = CurTime.wHour * 3600 + CurTime.wMinute * 60 + CurTime.wSecond;

            // 比赛开始
            if (nCurrSeconds >= (MatchStartTime.wHour * 3600 + MatchStartTime.wMinute * 60 + MatchStartTime.wSecond)) {
              // 设置状态
              m_MatchStatus = MatchStatus_Playing;
      ...
      if (IsMatchSignupEnd() == true) {
        ...
        if (m_MatchStatus == MatchStatus_WaitPlay) {
          ...
          if (m_pLockTimeMatch->cbMatchMode == MATCH_MODE_ROUND) {
            ...
            SYSTEMTIME CurTime;
            GetLocalTime(&CurTime);
            SYSTEMTIME MatchEndTime = m_pLockTimeMatch->MatchEndTime;
            bool SameDay = MatchEndTime.wYear == CurTime.wYear && MatchEndTime.wMonth == CurTime.wMonth && MatchEndTime.wDay == CurTime.wDay;
    ...
    case IDI_CHECK_END_MATCH: // 结束比赛
    {
      ...
      if (m_MatchStatus == MatchStatus_WaitEnd) {
        if (cbMatchStatus != MatchStatus_WaitEnd) {
          ...
          // 比赛结束
          bool bMatchOver = CompareSystemTime(&CurTime, &MatchEndTime) >= 0;
          ...
          do {
            ...
            if (pIServerUserItem->GetUserStatus() == US_PLAYING) {
              if (bMatchOver == false) {
                szMessage = std::format(TEXT("今天的比赛结束时间已到，请尽快在 {} 分钟内结束本局游戏。"), m_pLockTimeMatch->wEndDelayTime / 60);
                m_pIGameServiceFrame->SendGameMessage(pIServerUserItem, szMessage.c_str(), SMT_CHAT | SMT_GLOBAL);
              } else {
                szMessage = std::format(TEXT("本场比赛结束时间已到，请尽快在 {} 分钟内结束当前比赛。否则系统将会自动解散本回合并不计入成绩。"),
                                        m_pLockTimeMatch->wEndDelayTime / 60);
                m_pIGameServiceFrame->SendGameMessage(pIServerUserItem, szMessage.c_str(), SMT_CHAT | SMT_GLOBAL);
              }
            } else {
              if (bMatchOver == false) {
                szMessage = std::format(TEXT("今天的比赛到此结束，请您明天再来参加！"));
                m_pIGameServiceFrame->SendGameMessage(pIServerUserItem, szMessage.c_str(), SMT_CHAT | SMT_EJECT | SMT_GLOBAL | SMT_CLOSE_GAME);
              } else {
                // 发送结束消息
                szMessage = std::format(TEXT("本场比赛结束时间已到，感谢您的参与；请在 {} 分钟后前往相关页面查询您的本场比赛成绩。"),
                                        m_pLockTimeMatch->wEndDelayTime / 60);
                m_pIGameServiceFrame->SendGameMessage(pIServerUserItem, szMessage.c_str(), SMT_CHAT | SMT_EJECT | SMT_GLOBAL | SMT_CLOSE_GAME);
              }
            }
      ...
      if (m_MatchStatus == MatchStatus_End) {
        ...
        time_t MatchEndTime = GetUnixTime(&m_pLockTimeMatch->MatchEndTime);
        ...
        if (m_pLockTimeMatch->cbMatchMode == MATCH_MODE_ROUND || CurTime >= MatchEndTime) {
          ...
          if (m_pLockTimeMatch->cbMatchMode == MATCH_MODE_ROUND) {
            ...
            GetAsSystemTime(CurTime, &SystemTimeCurr);
...
bool CLockTimeMatch::OnEventDataBase(WORD wRequestID, IServerUserItem* pIServerUserItem, VOID* pData, WORD wDataSize, DWORD dwContextID) {
  switch (wRequestID) {
    ...
    case DBO_GR_MATCH_REVIVE_INFO: // 复活信息
    {
      ...
      // 淘汰判断
      if (!pMatchReviveInfo->bSuccessed ||
          (!pMatchReviveInfo->bSafeCardRevive && pMatchReviveInfo->cbReviveTimesed >= m_pLockTimeMatch->cbReviveTimes)) {
        bEliminate = true;
      }
    ...
    case DBO_GR_MATCH_RANK_LIST: // 比赛排行
    {
      ...
      while (true) {
        ...
        if (pIRewardUserItem->GetMatchData() == &bRewardUser) {
          ...
          // SnprintfT(szMessage,CountArray(szMessage),TEXT"点击查看比赛排行：http://service.foxuc.com/GameMatch.aspx?PlazaStationID=%d&TypeID=2&MatchID=%d"),
    ...
    case DBO_GR_MATCH_CANCEL_RESULT: // 取消结果
    {
      ...
      if (pMatchCancelResult->bSuccessed == true) {
        // 发送结束消息
        TCHAR szMessage[] = TEXT("因报名人数不足，取消本场比赛。已报名的玩家稍后将会收到系统退还的报名费用，请注意查收。");
  ...
  switch (wSubCmdID) {
    ...
    case SUB_GR_MATCH_BUY_SAFECARD: // 购买保险
    {
      ...
      if (pMatchBuySafeCard->bBuySafeCard) {
        ...
        // 投递数据
        m_pIDataBaseEngine->PostDataBaseRequest(MatchBuySafeCard.dwUserID, DBR_GR_MATCH_BUY_SAFECARD, dwSocketID, &MatchBuySafeCard,
                                                sizeof(MatchBuySafeCard));
      } else {
        // 消息通知
    ...
    case SUB_GR_MATCH_USER_REVIVE: // 玩家复活
    {
      ...
      // 投递数据
      m_pIDataBaseEngine->PostDataBaseRequest(MatchUserRevive.dwUserID, DBR_GR_MATCH_USER_REVIVE, dwSocketID, &MatchUserRevive,
                                              sizeof(MatchUserRevive));
...
bool CLockTimeMatch::OnEventUserItemScore(IServerUserItem* pIServerUserItem, BYTE cbReason) {
  ...
  // 发送数据
  m_pIGameServiceFrame->SendDataBatchToMobileUser(pIServerUserItem->GetTableID(), MDM_GR_USER, SUB_GR_USER_SCORE, &MobileUserScore,
                                                  sizeof(MobileUserScore));
...
bool CLockTimeMatch::OnEventUserLogon(IServerUserItem* pIServerUserItem) {
  ...
  // 比赛奖励
  MatchRule.wRewardCount = std::min((size_t) m_pMatchOption->wRewardCount, CountArray(MatchRule.MatchRewardInfo));
  ...
  if (pIServerUserItem->GetUserMatchStatus() != MUS_SIGNUPED) {
    ...
    if (m_pMatchOption->cbSignupMode & SIGNUP_MODE_MATCH_USER) {
      if (pIServerUserItem->IsAndroidUser() == false) {
        ...
        // 投递数据
        m_pIDataBaseEngine->PostDataBaseRequest(QueryMatchQualify.dwUserID, DBR_GR_MATCH_QUERY_QUALIFY, dwSocketID, &QueryMatchQualify,
                                                sizeof(QueryMatchQualify));
...
bool CLockTimeMatch::OnEventGameStart(ITableFrame* pITableFrame, WORD wChairCount) {
  ...
  for (INT i = 0; i < pITableFrame->GetChairCount(); i++) {
    ...
    // 构造提示消息
    auto text =
        std::format(TEXT("本局比赛是您的第 {} 局。"), pIServerUserItem->GetUserPlayCount() - pIServerUserItem->GetUserInfo()->dwDrawCount + 1);
    ...
    if (cbReason != REMOVE_BY_ENDMATCH) {
      ...
      if (m_pLockTimeMatch->lMatchCullScore > 0 && pIUserItem->GetUserScore() < m_pLockTimeMatch->lMatchCullScore) {
        ...
        if (m_pLockTimeMatch->cbReviveEnabled == TRUE) {
          ...
          // 投递数据
          m_pIDataBaseEngine->PostDataBaseRequest(MatchQueryRevive.dwUserID, DBR_GR_MATCH_QUERY_REVIVE, 0, &MatchQueryRevive,
                                                  sizeof(MatchQueryRevive));
        } else {
          // 淘汰提醒
      ...
      if (m_pMatchOption->cbRankingMode & RANKING_MODE_TOTAL_GRADES) {
        if (pIUserItem->GetUserPlayCount() - pIUserItem->GetUserInfo()->dwDrawCount == m_pLockTimeMatch->wMinPlayCount) {
          ...
          SnprintfT(szMessage, CountArray(szMessage), TEXT("您已完成%d局比赛，可以继续当前比赛或选择退出等待本次比赛活动结束后排名！"),
                    m_pLockTimeMatch->wMinPlayCount);
      ...
      if (m_pMatchOption->cbRankingMode & RANKING_MODE_SPECIAL_GRADES) {
        if (pIUserItem->GetUserPlayCount() - pIUserItem->GetUserInfo()->dwDrawCount == m_pMatchOption->wCountInnings) {
          TCHAR szMessage[128] = TEXT("");
          SnprintfT(szMessage, CountArray(szMessage),
                    TEXT("您已完成%d局比赛，系统将会记录您当前的成绩并重置比赛分，您可以继续当前比赛或选择退出等待本次比赛结束后查看排名！"),
                    m_pMatchOption->wCountInnings);
          m_pIGameServiceFrame->SendGameMessage(pIUserItem, szMessage, SMT_EJECT | SMT_CHAT | SMT_CLOSE_GAME);
          ...
          // 投递数据
          m_pIDataBaseEngine->PostDataBaseRequest(MatchRecordGrades.dwUserID, DBR_GR_MATCH_RECORD_GRADES, 0, &MatchRecordGrades,
                                                  sizeof(MatchRecordGrades));
    ...
    if (wTableID < m_pGameServiceOption->wTableCount) {
      ...
      for (WORD i = 0; i < m_pGameServiceAttrib->wChairCount; i++) {
        ...
        if (InsertDistribute(pITableUserItem) == true) {
          ...
          m_pIGameServiceFrame->SendGameMessage(pITableUserItem, TEXT("由于玩家离开，系统正在为您重新分配座位，请稍后..."),
                                                SMT_CHAT | SMT_TABLE_ROLL);
        }
      }
    }
  }
...
bool CLockTimeMatch::VerifyUserEnterMatch(IServerUserItem* pIServerUserItem) {
  ...
  // 淘汰判断
  if (m_pLockTimeMatch->lMatchCullScore > 0 && pIServerUserItem->GetUserScore() < m_pLockTimeMatch->lMatchCullScore) {
    SnprintfT(szMessage, CountArray(szMessage), TEXT("由于您的比赛币低于 %d，您将不能继续游戏！"), m_pLockTimeMatch->lMatchCullScore);
  ...
  // 最大局数
  if (m_pLockTimeMatch->wMaxPlayCount > 0 &&
      (pIServerUserItem->GetUserPlayCount() - pIServerUserItem->GetUserInfo()->dwDrawCount) == m_pLockTimeMatch->wMaxPlayCount) {
    SnprintfT(szMessage, CountArray(szMessage), TEXT("您已完成 %d 局比赛，您将不能继续游戏，请等待比赛结束来查看排名！"),
              m_pLockTimeMatch->wMaxPlayCount);
...
bool CLockTimeMatch::IsMatchSignupEnd() {
  ...
  SYSTEMTIME CurTime;
  GetLocalTime(&CurTime);
  SYSTEMTIME SignupEndTime = m_pLockTimeMatch->SignupEndTime;

  // 常规开赛
  if (m_pLockTimeMatch->cbMatchMode == MATCH_MODE_NORMAL) {
    // 比较时间
    return CompareSystemTime(&CurTime, &SignupEndTime) >= 0;
  } else {
    // 计算时间
    int nCurrSeconds = CurTime.wHour * 3600 + CurTime.wMinute * 60 + CurTime.wSecond;

    // 判断时间
    return nCurrSeconds >= (SignupEndTime.wHour * 3600 + SignupEndTime.wMinute * 60 + SignupEndTime.wSecond);
  }
}
...
bool CLockTimeMatch::IsMatchSignupStart() {
  ...
  SYSTEMTIME CurTime;
  GetLocalTime(&CurTime);
  SYSTEMTIME SignupStartTime = m_pLockTimeMatch->SignupStartTime;

  // 常规开赛
  if (m_pLockTimeMatch->cbMatchMode == MATCH_MODE_NORMAL) {
    // 比较时间
    return CompareSystemTime(&CurTime, &SignupStartTime) >= 0;
  } else {
    // 计算时间
    int nCurrSeconds = CurTime.wHour * 3600 + CurTime.wMinute * 60 + CurTime.wSecond;

    // 判断时间
    return nCurrSeconds >= (SignupStartTime.wHour * 3600 + SignupStartTime.wMinute * 60 + SignupStartTime.wSecond);
  }
}
...
DWORD CLockTimeMatch::GetMatchStartInterval() {
  ...
  // 比赛未开始
  if (CompareSystemTime(&CurTime, MatchStartTime) < 0) {
    dwTimeInterval = (DWORD) DiffSystemTime(MatchStartTime, &CurTime);
  }

  // 比赛已结束
  if (CompareSystemTime(&CurTime, MatchEndTime) >= 0) {
    dwTimeInterval = 0;
  }

  // 计算时间
  dwCurrStamp = CurTime.wHour * 3600 + CurTime.wMinute * 60 + CurTime.wSecond;
  dwStartStamp =
...
LONGLONG CLockTimeMatch::CalcMatchNo() {
  ...
  if (m_pLockTimeMatch->cbMatchMode == MATCH_MODE_ROUND) {
    ...
    // 变量定义
    LONGLONG diff_days = DiffSystemTime(&CurTime, &SystemStartTime) / 86400;
...
bool CLockTimeMatch::PerformDistribute() {
  ...
  while (true) {
    ...
    // 获取用户
    WORD wRandCount = std::max(m_pMatchOption->wMaxPartakeGameUser - m_pMatchOption->wMinPartakeGameUser + 1, 1);
...
VOID CLockTimeMatch::UpdateMatchStatus() {
  ...
  // 等待开始比赛
  if (MatchStatus == MatchStatus_Free &&
      CurTime.wHour * 3600 + CurTime.wMinute * 60 + CurTime.wSecond >=
          m_pLockTimeMatch->MatchStartTime.wHour * 3600 + m_pLockTimeMatch->MatchStartTime.wMinute * 60 + m_pLockTimeMatch->MatchStartTime.wSecond) {
  ...
  // 等待结束
  if (MatchStatus == MatchStatus_Playing &&
      CurTime.wHour * 3600 + CurTime.wMinute * 60 + CurTime.wSecond >=
  ...
  if (MatchStatus == MatchStatus_WaitEnd) {
    ...
    // 强制结束
    if ((DWORD) (CurTime.wHour * 3600 + CurTime.wMinute * 60 + CurTime.wSecond) >=
        (DWORD) (m_pLockTimeMatch->MatchEndTime.wHour * 3600 + m_pLockTimeMatch->MatchEndTime.wMinute * 60 + m_pLockTimeMatch->MatchEndTime.wSecond +
                 m_pLockTimeMatch->wEndDelayTime)) {
      MatchStatus = MatchStatus_End;
    }
  }
...
bool CLockTimeMatch::InsertDistribute(IServerUserItem* pIServerUserItem) {
  ...
  // 比赛状态 ？？？？？？？？？？？？？
  if (m_MatchStatus != MatchStatus_WaitPlay && m_MatchStatus != MatchStatus_Free && m_MatchStatus != MatchStatus_Playing) {
    return false;
  }
...
bool CLockTimeMatch::VerifyMatchTime(LPTSTR pszMessage, WORD wMaxCount, WORD& wMessageType) {
  ...
  // 比赛结束
  if (CompareSystemTime(&CurTime, &MatchEndTime) >= 0 && !bMatchRound) {
    wMessageType |= SMT_CLOSE_ROOM | SMT_CLOSE_GAME;
    SnprintfT(pszMessage, wMaxCount, TEXT("比赛已经结束,请您移驾至其他比赛房间参与比赛吧！"));
    return false;
  }

  // 比赛已结束
  if (nCurrSeconds >= MatchEndTime.wHour * 3600 + MatchEndTime.wMinute * 60 + MatchEndTime.wSecond && bMatchRound) {
    wMessageType |= SMT_CLOSE_ROOM | SMT_CLOSE_GAME;
    SnprintfT(pszMessage, wMaxCount, TEXT("今天的比赛已经结束,请您于明天%d时%d分%d秒前来参加比赛！"), MatchStartTime.wHour, MatchStartTime.wMinute,
              MatchStartTime.wSecond);
    return false;
  }

  // 报名未开始
  if (CompareSystemTime(&CurTime, &SignupStartTime) < 0 && !bMatchRound) {
    wMessageType |= SMT_CLOSE_ROOM;
    SnprintfT(pszMessage, wMaxCount, TEXT("比赛报名尚未开始,本场比赛报名时段为：%d-%d-%d %d:%d:%d至%d-%d-%d %d:%d:%d"), SignupStartTime.wYear,
              SignupStartTime.wMonth, SignupStartTime.wDay, SignupStartTime.wHour, SignupStartTime.wMinute, SignupStartTime.wSecond,
              SignupEndTime.wYear, SignupEndTime.wMonth, SignupEndTime.wDay, SignupEndTime.wHour, SignupEndTime.wMinute, SignupEndTime.wSecond);
    return false;
  }


  // 报名未开始
  if (bMatchRound && nCurrSeconds < (SignupStartTime.wHour * 3600 + SignupStartTime.wMinute * 60 + SignupStartTime.wSecond)) {
    wMessageType |= SMT_CLOSE_ROOM;
    SnprintfT(pszMessage, wMaxCount, TEXT("比赛报名尚未开始,请您于今天%d时%d分%d秒前来报名参加比赛！"), SignupStartTime.wHour,
              SignupStartTime.wMinute, SignupStartTime.wSecond);
    return false;
  }
...
bool CLockTimeMatch::SendReviveNotify(IServerUserItem* pIServerUserItem, BYTE cbReviveTimesed, bool bUseSafeCard) {
  if (bUseSafeCard == false) {
    ...
    // 构造消息
    auto text = std::format(TEXT("您的比赛币低于 {}，将不能继续游戏。 您只需支付 {} 金币便可复活继续比赛，您确定要复活吗？"),
                            m_pLockTimeMatch->lMatchCullScore, m_pLockTimeMatch->lReviveFee);

    // 发送数据
    WORD wDataSize = sizeof(MatchRevive) - sizeof(MatchRevive.szNotifyContent);
    wDataSize += lstrcpyn(MatchRevive.szNotifyContent, text.c_str(), CountArray(MatchRevive.szNotifyContent));
    m_pIGameServiceFrame->SendData(pIServerUserItem, MDM_GR_MATCH, SUB_GR_MATCH_REVIVE, &MatchRevive, wDataSize);
  } else {
    // 变量定义
    auto text =
        std::format(TEXT("您的比赛币低于 {}，将不能继续游戏。由于您购买了保险卡，系统将免费为您复活一次！"), m_pLockTimeMatch->lMatchCullScore);

    // 消息通知
    m_pIGameServiceFrame->SendRoomMessage(pIServerUserItem, text.c_str(), SMT_CHAT | SMT_EJECT);
  }
...
bool CLockTimeMatch::SendSafeCardNotify(IServerUserItem* pIServerUserItem) {
  ...
  // 构造消息
  auto text = std::format(TEXT("恭喜您报名成功！本场比赛出售保险卡, 被淘汰可免费复活一次，售价 {} 金币，您是否要购买？"), MatchSafeCard.lSafeCardFee);
...
bool CLockTimeMatch::SendDeductFeeNotify(IServerUserItem* pIServerUserItem) {
  ...
  if (m_pMatchOption->cbDeductArea == DEDUCT_AREA_SERVER && MatchFee.lSignupFee > 0) {
    ...
    auto text = std::format(TEXT("参赛将扣除报名费 {} {}，确认要参赛吗？"), m_pMatchOption->lSignupFee,
                            pszFeeType[m_pMatchOption->cbFeeType - FEE_TYPE_GOLD]);
    lstrcpyn(MatchFee.szNotifyContent, text.c_str(), CountArray(MatchFee.szNotifyContent));
  }
...
void CLockTimeMatch::SendTableUserMatchInfo(ITableFrame* pITableFrame, WORD wChairID) {
  ...
  // SnprintfT(MatchInfo.szTitle[1], CountArray(MatchInfo.szTitle[1]), TEXT("游戏基数：%d"), m_pLockTimeMatch->lMatchInitScore);
  // SnprintfT(MatchInfo.szTitle[2], CountArray(MatchInfo.szTitle[2]), TEXT("局制名称：定时开赛"));
  auto text = std::format(TEXT("积分方式：累计积分"));
  lstrcpyn(MatchInfo.szTitle[0], text.c_str(), CountArray(MatchInfo.szTitle[0]));
  text = std::format(TEXT("游戏基数：{}"), m_pLockTimeMatch->lMatchInitScore);
  lstrcpyn(MatchInfo.szTitle[1], text.c_str(), CountArray(MatchInfo.szTitle[1]));
  text = std::format(TEXT("局制名称：定时开赛"));
...
bool CLockTimeMatch::WriteUserAward(IServerUserItem* pIServerUserItem, tagMatchRankInfo* pMatchRankInfo) {
  ...
  if (pIServerUserItem != NULL) {
    ...
    // 比赛奖励
    GetLocalTime(&MatchResult.MatchEndTime);
