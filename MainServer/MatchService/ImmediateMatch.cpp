...
CImmediateMatch::~CImmediateMatch(void) {
  ...
  m_MatchGroup.insert(m_MatchGroup.end(), m_OverMatchGroup.begin(), m_OverMatchGroup.end());
  for (int i = 0; i < m_MatchGroup.size(); i++) {
    SafeDelete(m_MatchGroup[i]);
  }


  m_MatchGroup.clear();
  m_OverMatchGroup.clear();
}
...
bool CImmediateMatch::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter) {
  if (dwTimerID >= IDI_GROUP_TIMER_START) {
    // 计算当前定时器对应的比赛组 Loop 标识
    const DWORD dwTargetLoopTimer = (dwTimerID - IDI_GROUP_TIMER_START) / 10;
    const DWORD dwSubTimerID = dwTimerID - (dwTargetLoopTimer * 10);

    // 使用 C++20 过滤视图，筛选出所有匹配的组
    auto match_view = m_MatchGroup | std::views::filter([dwTargetLoopTimer](const auto& pGroup) {
                        return pGroup && pGroup->m_LoopTimer == dwTargetLoopTimer;
                      });

    // 批量分发
    for (auto& pGroup: match_view) {
      pGroup->OnTimeMessage(dwSubTimerID, dwBindParameter);
    }
    return true;
  }
  ...
  switch (dwTimerID) {
    case IDI_CHECK_OVER_MATCH: {
      ...
      for (INT_PTR i = m_MatchGroup.size() - 1; i >= 0; i--) {
        // 参赛用户小于椅子数量 就要结束
    ...
    case IDI_DELETE_OVER_MATCH: {
      for (auto pOverMatch: m_OverMatchGroup) {
...
bool CImmediateMatch::OnEventDataBase(WORD wRequestID, IServerUserItem* pIServerUserItem, VOID* pData, WORD wDataSize, DWORD dwContextID) {
  switch (wRequestID) {
    case DBO_GR_MATCH_SIGNUP_RESULT: // 报名结果
    {
      ...
      // 报名成功
      m_OnMatchUserItem.emplace_back(pIServerUserItem);
      ...
      if (pMatchSignupResult->cbSignupMode == SIGNUP_MODE_SIGNUP_FEE) {
        ...
        if (m_pMatchOption->cbDeductArea == DEDUCT_AREA_SERVER) {
          ...
          if (m_pMatchOption->lSignupFee > 0) {
            ...
            SnprintfT(szDescribe, CountArray(szDescribe), TEXT("退赛成功，退还报名费%lld%s！"), m_pMatchOption->lSignupFee,
                      pszFeeType[m_pMatchOption->cbFeeType - FEE_TYPE_GOLD]);
          } else {
            SnprintfT(szDescribe, CountArray(szDescribe), TEXT("退赛成功！"));
          }
    ...
    case DBO_GR_MATCH_RANK_LIST: // 比赛排行
    {
      ...
      // 发放奖励
      for (auto& pMatchRankInfo: pMatchRankList->MatchRankInfo) {
        // 查找玩家
      ...
      // 定位比赛组
      auto itr = std::ranges::find_if(m_MatchGroup, [pMatchRankList](auto& pGroup) {
        return pGroup && pGroup->m_lMatchNo == pMatchRankList->lMatchNo;
      });

      if (itr != m_MatchGroup.end()) {
        auto pImmediateGroup = *itr; // 暂存指针

        if (pImmediateGroup != nullptr) {
          // 状态通知与清理
          ...
          // 还原计时器编号
          m_LoopTimer.emplace_back(pImmediateGroup->m_LoopTimer);

          // 先将需要退赛的玩家指针拷贝到临时 vector，防止退赛回调间接修改 Map 导致迭代器崩溃
          std::vector<IServerUserItem*> users_to_quit;
          users_to_quit.reserve(pImmediateGroup->m_OnMatchUserMap.size());
          for (const auto& [dwUserID, pUserItem]: pImmediateGroup->m_OnMatchUserMap) {
            if (pUserItem) {
              users_to_quit.emplace_back(pUserItem);
            }
          }

          WORD wQuitUserCount = 0;
          const WORD wMatchUserCount = static_cast<WORD>(users_to_quit.size());

          // 安全遍历临时队列执行退赛与断线清理
          for (auto* pITempUserItem: users_to_quit) {
            pITempUserItem->SetUserMatchStatus(MUS_NULL);

            if (OnEventUserQuitMatch(pITempUserItem, UNSIGNUP_REASON_PLAYER)) {
              wQuitUserCount++;
            }

            // 处理代打/托管玩家踢出房间
            if (!pITempUserItem->IsAndroidUser() && pITempUserItem->IsTrusteeUser()) {
              ...
              if (wTableID != INVALID_TABLE) {
                ...
                // 优化后的快速桌子起立逻辑：使用 std::ranges::find_if 替代嵌套 for
                auto table_it = std::ranges::find_if(pImmediateGroup->m_MatchTableArray, [wTableID](const auto& pTable) {
                  return pTable && pTable->wTableID == wTableID;
                });
                if (table_it != pImmediateGroup->m_MatchTableArray.end() && *table_it) {
                  (*table_it)->pTableFrame->PerformStandUpAction(pITempUserItem);
                }
              }
          ...
          if (wQuitUserCount != wMatchUserCount) {
            CLogger::Error(TEXT("比赛结束后，退赛人数异常"));
          }

          // 批量回收桌子
          for (auto* pTempTable: pImmediateGroup->m_MatchTableArray) {
            SafeDelete(pTempTable); // 如果容器已是 unique_ptr，此循环完全可以不写！
          }
          pImmediateGroup->m_MatchTableArray.clear();
        }

        // 5. 【状态流转】：从进行中队列移除，推入结束队列
        // 💡 注意：这一步直接利用前面拿到的迭代器 itr 移除，性能提升到 O(1)
        m_MatchGroup.erase(itr);
        m_OverMatchGroup.emplace_back(pImmediateGroup);
        break;
      }
    }
  ...
  bool CImmediateMatch::OnEventUserJoinMatch(IServerUserItem * pIServerUserItem, BYTE cbReason, DWORD dwSocketID) {
    ...
    if (wTableCount < m_pImmediateMatch->wStartUserCount / CImmediateGroup::m_wChairCount) {
      // 构造消息
      auto text = std::format(TEXT("抱歉，当前桌子不够请等待。还差 {} 张桌子可以报名"),
                              m_pImmediateMatch->wStartUserCount / CImmediateGroup::m_wChairCount - wTableCount);
      SendRoomMessage(pIServerUserItem, text.c_str(), SMT_CHAT);
      return true;
    }
    ...
    // 重复报名
    INT_PTR dwUserCount = m_OnMatchUserItem.size();
    ...
    if (m_pMatchOption->lSignupFee == 0 || pIServerUserItem->IsAndroidUser() == true) {
      // 机器数判断
      if (pIServerUserItem->IsAndroidUser() == true && (m_pCurMatchGroup->m_wAndroidUserCount >= m_pImmediateMatch->wAndroidUserCount)) {
        return true;
      }
  ...
  bool CImmediateMatch::OnEventUserQuitMatch(IServerUserItem * pIServerUserItem, BYTE cbReason, WORD * pBestRank, DWORD dwContextID) {
    ...
    if (RemoveMatchUserItem(pIServerUserItem)) {
      // 从正在比赛的组中删除该玩家
      INT_PTR nGroupCount = m_MatchGroup.size();
  ...
  bool CImmediateMatch::OnEventMatchStart(CImmediateGroup * pMatch) {
    ASSERT(pMatch == m_pCurMatchGroup && m_LoopTimer.size() > 0);
    ...
    if (pMatch != NULL) {
      ...
      // 编号不够 不能报名的
      pMatch->m_LoopTimer = m_LoopTimer.front();
      m_LoopTimer.erase(m_LoopTimer.begin());
      m_MatchGroup.emplace_back(pMatch);
  ...
  bool CImmediateMatch::OnEventMatchOver(CImmediateGroup * pMatch) {
    ...
    if (pMatch != NULL) {
      ...
      GetLocalTime(&MatchOver.MatchEndTime);
  ...
  bool CImmediateMatch::SendMatchInfo(IServerUserItem * pIServerUserItem) {
    ...
    MatchNum.dwWaitting = (DWORD) m_pCurMatchGroup->m_OnMatchUserMap.size();
    ...
    // 发送报名状态
    DWORD dwUserCount = (DWORD) m_OnMatchUserItem.size();
    for (INT_PTR i = 0; i < dwUserCount; i++) {
  ...
  bool CImmediateMatch::SendGroupUserMessage(LPCTSTR pStrMessage, CImmediateGroup * pMatch) {
    for (auto [dwUserID, pUserItem]: pMatch->m_OnMatchUserMap) {
  ...
  void CImmediateMatch::InsertNullTable() {
    for (int i = 0; i < m_pGameServiceOption->wTableCount; i++) {
      // 超过就退出
      WORD nCurTableCount = (WORD) m_pCurMatchGroup->m_MatchTableArray.size();
      if (nCurTableCount >= m_pImmediateMatch->wStartUserCount / CImmediateGroup::m_wChairCount) {
        break;
      }

      BOOL bIsInsert = true;
      for (int j = 0; j < m_MatchGroup.size(); j++) {
        WORD wMatchGroupTableCount = (WORD) m_MatchGroup[j]->m_MatchTableArray.size();
  ...
  bool CImmediateMatch::WriteUserAward(IServerUserItem * pIServerUserItem, tagMatchRankInfo * pMatchRankInfo) {
    ...
    if (bWriteReward == true) {
      ...
      // 构造消息
      auto text = std::format(TEXT("比赛已结束，恭喜您获得第{}名，欢迎您参加其他场次的比赛！"), pMatchRankInfo->wRankID);

      // 发送消息
      SendGameMessage(pIServerUserItem, text.c_str(), SMT_CHAT | SMT_EJECT | SMT_CLOSE_GAME | SMT_MATCHRESULT);
  ...
  bool CImmediateMatch::RemoveMatchUserItem(IServerUserItem * pIServerUserItem) {
    for (INT_PTR i = 0; i < m_OnMatchUserItem.size(); i++) {
      // 获取用户
      ...
      if (pOnLineUserItem == pIServerUserItem) {
        ...
        m_OnMatchUserItem.erase(m_OnMatchUserItem.begin() + i);
        return true;
      }
    }
