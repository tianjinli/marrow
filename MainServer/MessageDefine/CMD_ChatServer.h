...
struct CMD_GC_MB_LogonByUserID {
  ...
  NCHAR szPhoneMode[LEN_PHONE_MODE]; // 手机型号
};
...
struct CMD_GC_UserGameStatusNotify {
  ...
  WORD wServerID; // 房间标识
  WORD wTableID; // 桌子标识
  NCHAR szServerName[LEN_SERVER]; // 房间名称
};
...
struct CMD_GC_UserChat {
  ...
  NCHAR szFontName[LEN_FONT_NAME]; // 字体名称
  NCHAR szMessageContent[LEN_MESSAGE_CONTENT]; // 消息内容
};
...
struct CMD_GC_ModifyGroup {
  DWORD dwUserID; // 用户标识
  BYTE cbGroupID; // 组别标识
  NCHAR szGroupName[GROUP_LEN_NAME]; // 组别名称
};
...
struct CMD_GC_UserShare {
  ...
  NCHAR szShareImageAddr[128]; // 图片地址
  NCHAR szMessageContent[128]; // 消息内容
};
...
struct CMD_GC_OperateFailure {
  ...
  NCHAR szDescribeString[128]; // 错误消息
};
