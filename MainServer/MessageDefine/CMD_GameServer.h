...
struct CMD_GR_LogonUserID {
  ...
  NCHAR szServerPasswd[LEN_PASSWORD]; // 房间密码
...
struct CMD_GR_UserSitDown {
  WORD wTableID; // 桌子位置
  WORD wChairID; // 椅子位置
  NCHAR szPassword[LEN_PASSWORD]; // 桌子密码
};
...
struct CMD_GR_UserGameData {
  DWORD dwUserID; // 用户标识
  NCHAR szUserGameData[LEN_GAME_DATA]; // 游戏数据
};

// 请求失败
struct CMD_GR_RequestFailure {
  LONG lErrorCode; // 错误代码
  NCHAR szDescribeString[256]; // 描述信息
};
...
struct CMD_GR_C_UserChat {
  ...
  NCHAR szChatString[LEN_USER_CHAT]; // 聊天信息
};
...
struct CMD_GR_S_TaskFinish {
  ...
  NCHAR szTaskName[LEN_TASK_NAME]; // 任务名称
};
...
struct CMD_GR_C_SendTrumpet {
  ...
  NCHAR szTrumpetContent[TRUMPET_MAX_CHAR]; // 喇叭内容
};
...
struct CMD_GR_SendWarning {
  WORD wChatLength; // 信息长度
  DWORD dwTargetUserID; // 目标用户
  NCHAR szWarningMessage[LEN_USER_CHAT]; // 警告消息
};
...
struct CMD_GR_SendMessage {
  ...
  WORD wChatLength; // 信息长度
  NCHAR szSystemMessage[LEN_USER_CHAT]; // 系统消息
};
...
struct CMD_GR_KickAllUser {
  NCHAR szKickMessage[LEN_USER_CHAT]; // 踢出提示
};
...
struct CMD_GR_Match_Wait_Tip {
  ...
  NCHAR szMatchName[LEN_SERVER]; // 比赛名称
};
...
struct CMD_GR_MatchResult {
  ...
  NCHAR szNickName[LEN_ACCOUNTS]; // 玩家昵称

  // 比赛信息
  WORD wMatchRank; // 比赛名次
  NCHAR szMatchName[64]; // 比赛名称
...
struct CMD_GR_MatchDesc {
  NCHAR szTitle[MAX_MATCH_DESC][16]; // 信息标题
  NCHAR szDescribe[MAX_MATCH_DESC][64]; // 描述内容
...
struct CMD_GR_CreateTable {
  ...
  NCHAR szPassword[LEN_PASSWORD]; // 密码设置
...
struct CMD_GR_DissumeTable {
  ...
  NCHAR szRoomID[ROOM_ID_LEN]; // 桌子 I D
