...
struct CMD_CS_C_RegisterPlaza {
  NCHAR szServerAddr[32]; // 服务地址
  NCHAR szServerName[LEN_SERVER]; // 服务器名
};
...
struct CMD_CS_C_SearchTable {
  ...
  NCHAR szPersonalTableID[7]; // 房间标识
};
...
struct CMD_CS_C_CreateTable {
  ...
  DWORD dwSocketID; // 网络标识
  DWORD dwClientAddr; // 连接地址
  NCHAR szClientAddr[16]; // 创建房间的IP地址
  // 桌子信息
...
struct CMD_CS_C_DismissTable {
  ...
  DWORD dwServerID; // 房间I D
  DWORD dwTableID; // 桌子I D
  CMD_CS_C_DismissTable() { dwSocketID = -1; }
};
