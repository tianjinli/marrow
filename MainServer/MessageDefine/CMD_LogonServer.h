...
struct CMD_GP_LogonGameID {
  ...
  NCHAR szMachineID[LEN_MACHINE_ID]; // 机器序列

  // 登录信息
  DWORD dwGameID; // 游戏 I D
  NCHAR szPassword[LEN_MD5]; // 登录密码
...
struct CMD_GP_LogonAccounts {
  ...
  BYTE cbValidateFlags; // 校验标识
  NCHAR szPassword[LEN_MD5]; // 登录密码
  NCHAR szAccounts[LEN_ACCOUNTS]; // 登录帐号
  NCHAR szPassPortID[LEN_PASS_PORT_ID]; // 身份证号
};
...
struct CMD_GP_RegisterAccounts {
  ...
  // 密码变量
  NCHAR szLogonPass[LEN_MD5]; // 登录密码

  // 注册信息
  WORD wFaceID; // 头像标识
  BYTE cbGender; // 用户性别
  NCHAR szAccounts[LEN_ACCOUNTS]; // 登录帐号
  NCHAR szNickName[LEN_NICKNAME]; // 用户昵称
  NCHAR szPassPortID[LEN_PASS_PORT_ID]; // 证件号码
  NCHAR szCompellation[LEN_COMPELLATION]; // 真实名字
  BYTE cbValidateFlags; // 校验标识
...
struct CMD_GP_LogonVisitor {
  ...
  // 连接信息
  NCHAR szMachineID[LEN_MACHINE_ID]; // 机器标识
};
...
struct CMD_GP_LogonSuccess {
  ...
  NCHAR szAccounts[LEN_ACCOUNTS]; // 登录帐号
  NCHAR szNickName[LEN_ACCOUNTS]; // 用户昵称
  NCHAR szDynamicPass[LEN_PASSWORD]; // 动态密码
  NCHAR szGroupName[LEN_GROUP_NAME]; // 社团名字
...
struct CMD_GP_LogonFailure {
  ...
  NCHAR szDescribeString[128]; // 描述消息
};
...
struct CMD_GP_QueryVerifyCode {
  ...
  NCHAR szMachineID[LEN_MACHINE_ID]; // 机器码 (C2S)
  NCHAR szVerifyCode[LEN_VERIFY_CODE]; // 验证码 (S2C)
};

// 验证结果
struct CMD_GP_VerifyCodeResult {
  BYTE cbResultCode; // 结果标识（1成功，0失败，2重复申请）
  NCHAR szDescString[64]; // 结果字符串
};
...
struct CMD_GP_ModifyLogonPass {
  DWORD dwUserID; // 用户 I D
  NCHAR szDesPassword[LEN_PASSWORD]; // 用户密码
  NCHAR szScrPassword[LEN_PASSWORD]; // 用户密码
};
...
struct CMD_GP_ModifyUnderWrite {
  DWORD dwUserID; // 用户 I D
  NCHAR szPassword[LEN_PASSWORD]; // 用户密码
  NCHAR szUnderWrite[LEN_UNDER_WRITE]; // 个性签名
};
...
struct CMD_GP_UserEnableInsure {
  ...
  NCHAR szLogonPass[LEN_PASSWORD]; // 登录密码
  NCHAR szInsurePass[LEN_PASSWORD]; // 银行密码
  NCHAR szMachineID[LEN_MACHINE_ID]; // 机器序列
};
...
struct CMD_GP_UserTransferScore {
  ...
  NCHAR szPassword[LEN_MD5]; // 银行密码
  NCHAR szAccounts[LEN_NICKNAME]; // 目标用户
  NCHAR szMachineID[LEN_MACHINE_ID]; // 机器序列
  NCHAR szTransRemark[LEN_TRANS_REMARK]; // 转账备注
};
...
struct CMD_GP_QueryUserInfoRequest {
  ...
  NCHAR szAccounts[LEN_ACCOUNTS]; // 目标用户
};
...
struct CMD_GP_QueryTransferRebate {
  DWORD dwUserID; // 用户I D
  NCHAR szPassword[LEN_PASSWORD]; // 登录密码
};
...
struct CMD_GP_CheckInResult {
  ...
  SCORE lScore; // 当前金币
  NCHAR szNotifyContent[128]; // 提示内容
};
...
struct CMD_GP_QueryUserGameData {
  WORD wKindID; // 游戏标识
  DWORD dwUserID; // 用户标识
  NCHAR szDynamicPass[LEN_MD5]; // 用户密码
};
...
struct CMD_GP_MatchSignupResult {
  ...
  WORD wServerID; // 房间标识
  SCORE lCurrScore; // 当前金币
  NCHAR szDescribeString[128]; // 描述信息
};
//////////////////////////////////////////////////////////////////////////////////
// 账户绑定
struct CMD_GP_AccountBind {
  ...
  NCHAR szBindNewAccounts[LEN_ACCOUNTS]; // 绑定帐号
  NCHAR szBindNewPassword[LEN_PASSWORD]; // 绑定密码
  NCHAR szBindNewSpreader[LEN_ACCOUNTS]; // 绑定推荐
};
...
struct CMD_GP_AccountBind_Exists {
  ...
  NCHAR szBindExistsAccounts[LEN_ACCOUNTS]; // 绑定帐号
  NCHAR szBindExistsPassword[LEN_PASSWORD]; // 绑定密码
};
...
struct CMD_GP_OperateSuccess {
  ...
  NCHAR szDescribeString[128]; // 成功消息
};
...
struct CMD_GP_S_PropertyPresent {
  ...
  WORD wPropCount; // 使用数目
  WORD wType; // 目标类型
  NCHAR szRecvNickName[16]; // 对谁使用
  int nHandleCode; // 返回码
  NCHAR szNotifyContent[64]; // 提示内容
};
...
struct CMD_MB_LogonGameID {
  ...
  // 连接信息
  NCHAR szMachineID[LEN_MACHINE_ID]; // 机器标识
  NCHAR szMobilePhone[LEN_MOBILE_PHONE]; // 电话号码
};
...
struct CMD_MB_LogonOtherPlatform {
  ...
  NCHAR szUserUin[LEN_USER_UIN]; // 用户Uin
  NCHAR szNickName[LEN_NICKNAME]; // 用户昵称
  NCHAR szCompellation[LEN_COMPELLATION]; // 真实名字
...
struct CMD_MB_LogonSuccess {
  ...
  DWORD dwExperience; // 经验数值
  SCORE lLoveLiness; // 用户魅力
  NCHAR szAccounts[LEN_ACCOUNTS]; // 用户帐号
  NCHAR szNickName[LEN_NICKNAME]; // 用户昵称
  NCHAR szDynamicPass[LEN_PASSWORD]; // 动态密码
...
struct CMD_MB_C_QueryVideoDetails {
  NCHAR szPersonalGUID[PERSONAL_ROOM_GUID]; // 约战房唯一ID
};
...
struct CMD_MB_C_QueryPlayBackCode {
  DWORD dwUserID; // 用户 I D
  NCHAR szVideoNumber[LEN_VIDEO_NUMBER]; // 视频唯一ID
};
...
struct CMD_MB_QueryGameServerResult {
  ...
  // 错误描述
  NCHAR szErrDescrybe[MAX_PATH]; // 错误描述
};

// 搜索别人房间
struct CMD_MB_SearchServerTable {
  NCHAR szServerID[7]; // 房间编号
  DWORD dwKindID; // 房间类型
};
