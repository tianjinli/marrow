...
bool CAndroidUserItem::KillGameTimer(UINT nTimerID) {
  ...
  if (nTimerID != 0) {
    // 寻找子项
    for (INT_PTR i = 0; i < m_TimerItemActive.size(); i++) {
      // 获取时间
      ...
      if (pTimerItem->nTimerID == nTimerID) {
        ...
        m_TimerItemBuffer.emplace_back(pTimerItem);

        return true;
      }
    }
  } else {
    m_TimerItemBuffer.insert(m_TimerItemBuffer.end(), m_TimerItemActive.begin(), m_TimerItemActive.end());
...
bool CAndroidUserItem::SetGameTimer(UINT nTimerID, UINT nElapse) {
  ...
  WORD wStorageCount = (WORD) m_TimerItemBuffer.size();

  // 库存子项
  if (wStorageCount > 0) {
    // 获取对象
    pTimerItem = m_TimerItemBuffer.back();

    // 加入数组
    m_TimerItemActive.emplace_back(pTimerItem);
    m_TimerItemBuffer.pop_back();
  }
...
bool CAndroidUserItem::OnTimerPulse(DWORD dwTimerID, WPARAM dwBindParameter) {
  // 寻找子项
  for (int i = m_TimerItemActive.size(); i-- > 0;) {
    // 变量定义
    tagTimerItem* pTimerItem = m_TimerItemActive[i];
    ...
    if (pTimerItem->nTimeLeave <= 1L) {
      ...
      m_TimerItemBuffer.emplace_back(pTimerItem);
        ...
        if (m_pIAndroidUserItemSink->OnEventTimer(pTimerItem->nTimerID) == false) {
          CLogger::Error("OnTimerPulse: TimerID={}", pTimerItem->nTimerID);
        }
      }
    } else {
      // 设置变量
...
bool CAndroidUserItem::OnSocketSubLogonFinish(VOID* pData, WORD wDataSize) {
  ...
  m_dwReposeTickCount = m_dwMinSitInterval + rand() % (std::max(m_dwMaxSitInterval - m_dwMinSitInterval, (DWORD) 1));
...
bool CAndroidUserItem::OnSocketSubUserStatus(VOID* pData, WORD wDataSize) {
  ...
  if (m_CurrentUserStatus.cbUserStatus == US_SIT) {
    if (m_AndroidService.dwSwitchTableInnings > 0 && m_AndroidService.dwSwitchTableInnings <= m_dwPlayInnings) {
      // 设置动作
      m_dwPlayInnings = 0;
...
VOID CAndroidUserItem::RepositUserItem() {
  ...
  // 索引变量
  m_wRoundID = std::max(1, (int) m_wRoundID + 1);
