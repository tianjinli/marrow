...
// 类型定义
using CUserTaskEntryArray = std::vector<tagUserTaskEntry*>; // 用户任务入口
using CTaskParameterArray = std::vector<tagTaskParameter*>; // 任务参数

// 类型定义
using CTaskParameterMap = std::unordered_map<WORD, tagTaskParameter*>; // 任务参数映射
using CUserTaskEntryMap = std::unordered_map<DWORD, tagUserTaskEntry*>; // 用户任务入口映射
