...
CServerUserItem::CServerUserItem() {
  ...
  // 设置质数
  m_UserGameDataMap.reserve(17);
}

// 析构函数
CServerUserItem::~CServerUserItem() {
  // 清空数据
  for (auto& pGameDataItem: m_UserGameDataMap | std::views::values) {
...
VOID CServerUserItem::GetUserGameData(INT nKey, LPTSTR pszValue, INT nMaxCount) {
  ...
  if (const auto itr = m_UserGameDataMap.find(nKey); itr != m_UserGameDataMap.end()) {
    auto pGameDataItem = itr->second;
...
VOID CServerUserItem::ResetUserItem() {
  ...
  // 清空数据
  for (auto [wKey, pGameDataItem]: m_UserGameDataMap) {
    SafeDelete(pGameDataItem);
  }
  m_UserGameDataMap.clear();
}
...
VOID CServerUserItem::InitUserGameDataMap() {
  ...
  std::vector<StringT> SubItemArray = StringUtils::Split(StringT(m_szUserGameData), g_szGameDataSpe);
  ...
  for (INT i = nSubItemCount - 1; i >= 0; i -= 2) {
    SnprintfT(szSubItem, CountArray(szSubItem), TEXT("%s"), SubItemArray.at(i - 1).c_str());

    bool bPureDigit = IsPureDigitString(szSubItem);
    if (bPureDigit) {
      INT nKey = StrToInt(szSubItem);
      auto pszValue = SubItemArray.at(i).c_str();
...
bool CServerUserItem::IsValidGameData(const StringT& strSource, const StringT& strSpliter) {
  ...
  std::vector<StringT> SubItemArray = StringUtils::Split(strSource, strSpliter);
  ...
  for (INT i = 0; i < nSubItemCount; i += 2) {
    ...
    bool bPureDigit = IsPureDigitString(szSubItem);
    bool bChString = SubItemArray.at(i + 1).length() > 0;
...
CServerUserManager::CServerUserManager() {
  ...
  // 设置质数
  m_UserIDMap.reserve(PRIME_SERVER_USER);
...
bool CServerUserManager::DeleteUserItem(IServerUserItem* pIServerUserItem) {
  ...
  // 寻找对象
  if (auto itr = std::ranges::find_if(m_UserItemArray,
                                      [pIServerUserItem](CServerUserItem* pTempUserItem) {
                                        return pTempUserItem == pIServerUserItem;
                                      });
      itr == m_UserItemArray.end()) {
    // 重置对象
    pTempUserItem->ResetUserItem();
    m_UserItemStore.emplace_back(pTempUserItem);

    // 删除对象
    m_UserItemArray.erase(itr);
...
bool CServerUserManager::InsertUserItem(IServerUserItem** pIServerUserResult, tagUserInfo& UserInfo, tagUserInfoPlus& UserInfoPlus) {
  ...
  if (m_UserItemStore.size() > 0) {
    ...
    if ((pServerUserItem = new (std::nothrow) CServerUserItem()) == NULL) {
      return false;
    }
  }
