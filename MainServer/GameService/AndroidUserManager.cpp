...
static constexpr std::array<int, 24> kHourIndexMap = {
    10, // 00
    0, // 01
    0, // 02
    1, // 03
...
CAndroidUserManager::CAndroidUserManager() {
  ...
  // 设置索引
  m_AndroidUserItemMap.reserve(PRIME_ANDROID_USER);
  m_AndroidUserItemMap.clear();
}

// 析构函数
CAndroidUserManager::~CAndroidUserManager() {
  // 清理断言
  ASSERT(m_AndroidUserItemMap.size() == 0L);
  ASSERT(m_AndroidParameterArray.size() == 0L);
  ASSERT(m_AndroidUserItemArray.size() == 0L);
  ASSERT(m_AndroidUserItemBuffer.size() == 0L);
}
...
bool CAndroidUserManager::ConcludeService() {
  // 删除存储
  for (INT_PTR i = 0; i < m_AndroidUserItemArray.size(); i++) {
    SafeRelease(m_AndroidUserItemArray[i]);
  }

  // 删除存储
  for (INT_PTR i = 0; i < m_AndroidParameterArray.size(); i++) {
    SafeDelete(m_AndroidParameterArray[i]);
  }

  // 清理数组
  m_AndroidUserItemMap.clear();
  m_AndroidParameterArray.clear();
  m_AndroidUserItemArray.clear();
  m_AndroidUserItemBuffer.clear();
...
bool CAndroidUserManager::AddAndroidParameter(tagAndroidParameter AndroidParameter[], WORD wParameterCount) {
  ...
  for (WORD wIndex = 0; wIndex < wParameterCount; wIndex++) {
    // 数目限制
    if (m_AndroidParameterArray.size() >= MAX_BATCH) {
      break;
    }
    ...
    if (pAndroidParameter == NULL) {
      ...
      // 查找空缺
      for (INT_PTR nIndex = 0; nIndex < m_AndroidParameterArray.size(); nIndex++) {
      ...
      if (bVacancyExist == false) {
        m_AndroidParameterArray.emplace_back(pAndroidParameter);
      }
    }
...
bool CAndroidUserManager::InsertAndroidInfo(tagAndroidAccountsInfo AndroidAccountsInfo[], WORD wAndroidCount, DWORD dwBatchID) {
  ...
  for (WORD wIndex = 0; wIndex < wAndroidCount; wIndex++) {
    ...
    // 从映射中查找
    if (m_AndroidUserItemMap.find(AndroidAccountsInfo[wIndex].dwUserID) != m_AndroidUserItemMap.end()) {
      continue;
    }
...
bool CAndroidUserManager::DeleteAndroidUserItem(DWORD dwAndroidID, bool bStockRetrieve) {
  ...
  // 关闭事件
  m_pITCPNetworkEngineEvent->OnEventTCPNetworkShut(0L, dwAndroidID, 0);
...
IAndroidUserItem* CAndroidUserManager::SearchAndroidUserItem(DWORD dwUserID, DWORD dwContextID) {
  // 查找机器
  CAndroidUserItem* pAndroidUserItem = NULL;
  if (auto itr = m_AndroidUserItemMap.find(dwUserID); itr != m_AndroidUserItemMap.end()) {
    pAndroidUserItem = itr->second;
  }
...
IAndroidUserItem* CAndroidUserManager::CreateAndroidUserItem(tagAndroidItemConfig& AndroidItemConfig) {
  ...
  // 连接模拟
  if (m_pITCPNetworkEngineEvent->OnEventTCPNetworkBind(0L, MAKELONG(wAndroidIndex, wRoundID)) == false) {
    // 错误断言
    ASSERT(FALSE);
...
bool CAndroidUserManager::OnEventTimerPulse(DWORD dwTimerID, WPARAM dwBindParameter) {
  ...
  if (dwTimerID >= IDI_ANDROID_OUT && dwTimerID < IDI_ANDROID_OUT + MAX_BATCH) {
    // 变量定义
    DWORD dwUserID = 0L;
    INT_PTR nActiveCount = m_AndroidUserItemMap.size();
    ...
    // 退出处理
    for (auto [dwUserID, pAndroidUserItem]: m_AndroidUserItemMap) {
    ...
    if (bLogoutFinished == false) {
      ...
      if (pAndroidParameter->bIsValided == false) {
        ...
        if (wParameterIndex == GetAndroidParameterIndex(pAndroidParameter)) {
          ...
          CLogger::Info(TEXT("本批次机器人全部退出,该批次将被删除, 批次ID={},{}"), pAndroidParameter->AndroidParameter.dwBatchID, nActiveCount);
        ...
        CLogger::Info(TEXT("本批次机器人全部退出,, 批次ID={},{}"), pAndroidParameter->AndroidParameter.dwBatchID, nActiveCount);
  ...
  if (dwTimerID == IDI_ANDROID_PULSE) {
    ...
    for (auto [dwUserID, pAndroidUserItem]: m_AndroidUserItemMap) {
      ...
      if (pAndroidUserItem->m_pIServerUserItem != NULL) {
        if (pAndroidUserItem->OnTimerPulse(dwTimerID, dwBindParameter) == false) {
          ASSERT(false && TEXT("机器人脉冲事件处理失败"));
...
bool CAndroidUserManager::GetAndroidLoadInfo(DWORD& dwBatchID, DWORD& dwLoadCount) {
  // 数目判断
  if (m_AndroidParameterArray.size() == 0) {
    return false;
  }
  ...
  for (INT_PTR nIndex = 0; nIndex < m_AndroidParameterArray.size(); nIndex++) {
    ...
    if (dwRemaindTime > ANDROID_UNLOAD_TIME) {
      ...
      dwLoadCount =
          std::min(pAndroidParameter->AndroidParameter.dwAndroidCount, dwRemaindTime / pAndroidParameter->AndroidParameter.dwEnterMaxInterval);
      if (dwLoadCount == 0) {
        continue;
      }
...
bool CAndroidUserManager::SendDataToClient(WORD wMainCmdID, WORD wSubCmdID, VOID* pData, WORD wDataSize) {
  ...
  for (INT_PTR i = 0; i < m_AndroidUserItemArray.size(); i++) {
    ...
    if (pAndroidUserItem->OnSocketRead(wMainCmdID, wSubCmdID, pData, wDataSize) == false) {
      // 错误断言
      ASSERT(false && TEXT("机器人发送数据到客户端失败"));
...
bool CAndroidUserManager::SendDataToServer(DWORD dwAndroidID, WORD wMainCmdID, WORD wSubCmdID, VOID* pData, WORD wDataSize) {
  ...
  // 消息处理
    ...
    CLogger::Error(TEXT("SendDataToServer: MainID={},SubID={}"), Command.wMainCmdID, Command.wSubCmdID);
    ASSERT(false && TEXT("机器人发送数据到服务器失败"));
...
CAndroidUserItem* CAndroidUserManager::GetAndroidUserItem(WORD wIndex) {
  ...
  // 效验索引
  ASSERT((wIndex - INDEX_ANDROID) < m_AndroidUserItemArray.size());
  if (((wIndex - INDEX_ANDROID) >= m_AndroidUserItemArray.size())) {
    return NULL;
  }
...
VOID CAndroidUserManager::FreeAndroidUserItem(DWORD dwUserID) {
  ...
  // 设置索引
  m_AndroidUserItemMap.erase(dwUserID);
  m_AndroidUserItemBuffer.emplace_back(pAndroidUserItem);
...
CAndroidUserItem* CAndroidUserManager::ActiveAndroidUserItem(tagAndroidItemConfig& AndroidItemConfig) {
  // 变量定义
  CAndroidUserItem* pAndroidUserItem = NULL;
  INT_PTR nFreeItemCount = m_AndroidUserItemBuffer.size();
  ...
  if (nFreeItemCount > 0) {
    ...
    // 删除数组
    m_AndroidUserItemBuffer.erase(m_AndroidUserItemBuffer.begin() + nItemPostion);
  }

  // 创建对象
  if (pAndroidUserItem == NULL) {
    // 数目判断
    if (m_AndroidUserItemArray.size() >= MAX_ANDROID) {
      ASSERT(FALSE);
      return NULL;
    }
      ...
      // 创建对象
    pAndroidUserItem = new (std::nothrow) CAndroidUserItem;

    // 错误判断
    if (pAndroidUserItem == NULL) {
      // 错误断言
      ASSERT(false && TEXT("创建机器人对象失败"));
      ...
      // 设置数组
    m_AndroidUserItemArray.emplace_back(pAndroidUserItem);
    WORD wCurrentIndex = (WORD) m_AndroidUserItemArray.size() - 1;
