...
bool CAttemperEngineSink::OnEventTCPSocketSubUserStatus(VOID* pData, WORD wDataSize) {
  ...
  // 重包判断
  if ((pUserGameStatus->cbGameStatus == pLocalUserItem->GetGameStatus()) && (pUserGameStatus->wServerID == pLocalUserItem->GetServerID()) &&
...
// 应答事件
bool CAttemperEngineSink::OnEventTCPNetworkBind(DWORD dwSocketID, DWORD dwClientAddr) {
  // 变量定义
...
// 关闭事件
bool CAttemperEngineSink::OnEventTCPNetworkShut(DWORD dwSocketID, DWORD dwClientAddr, DWORD dwActiveTime) {
  // 变量定义
  WORD wBindIndex = LOWORD(dwSocketID);
  tagBindParameter* pBindParameter = GetBindParameter(wBindIndex);
  if (pBindParameter == NULL) {
  ...
  try {
    ...
    if (pServerUserItem != NULL) {
      ...
      // SnprintfT(szMessage,CountArray(szMessage),TEXT("%d-----下线通知"),pServerUserItem->GetUserID());
    ...
    SnprintfT(szMessage, CountArray(szMessage), TEXT("关闭连接异常: dwSocketID=%d"), dwSocketID);
    CTraceService::TraceString(szMessage, TraceLevel_Normal);
  }
  // 清除信息
    ...
    if (pServerUserItem->GetUserItemKind() == enLocalKind) {
      ...
      if (pLocalUserItem->GetMainStatus() != FRIEND_US_OFFLINE && (pServerUserItem->GetClientAddr() != pBindParameter->dwClientAddr)) {
        if (pLocalUserItem->GetSocketID() != dwSocketID) {
          // TCHAR szMessage[256]=TEXT("");
          // SnprintfT(szMessage,CountArray(szMessage),TEXT("dwSocketID0=%d,Address0=%d,dwSocketID1=%d,Address1=%d"),
          ...
          SnprintfT(UserLogonAfresh.wNotifyMessage, CountArray(UserLogonAfresh.wNotifyMessage), TEXT("您的账号在别处登录,您被迫下线！"));
...
bool CAttemperEngineSink::OnTCPNetworkSubApplyForFriend(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pUserFriendGroup != NULL) {
    ...
    if (pServerFriendInfo != NULL && pServerFriendInfo->cbGroupID != 0) {
      ...
      lstrcpyn(ApplyForFriend.szDescribeString, TEXT("已经是好友，无需申请"), CountArray(ApplyForFriend.szDescribeString));
  ...
  if (pFriendUserItem != NULL) {
    // TCHAR szMessage[256]=TEXT("");
    // SnprintfT(szMessage,CountArray(szMessage),TEXT(".........%d申请加%d为好友,%d当前状态为%d"),pApplyForFriend->dwUserID,pApplyForFriend->dwFriendID,pApplyForFriend->dwFriendID,pFriendUserItem->GetMainStatus());
  ...
  lstrcpyn(ApplyForFriend.szDescribeString, TEXT(""), CountArray(ApplyForFriend.szDescribeString));
  WORD wDataSize2 = CountStringBuffer(ApplyForFriend.szDescribeString);
  WORD wHeadSize = sizeof(ApplyForFriend) - sizeof(ApplyForFriend.szDescribeString);
  SendData(dwSocketID, MDM_GC_USER, SUB_GC_APPLYFOR_FRIEND_ECHO, &ApplyForFriend, wHeadSize + wDataSize2);
...
bool CAttemperEngineSink::OnTCPNetworkSubRespondFriend(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pServerUserItem == NULL) {
    ...
    lstrcpyn(RespondFriendEcho.szDescribeString, TEXT("未找到此用户"), CountArray(RespondFriendEcho.szDescribeString));
  ...
  if (pUserFriendGroup != NULL) {
    if (pUserFriendGroup->SearchFriendItem(pRespondFriend->dwUserID) != NULL) {
      ...
      ZeroMemory(&RespondFriendEcho, sizeof(RespondFriendEcho));
      RespondFriendEcho.lErrorCode = CHAT_MSG_ERR;
      lstrcpyn(RespondFriendEcho.szDescribeString, TEXT("已经是好友"), CountArray(RespondFriendEcho.szDescribeString));
      WORD wDataSize2 = CountStringBuffer(RespondFriendEcho.szDescribeString);
      WORD wHeadSize = sizeof(RespondFriendEcho) - sizeof(RespondFriendEcho.szDescribeString);
      SendData(dwSocketID, MDM_GC_USER, SUB_GC_RESPOND_FRIEND_ECHO, &RespondFriendEcho, wHeadSize + wDataSize2);
      return true;
  ...
  if (pRespondFriend->bAccepted == true) {
    ...
    SnprintfT(RespondNotify.szNotifyContent, CountArray(RespondNotify.szNotifyContent), TEXT("%s 拒绝了您的好友申请！"),
              ((CLocalUserItem*) pServerUserItem)->GetNickName());
  ...
  lstrcpyn(RespondFriendEcho.szDescribeString, TEXT(""), CountArray(RespondFriendEcho.szDescribeString));
  WORD wDataSize2 = CountStringBuffer(RespondFriendEcho.szDescribeString);
  WORD wHeadSize = sizeof(RespondFriendEcho) - sizeof(RespondFriendEcho.szDescribeString);
  SendData(dwSocketID, MDM_GC_USER, SUB_GC_RESPOND_FRIEND_ECHO, &RespondFriendEcho, wHeadSize + wDataSize2);
...
bool CAttemperEngineSink::OnTCPNetworkSubRoomInvite(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pServerUserItem == NULL) {
    ...
    lstrcpyn(InviteGameEcho.szDescribeString, TEXT("信息超时"), CountArray(InviteGameEcho.szDescribeString));
  ...
  if (pInvitedUserItem == NULL) {
    ...
    ZeroMemory(&InviteGameEcho, sizeof(InviteGameEcho));
    InviteGameEcho.lErrorCode = CHAT_MSG_ERR;
    lstrcpyn(InviteGameEcho.szDescribeString, TEXT("未找到此用户"), CountArray(InviteGameEcho.szDescribeString));
    WORD wDataSize = CountStringBuffer(InviteGameEcho.szDescribeString);
    WORD wHeadSize = sizeof(InviteGameEcho) - sizeof(InviteGameEcho.szDescribeString);
    SendData(dwSocketID, MDM_GC_USER, SUB_GC_INVITE_GAME_ECHO, &InviteGameEcho, wHeadSize + wDataSize);
  ...
  if (pInvitedUserItem->GetMainStatus() == FRIEND_US_OFFLINE) {
    ...
    ZeroMemory(&InviteGameEcho, sizeof(InviteGameEcho));
    InviteGameEcho.lErrorCode = CHAT_MSG_ERR;
    lstrcpyn(InviteGameEcho.szDescribeString, TEXT("用户已经离线"), CountArray(InviteGameEcho.szDescribeString));
    WORD wDataSize = CountStringBuffer(InviteGameEcho.szDescribeString);
    WORD wHeadSize = sizeof(InviteGameEcho) - sizeof(InviteGameEcho.szDescribeString);
    SendData(dwSocketID, MDM_GC_USER, SUB_GC_INVITE_GAME_ECHO, &InviteGameEcho, wHeadSize + wDataSize);
    return true;
  ...
  lstrcpyn(InviteGameEcho.szDescribeString, TEXT(""), CountArray(InviteGameEcho.szDescribeString));
...
bool CAttemperEngineSink::OnTCPNetworkSubUserChat(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pServerUserItem == NULL) {
    ...
    lstrcpyn(UserChatEcho.szDescribeString, TEXT("信息超时"), CountArray(UserChatEcho.szDescribeString));
  ...
  if (pTargetUserItem != NULL) {
    ...
    lstrcpyn(UserChatEcho.szDescribeString, TEXT(""), CountArray(UserChatEcho.szDescribeString));
    WORD wDataSize = CountStringBuffer(UserChatEcho.szDescribeString);
    WORD wHeadSize = sizeof(UserChatEcho) - sizeof(UserChatEcho.szDescribeString);
    SendData(dwSocketID, MDM_GC_USER, SUB_GC_USER_CHAT_ECHO, &UserChatEcho, wHeadSize + wDataSize);
    ...
    ZeroMemory(&UserChatEcho, sizeof(UserChatEcho));
    UserChatEcho.lErrorCode = CHAT_MSG_ERR;
    lstrcpyn(UserChatEcho.szDescribeString, TEXT("发送失败，对方已经离线"), CountArray(UserChatEcho.szDescribeString));
    WORD wDataSize = CountStringBuffer(UserChatEcho.szDescribeString);
    WORD wHeadSize = sizeof(UserChatEcho) - sizeof(UserChatEcho.szDescribeString);
    SendData(dwSocketID, MDM_GC_USER, SUB_GC_USER_CHAT_ECHO, &UserChatEcho, wHeadSize + wDataSize);
...
bool CAttemperEngineSink::OnTCPNetworkSubTrumpet(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pServerUserItem == NULL) {
    ...
    lstrcpyn(TrumpetEcho.szDescribeString, TEXT("信息超时"), CountArray(TrumpetEcho.szDescribeString));
...
bool CAttemperEngineSink::OnTCPNetworkDeleteFriend(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pServerUserItem == NULL) {
    ...
    lstrcpyn(DeleteEcho.szDescribeString, TEXT("信息超时"), CountArray(DeleteEcho.szDescribeString));
...
bool CAttemperEngineSink::SaveOfflineMessage(DWORD dwUserID, WORD wMessageType, VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  for (INT i = 0; i < CountArray(cbOffLineData); i++) {
    SnprintfT(&szOffLineData[i * 2], 3, TEXT("%02X"), cbOffLineData[i]);
  }
...
bool CAttemperEngineSink::OnEventDataBaseResult(WORD wRequestID, DWORD dwContextID, VOID* pData, WORD wDataSize) {
  try {
    ...
    TCHAR szMessage[256] = TEXT("");
    SnprintfT(szMessage, CountArray(szMessage), TEXT("OnEventDataBaseResult-wRequestID=%d"), wRequestID);
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  // TCHAR szMessage[256]=TEXT("");
  // SnprintfT(szMessage,CountArray(szMessage),TEXT("%d-----上线通知"),UserOnLine.dwUserID);
  // CTraceService::TraceString(szMessage,TraceLevel_Normal);
...
bool CAttemperEngineSink::OnDBLoadUserFriend(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  while (wFriendCount > CountArray(IPCUserFriendInfo.FriendInfo)) {
    ...
    CopyMemory(IPCUserFriendInfo.FriendInfo, &pUserFriendInfo->FriendInfo[pUserFriendInfo->wFriendCount - wFriendCount],
               sizeof(tagClientFriendInfo) * IPCUserFriendInfo.wFriendCount);
  ...
  if (wFriendCount > 0) {
    ...
    CopyMemory(IPCUserFriendInfo.FriendInfo, &pUserFriendInfo->FriendInfo[pUserFriendInfo->wFriendCount - wFriendCount],
               sizeof(tagClientFriendInfo) * wFriendCount);
  ...
  if (pServerUserItem != NULL) {
    ...
    if (pUserFriendGroup != NULL) {
      ...
      if (pFriendUserItem != NULL && pFriendUserItem->GetMainStatus() == FRIEND_US_ONLINE) {
        ...
        // 发送数据
        SendData(((CLocalUserItem*) pFriendUserItem)->GetSocketID(), MDM_GC_USER, SUB_GC_DELETE_FRIEND_NOTIFY, &DeleteFriendNotify,
...
bool CAttemperEngineSink::OnDBSearchUserResult(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  if (cbUserCount > 0) {
    ...
    lstrcpyn(SearchUserEcho.szDescribeString, TEXT("查询不到用户"), CountArray(SearchUserEcho.szDescribeString));
...
bool CAttemperEngineSink::OnDBUserAddFriend(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  if (pServerUserItem != NULL) {
    if (pAddFriend->bLoadUserInfo == false && pRequestUserItem && pRequestUserItem->GetUserItemKind() == enLocalKind) {
      ...
      lstrcpyn(UserFriendInfo.FriendInfo[0].szUnderWrite, pRequestLocalInfo->GetUserInfo()->szUnderWrite,
               CountArray(UserFriendInfo.FriendInfo[0].szUnderWrite));
      lstrcpyn(UserFriendInfo.FriendInfo[0].szCompellation, pRequestLocalInfo->GetUserInfo()->szCompellation,
               CountArray(UserFriendInfo.FriendInfo[0].szCompellation));
      lstrcpyn(UserFriendInfo.FriendInfo[0].szPhoneMode, pRequestLocalInfo->GetUserInfo()->szPhoneMode,
               CountArray(UserFriendInfo.FriendInfo[0].szPhoneMode));
  ...
  if (pRequestUserItem != NULL) {
    if (pRequestUserItem->GetUserItemKind() == enLocalKind) {
      ...
      if (pRequestUserItem->GetMainStatus() != FRIEND_US_OFFLINE) {
        ...
        lstrcpyn(UserFriendInfo.FriendInfo[0].szUnderWrite, pServerLocalInfo->GetUserInfo()->szUnderWrite,
                 CountArray(UserFriendInfo.FriendInfo[0].szUnderWrite));
        lstrcpyn(UserFriendInfo.FriendInfo[0].szCompellation, pServerLocalInfo->GetUserInfo()->szCompellation,
                 CountArray(UserFriendInfo.FriendInfo[0].szCompellation));
        lstrcpyn(UserFriendInfo.FriendInfo[0].szPhoneMode, pServerLocalInfo->GetUserInfo()->szPhoneMode,
                 CountArray(UserFriendInfo.FriendInfo[0].szPhoneMode));
      ...
      // 消息提醒
      CMD_GC_RespondNotify RespondNotify;
      SnprintfT(RespondNotify.szNotifyContent, CountArray(RespondNotify.szNotifyContent), TEXT("%s 同意了您的好友申请！"),
                pServerLocalInfo->GetNickName());
