...
public:
  // 修改状态
  virtual VOID SetMainStatus(BYTE cbMainStatus) = 0;
  // 设置状态
  virtual VOID SetGameStatus(BYTE cbGameStatus, WORD wServerID, WORD wTableID) = 0;
  // 用户状态
  virtual BYTE GetMainStatus() = 0;
  // 用户状态
  virtual BYTE GetGameStatus() = 0;
  // 房间标识
  virtual WORD GetServerID() = 0;
  // 桌子标识
...
// 用户索引类
using CServerUserItemArray = std::vector<CServerUserItem*>;
using CServerUserItemMap = std::unordered_map<DWORD, CServerUserItem*>;

// 用户管理类
class CServerUserManager {
  // 用户变量
protected:
  CServerUserItemMap m_UserIDMap{PRIME_PLATFORM_USER}; // 用户索引
...
public:
  // 在线人数
  virtual DWORD GetUserItemCount() { return (DWORD) m_UserItemArray.size(); }
...
// 用户索引类
using CUserFriendGroupArray = std::vector<CUserFriendGroup*>;
using CUserFriendGroupMap = std::unordered_map<DWORD, CUserFriendGroup*>;

// 好友管理类
class CFriendGroupManager {
  // 用户变量
protected:
  CUserFriendGroupMap m_UserIDMap{PRIME_PLATFORM_USER}; // 用户索引
