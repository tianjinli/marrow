...
namespace detail {
  ...
  constexpr auto get([[maybe_unused]] const T& first, [[maybe_unused]] const Args&... rest) -> const auto& {
  ...
  constexpr auto get_arg_index_by_name(basic_string_view<Char> name, type_list<Args...>) -> int {
  ...
  template<typename Char>
  constexpr auto make_text(basic_string_view<Char> s, size_t pos, size_t size) -> text<Char> {
    return {{&s[pos], size}};
  }
  ...
  template<typename Char, typename V, int N>
  struct field {
    using char_type = Char;
  ...
  struct runtime_named_field {
    ...
    template<typename OutputIt, typename T>
    constexpr static auto try_format_argument(OutputIt& out,
  ...
  struct spec_field {
    ...
    template<typename OutputIt, typename... T>
    constexpr FMT_INLINE auto format(OutputIt out, const T&... args) const -> OutputIt {
  ...
  template<typename T, typename Char>
  constexpr auto parse_specs(basic_string_view<Char> str, size_t pos, int next_arg_id) -> parse_specs_result<T, Char> {
...
template<typename CompiledFormat, typename... T, typename Char = typename CompiledFormat::char_type,
         FMT_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
FMT_INLINE FMT_CONSTEXPR_STRING auto format(const CompiledFormat& cf, const T&... args) -> std::basic_string<Char> {
  auto s = std::basic_string<Char>();
  cf.format(std::back_inserter(s), args...);
  return s;
}

template<typename OutputIt, typename CompiledFormat, typename... T, FMT_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
constexpr FMT_INLINE auto format_to(OutputIt out, const CompiledFormat& cf, const T&... args) -> OutputIt {
  return cf.format(out, args...);
}

template<typename S, typename... T, FMT_ENABLE_IF(is_compiled_string<S>::value)>
FMT_INLINE FMT_CONSTEXPR_STRING auto format(const S&, T&&... args) -> std::basic_string<typename S::char_type> {
  ...
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>, detail::unknown_format>()) {
    return fmt::format(static_cast<basic_string_view<typename S::char_type>>(S()), std::forward<T>(args)...);
  } else {
...
FMT_CONSTEXPR auto format_to(OutputIt out, const S&, T&&... args) -> OutputIt {
  constexpr auto compiled = detail::compile<T...>(S());
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>, detail::unknown_format>()) {
    return fmt::format_to(out, static_cast<basic_string_view<typename S::char_type>>(S()), std::forward<T>(args)...);
  } else {
...
#define FMT_STATIC_FORMAT(fmt_str, ...) \
  fmt::static_format_result<fmt::formatted_size(FMT_COMPILE(fmt_str), __VA_ARGS__) + 1>(FMT_COMPILE(fmt_str), __VA_ARGS__)
