...
struct rgb {
  ...
  constexpr rgb(color hex) : r((uint32_t(hex) >> 16) & 0xFF), g((uint32_t(hex) >> 8) & 0xFF), b(uint32_t(hex) & 0xFF) {}
  uint8_t r;
  uint8_t g;
  uint8_t b;
};
...
namespace detail {
  ...
  struct color_type {
    ...
    constexpr color_type(rgb rgb_color) noexcept :
        color_type(static_cast<color>((static_cast<uint32_t>(rgb_color.r) << 16) | (static_cast<uint32_t>(rgb_color.g) << 8) | rgb_color.b)) {}
...
namespace detail {
  ...
  struct ansi_color_escape {
    ...
    FMT_CONSTEXPR auto end() const noexcept -> const Char* { return buffer + size; }
