...
public:
  ...
  constexpr auto dynamic() const -> bool { return (data_ & (width_mask | precision_mask)) != 0; }
  ...
  constexpr auto fill_unit() const -> Char {
    ...
    return static_cast<Char>(static_cast<uchar>(fill_data_[0]) | (static_cast<uchar>(fill_data_[1]) << 8) |
...
#define FMT_TYPE_CONSTANT(Type, constant) \
  ...
  template<typename Char>
  FMT_CONSTEXPR auto parse_width(const Char* begin, const Char* end, format_specs& specs, arg_ref<Char>& width_ref, parse_context<Char>& ctx)
      -> const Char* {
  ...
  template<typename Char>
  FMT_CONSTEXPR auto parse_format_specs(const Char* begin, const Char* end, dynamic_format_specs<Char>& specs, parse_context<Char>& ctx,
...
// template 'void fmt::v11::detail::buffer<T>::append(const U *,const U *)'.
  ...
  struct has_back_insert_iterator_container_append<
      OutputIt, InputIt, void_t<decltype(get_container(std::declval<OutputIt>()).append(std::declval<InputIt>(), std::declval<InputIt>()))>>
      : std::true_type {};
  ...
  struct has_back_insert_iterator_container_insert_at_end<
      ...
      void_t<decltype(get_container(std::declval<OutputIt>())
                          .insert(get_container(std::declval<OutputIt>()).end(), std::declval<InputIt>(), std::declval<InputIt>()))>>
      : std::true_type {};
...
#define FMT_BUILTIN
  ...
  constexpr auto encode_types() -> unsigned long long {
    return static_cast<unsigned>(stored_type_constant<First, Context>::value) | (encode_types<Context, T...>() << packed_arg_bits);
  }
  ...
  struct format_arg_store {
    ...
    using type = conditional_t<NUM_NAMED_ARGS == 0, arg_t<Context, NUM_ARGS>[max_of<size_t>(1, NUM_ARGS)],
...
public:
  ...
  FMT_CONSTEXPR context(iterator out, format_args args, locale_ref loc = {}) : out_(out), args_(args), loc_(loc) {}
...
private:
  ...
  using checker = detail::format_string_checker<char, static_cast<int>(sizeof...(T)), num_static_named_args,
...
public:
  ...
  FMT_CONSTEVAL FMT_ALWAYS_INLINE fstring(const char (&s)[N]) : str(s, N - 1) {
    ...
    static_assert(count<(is_view<remove_cvref_t<T>>::value && std::is_reference<T>::value)...>() == 0, "passing views as lvalues is disallowed");
    if (FMT_USE_CONSTEVAL) {
