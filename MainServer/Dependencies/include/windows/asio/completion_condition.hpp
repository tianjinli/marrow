...
namespace asio {
  ...
  namespace detail {
    ...
    public:
      ...
      std::size_t operator()(const Error& err, std::size_t bytes_transferred) {
        return (!!err || bytes_transferred >= size_)
                   ? 0
                   : (size_ - bytes_transferred < default_max_transfer_size ? size_ - bytes_transferred : std::size_t(default_max_transfer_size));
      }
    ...
    template<typename T>
    struct is_completion_condition_helper<T, enable_if_t<is_same<result_of_t<T(asio::error_code, std::size_t)>, bool>::value> > : true_type {};

    template<typename T>
    struct is_completion_condition_helper<T, enable_if_t<is_same<result_of_t<T(asio::error_code, std::size_t)>, std::size_t>::value> > : true_type {};
