...
namespace asio {
  namespace ssl {
    namespace detail {
      ...
      public:
        ...
        io_op(io_op&& other) :
            asio::detail::base_from_cancellation_state<Handler>(static_cast<asio::detail::base_from_cancellation_state<Handler>&&>(other)),
            next_layer_(other.next_layer_),
            core_(other.core_),
                ...
                switch (want_ = op_(core_.engine_, ec_, bytes_transferred_)) {
                  ...
                  case engine::want_output:
                    ...
                    if (core_.expiry(core_.pending_write_) == core_.neg_infin()) {
                      ...
                      asio::async_write(next_layer_, core_.engine_.get_output(core_.output_buffer_), static_cast<io_op&&>(*this));
                    } else {
                      ASIO_HANDLER_LOCATION((__FILE__, __LINE__, Operation::tracking_name()));
  ...
  template<template<typename, typename> class Associator, typename Stream, typename Operation, typename Handler, typename DefaultCandidate>
  struct associator<Associator, ssl::detail::io_op<Stream, Operation, Handler>, DefaultCandidate> : Associator<Handler, DefaultCandidate> {
    static typename Associator<Handler, DefaultCandidate>::type get(const ssl::detail::io_op<Stream, Operation, Handler>& h) noexcept {
