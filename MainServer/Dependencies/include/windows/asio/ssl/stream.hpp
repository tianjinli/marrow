...
namespace asio {
  namespace ssl {
    ...
    public:
      ...
      stream(Arg&& arg, context& ctx) : next_layer_(static_cast<Arg&&>(arg)), core_(ctx.native_handle(), next_layer_.lowest_layer().get_executor()) {}
       ...
       */
      template<typename Arg>
      stream(Arg&& arg, native_handle_type handle) : next_layer_(static_cast<Arg&&>(arg)), core_(handle, next_layer_.lowest_layer().get_executor()) {}
       ...
       */
      stream(stream&& other) : next_layer_(static_cast<Stream&&>(other.next_layer_)), core_(static_cast<detail::stream_core&&>(other.core_)) {}
       ...
       */
      template<ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code)) HandshakeToken = default_completion_token_t<executor_type>>
      auto async_handshake(handshake_type type, HandshakeToken&& token = default_completion_token_t<executor_type>())
          -> decltype(async_initiate<HandshakeToken, void(asio::error_code)>(declval<initiate_async_handshake>(), token, type)) {
       ...
       */
      template<typename ConstBufferSequence,
               ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code, std::size_t)) BufferedHandshakeToken = default_completion_token_t<executor_type>>
      auto async_handshake(handshake_type type, const ConstBufferSequence& buffers,
          ...
          -> decltype(async_initiate<BufferedHandshakeToken, void(asio::error_code, std::size_t)>(declval<initiate_async_buffered_handshake>(), token,
                                                                                                  type, buffers)) {
        return async_initiate<BufferedHandshakeToken, void(asio::error_code, std::size_t)>(initiate_async_buffered_handshake(this), token, type,
                                                                                           buffers);
      }
       ...
       * @c async_write_some operations.
       */
      template<ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code)) ShutdownToken = default_completion_token_t<executor_type>>
      ...
      auto async_write_some(const ConstBufferSequence& buffers, WriteToken&& token = default_completion_token_t<executor_type>())
          -> decltype(async_initiate<WriteToken, void(asio::error_code, std::size_t)>(declval<initiate_async_write_some>(), token, buffers)) {
        return async_initiate<WriteToken, void(asio::error_code, std::size_t)>(initiate_async_write_some(this), token, buffers);
      }
      ...
      auto async_read_some(const MutableBufferSequence& buffers, ReadToken&& token = default_completion_token_t<executor_type>())
          -> decltype(async_initiate<ReadToken, void(asio::error_code, std::size_t)>(declval<initiate_async_read_some>(), token, buffers)) {
        return async_initiate<ReadToken, void(asio::error_code, std::size_t)>(initiate_async_read_some(this), token, buffers);
      }
    ...
    private:
      ...
      public:
        ...
        void operator()(BufferedHandshakeHandler&& handler, handshake_type type, const ConstBufferSequence& buffers) const {
          ...
          detail::async_io(self_->next_layer_, self_->core_, detail::buffered_handshake_op<ConstBufferSequence>(type, buffers), handler2.value);
        }
      ...
      public:
        ...
        void operator()(WriteHandler&& handler, const ConstBufferSequence& buffers) const {
          ...
          detail::async_io(self_->next_layer_, self_->core_, detail::write_op<ConstBufferSequence>(buffers), handler2.value);
        }
      ...
      public:
        ...
        void operator()(ReadHandler&& handler, const MutableBufferSequence& buffers) const {
          ...
          detail::async_io(self_->next_layer_, self_->core_, detail::read_op<MutableBufferSequence>(buffers), handler2.value);
        }
