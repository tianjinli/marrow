...
namespace asio {
  namespace experimental {
    namespace detail {
      ...
      struct parallel_group_completion_handler {
        typedef decay_t<prefer_result_t<associated_executor_t<Handler>, execution::outstanding_work_t::tracked_t>> executor_type;
        ...
        std::tuple<parallel_group_op_result<typename parallel_op_signature_as_tuple<completion_signature_of_t<Ops>>::type>...> args_{};
      };
      ...
      struct parallel_group_op_handler {
        ...
        parallel_group_op_handler(std::shared_ptr<parallel_group_state<Condition, Handler, Ops...>> state) : state_(std::move(state)) {}
      ...
      struct parallel_group_op_handler_with_executor : parallel_group_op_handler<I, Condition, Handler, Ops...> {
        ...
        parallel_group_op_handler_with_executor(std::shared_ptr<parallel_group_state<Condition, Handler, Ops...>> state, executor_type ex) :
            parallel_group_op_handler<I, Condition, Handler, Ops...>(std::move(state)) {
          cancel_proxy_ = &this->state_->cancellation_signals_[I].slot().template emplace<cancel_proxy>(this->state_, std::move(ex));
        }
        ...
        struct cancel_proxy {
          ...
          void operator()(cancellation_type_t type) {
            if (auto state = state_.lock()) {
              ...
              asio::dispatch(executor_, [state, sig, type] {
                sig->emit(type);
              });
            }
          }
      ...
      struct parallel_group_op_launcher {
        ...
        static void launch(Op& op, const std::shared_ptr<parallel_group_state<Condition, Handler, Ops...>>& state) {
          ...
          std::move(op)(parallel_group_op_handler_with_executor<ex_type, I, Condition, Handler, Ops...>(state, std::move(ex)));
        }
      };
      ...
      struct parallel_group_op_launcher<
          I, Op, enable_if_t<is_same<typename associated_executor<Op>::asio_associated_executor_is_unspecialised, void>::value>> {
        template<typename Condition, typename Handler, typename... Ops>
        static void launch(Op& op, const std::shared_ptr<parallel_group_state<Condition, Handler, Ops...>>& state) {
      ...
      struct parallel_group_cancellation_handler {
        parallel_group_cancellation_handler(std::shared_ptr<parallel_group_state<Condition, Handler, Ops...>> state) : state_(std::move(state)) {}
      ...
      void parallel_group_launch(Condition cancellation_condition, Handler handler, std::tuple<Ops...>& ops, asio::detail::index_sequence<I...>) {
        ...
        std::shared_ptr<state_type> state =
            std::allocate_shared<state_type>(asio::detail::recycling_allocator<state_type, asio::detail::thread_info_base::parallel_group_tag>(),
      ...
      struct ranged_parallel_group_completion_handler {
        ...
        void invoke(asio::detail::index_sequence<I...>) {
          typedef typename parallel_op_signature_as_tuple<
              typename ranged_parallel_group_signature<completion_signature_of_t<Op>, Allocator>::raw_type>::type vectors_type;

          // Construct all result vectors using the supplied allocator.
          vectors_type vectors{typename std::tuple_element<I, vectors_type>::type(ASIO_REBIND_ALLOC(Allocator, int)(allocator_))...};
      ...
      struct ranged_parallel_group_state {
        ...
        // The cancellation signals for each operation in the group.
        std::deque<asio::cancellation_signal, ASIO_REBIND_ALLOC(Allocator, asio::cancellation_signal)> cancellation_signals_;
      ...
      struct ranged_parallel_group_op_handler {
        ...
        ranged_parallel_group_op_handler(std::shared_ptr<ranged_parallel_group_state<Condition, Handler, Op, Allocator>> state, std::size_t idx) :
            state_(std::move(state)),
            idx_(idx) {}
      ...
      struct ranged_parallel_group_op_handler_with_executor : ranged_parallel_group_op_handler<Condition, Handler, Op, Allocator> {
        ...
        ranged_parallel_group_op_handler_with_executor(std::shared_ptr<ranged_parallel_group_state<Condition, Handler, Op, Allocator>> state,
                                                       executor_type ex, std::size_t idx) :
            ranged_parallel_group_op_handler<Condition, Handler, Op, Allocator>(std::move(state), idx) {
          cancel_proxy_ = &this->state_->cancellation_signals_[idx].slot().template emplace<cancel_proxy>(this->state_, std::move(ex));
        }
        ...
        // Proxy handler that forwards the emitted signal to the correct executor.
        struct cancel_proxy {
          cancel_proxy(std::shared_ptr<ranged_parallel_group_state<Condition, Handler, Op, Allocator>> state, executor_type ex) :
              state_(std::move(state)),
              executor_(std::move(ex)) {}
      ...
      struct ranged_parallel_group_cancellation_handler {
        ranged_parallel_group_cancellation_handler(std::shared_ptr<ranged_parallel_group_state<Condition, Handler, Op, Allocator>> state) :
            state_(std::move(state)) {}
      ...
      void ranged_parallel_group_launch(Condition cancellation_condition, Handler handler, Range&& range, const Allocator& allocator) {
        ...
        for (auto&& op: std::forward<Range>(range)) {
          ...
          ex_type ex = asio::get_associated_executor(op);
          std::move(op)(ranged_parallel_group_op_handler_with_executor<ex_type, Condition, Handler, op_type, Allocator>(state, std::move(ex), idx++));
        }
  ...
  struct associator<Associator, experimental::detail::parallel_group_completion_handler<Handler, Ops...>, DefaultCandidate>
    ...
    static auto get(const experimental::detail::parallel_group_completion_handler<Handler, Ops...>& h, const DefaultCandidate& c) noexcept
        -> decltype(Associator<Handler, DefaultCandidate>::get(h.handler_, c)) {
      return Associator<Handler, DefaultCandidate>::get(h.handler_, c);
    }
  };

  template<template<typename, typename> class Associator, typename Handler, typename Op, typename Allocator, typename DefaultCandidate>
  struct associator<Associator, experimental::detail::ranged_parallel_group_completion_handler<Handler, Op, Allocator>, DefaultCandidate>
      : Associator<Handler, DefaultCandidate> {
    static typename Associator<Handler, DefaultCandidate>::type get(
