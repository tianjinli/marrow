...
namespace asio {
  ...
  public:
    ...
    constexpr explicit cancellation_state(CancellationSlot slot) : impl_(slot.is_connected() ? &slot.template emplace<impl<>>() : 0) {}
    ...
    constexpr cancellation_state(CancellationSlot slot, InFilter in_filter, OutFilter out_filter) :
        impl_(slot.is_connected()
                  ? &slot.template emplace<impl<InFilter, OutFilter>>(static_cast<InFilter&&>(in_filter), static_cast<OutFilter&&>(out_filter))
                  : 0) {}
