...
namespace asio {
  namespace ip {
    ...
    public:
      ...
      static basic_resolver_results create(asio::detail::addrinfo_type* address_info, const std::string& host_name, const std::string& service_name) {
        ...
        while (address_info) {
          if (address_info->ai_family == ASIO_OS_DEF(AF_INET) || address_info->ai_family == ASIO_OS_DEF(AF_INET6)) {
            ...
            results.values_->push_back(basic_resolver_entry<InternetProtocol>(endpoint, actual_host_name, service_name));
          }
      ...
      static basic_resolver_results create(Windows::Foundation::Collections::IVectorView<Windows::Networking::EndpointPair ^> ^ endpoints,
        ...
        if (endpoints->Size) {
          ...
          for (unsigned int i = 0; i < endpoints->Size; ++i) {
            ...
            if (hints.ai_family == ASIO_OS_DEF(AF_INET) && pair->RemoteHostName->Type != Windows::Networking::HostNameType::Ipv4) {
              continue;
            }

            if (hints.ai_family == ASIO_OS_DEF(AF_INET6) && pair->RemoteHostName->Type != Windows::Networking::HostNameType::Ipv6) {
              continue;
            }

            results.values_->push_back(basic_resolver_entry<InternetProtocol>(
                typename InternetProtocol::endpoint(ip::make_address(asio::detail::winrt_utils::string(pair->RemoteHostName->CanonicalName)),
