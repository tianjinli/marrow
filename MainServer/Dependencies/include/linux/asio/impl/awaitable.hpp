...
namespace asio {
  namespace detail {
    ...
    public:
      ...
      auto await_transform(this_coro::reset_cancellation_state_1_t<Filter> reset) noexcept {
        struct result {
          ...
          auto await_resume() { return this_->attached_thread_->reset_cancellation_state(static_cast<Filter&&>(filter_)); }
        };
      ...
      auto await_transform(this_coro::reset_cancellation_state_2_t<InFilter, OutFilter> reset) noexcept {
        struct result {
          ...
          auto await_resume() {
            return this_->attached_thread_->reset_cancellation_state(static_cast<InFilter&&>(in_filter_), static_cast<OutFilter&&>(out_filter_));
          }
        };
      ...
      auto await_transform(detail::awaitable_thread_has_context_switched) noexcept {
        struct result {
          ...
          bool& await_resume() const noexcept { return this_->attached_thread_->entry_point()->has_context_switched_; }
        };
    ...
    public:
      ...
      awaitable_thread(awaitable<awaitable_thread_entry_point, Executor> p, const Executor& ex, cancellation_slot parent_cancel_slot,
      ...
      void reset_cancellation_state() {
        bottom_of_stack_.frame_->cancellation_state_ = cancellation_state(bottom_of_stack_.frame_->parent_cancellation_slot_);
      }
      ...
      cancellation_slot_type get_cancellation_slot() const noexcept { return bottom_of_stack_.frame_->cancellation_state_.slot(); }
    ...
    public:
      ...
      awaitable_async_op_handler(awaitable_thread<Executor>* h, result_type& result) : awaitable_thread<Executor>(std::move(*h)), result_(result) {}
...
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
#if defined(ASIO_HAS_SOURCE_LOCATION)
              ASIO_HANDLER_LOCATION((self->location_.file_name(), self->location_.line(), self->location_.function_name()));
#endif // defined(ASIO_HAS_SOURCE_LOCATION)
#endif // defined(ASIO_ENABLE_HANDLER_TRACKING)
