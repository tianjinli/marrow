...
namespace asio {
  namespace detail {
    ...
    struct wrapped_executor {
      ...
      static const Dispatcher& get(const Dispatcher& dispatcher) noexcept { return dispatcher; }
    };
    ...
    struct wrapped_executor<Dispatcher, void_type<typename Dispatcher::executor_type>> {
      ...
      static executor_type get(const Dispatcher& dispatcher) noexcept { return dispatcher.get_executor(); }
    };
    ...
    public:
      ...
      wrapped_handler(Dispatcher dispatcher, Handler& handler) : dispatcher_(dispatcher), handler_(static_cast<Handler&&>(handler)) {}

      wrapped_handler(const wrapped_handler& other) : dispatcher_(other.dispatcher_), handler_(other.handler_) {}

      wrapped_handler(wrapped_handler&& other) : dispatcher_(other.dispatcher_), handler_(static_cast<Handler&&>(other.handler_)) {}

      executor_type get_executor() const noexcept { return wrapped_executor<Dispatcher>::get(dispatcher_); }
