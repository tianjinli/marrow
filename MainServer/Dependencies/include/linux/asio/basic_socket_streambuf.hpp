...
/// Iostream streambuf for a socket.
  ...
  protected:
    int_type underflow() {
      ...
      for (;;) {
        ...
        detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer> bufs(asio::buffer(get_buffer_) + putback_max);
        detail::signed_size_type bytes = detail::socket_ops::recv(socket().native_handle(), bufs.buffers(), bufs.count(), 0, ec_);
    ...
    int_type overflow(int_type c) {
      ...
      while (output_buffer.size() > 0) {
        ...
        detail::signed_size_type bytes = detail::socket_ops::send(socket().native_handle(), bufs.buffers(), bufs.count(), 0, ec_);
  ...
  private:
    ...
    int timeout() const {
      int64_t msec = traits_helper::to_posix_duration(traits_helper::subtract(expiry_time_, traits_helper::now())).total_milliseconds();
    ...
    void connect_to_endpoints(EndpointIterator begin, EndpointIterator end) {
      ...
      for (EndpointIterator i = begin; i != end; ++i) {
        ...
        if (detail::socket_ops::getsockopt(socket().native_handle(), 0, SOL_SOCKET, SO_ERROR, &connect_error, &connect_error_len, ec_) ==
