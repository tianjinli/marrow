  ...
  namespace experimental {
    ...
    namespace detail {
      ...
      struct coro_error<std::exception_ptr> {
        static std::exception_ptr invalid() { return std::make_exception_ptr(asio::system_error(coro_error<asio::error_code>::invalid())); }

        static std::exception_ptr cancelled() { return std::make_exception_ptr(asio::system_error(coro_error<asio::error_code>::cancelled())); }

        static std::exception_ptr interrupted() { return std::make_exception_ptr(asio::system_error(coro_error<asio::error_code>::interrupted())); }

        static std::exception_ptr done() { return std::make_exception_ptr(asio::system_error(coro_error<asio::error_code>::done())); }
      };
        ...
        struct awaitable_t {
          ...
          auto await_suspend(coroutine_handle<coro_promise<Y, R, E, A>> h) -> coroutine_handle<> {
            ...
            if (hp.get_executor() == coro.get_executor()) {
              ...
              coro.coro_->awaited_from = dispatch_coroutine(asio::prefer(hp.get_executor(), execution::outstanding_work.tracked), [h]() mutable {
                                           h.resume();
                                         }).handle;
...
#pragma warning(pop)
      ...
      struct coro_promise final : coro_promise_allocator<Allocator>,
        ...
        cancellation_slot_type get_cancellation_slot() const noexcept { return cancel ? cancel->state.slot() : cancellation_slot_type{}; }

        using allocator_type = typename std::allocator_traits<associated_allocator_t<Executor>>::template rebind_alloc<std::byte>;
        ...
        auto await_transform(this_coro::reset_cancellation_state_2_t<InFilter, OutFilter> reset) noexcept {
          struct result {
            ...
            auto await_resume() { return src_->reset_cancellation_state(static_cast<InFilter&&>(in_filter_), static_cast<OutFilter&&>(out_filter_)); }
          };
        ...
        auto await_transform(Op&& op, constraint_t<is_async_operation<Op>::value> = 0) {
          ...
          using handler_type = typename detail::coro_completion_handler_type<signature>::template completion_handler<coro_promise>;
    ...
    struct coro<Yield, Return, Executor, Allocator>::awaitable_t {
      ...
      auto await_suspend(detail::coroutine_handle<detail::coro_promise<Y, R, E, A>> h) -> detail::coroutine_handle<> {
        ...
        if (hp.get_executor() == coro_.get_executor()) {
          ...
          coro_.coro_->awaited_from = detail::dispatch_coroutine(asio::prefer(hp.get_executor(), execution::outstanding_work.tracked), [h]() mutable {
                                        h.resume();
                                      }).handle;
          ...
          struct cancel_handler {
            ...
            cancel_handler(E e, coro& coro) : e(e), coro_(coro.coro_) { st->second.state = cancellation_state(st->second.slot = st->first.slot()); }
          ...
          auto hh = detail::coroutine_handle<detail::coro_promise<Yield, Return, Executor, Allocator>>::from_promise(*coro_.coro_);
    ...
    struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume {
      ...
      auto handle(E exec, WaitHandler&& handler, std::true_type /* error is noexcept */, std::true_type /* result is void */) // noexcept
      {
      ...
      auto handle(E exec, WaitHandler&& handler, std::true_type /* error is noexcept */, std::false_type /* result is void */) // noexcept
      {
      ...
      template<typename E, typename WaitHandler>
      auto handle(E exec, WaitHandler&& handler, std::false_type /* error is noexcept */, std::true_type /* result is void */) {
        return [the_coro = coro_, h = std::forward<WaitHandler>(handler), exec = std::move(exec)]() mutable {
          if (!the_coro) {
      ...
      template<typename E, typename WaitHandler>
      auto handle(E exec, WaitHandler&& handler, std::false_type /* error is noexcept */, std::false_type /* result is void */) {
        return [the_coro = coro_, h = std::forward<WaitHandler>(handler), exec = std::move(exec)]() mutable {
          if (!the_coro) {
          ...
          if (!ch) {
            ...
            return asio::post(exec, asio::append(std::move(h), detail::coro_error<error_type>::done(), result_type{}));
          } else {
            the_coro->awaited_from = detail::post_coroutine(exec, std::move(h), the_coro->error_, the_coro->result_).handle;
            the_coro->reset_error();
            ch.resume();
          }
        };
      ...
      void operator()(WaitHandler&& handler) {
        const auto exec = asio::prefer(get_associated_executor(handler, get_executor()), execution::outstanding_work.tracked);
        ...
        asio::dispatch(get_executor(),
                       handle(exec, std::forward<WaitHandler>(handler), std::integral_constant<bool, is_noexcept>{}, std::is_void<result_type>{}));
      }
      ...
      void operator()(WaitHandler&& handler, Input&& input) {
        ...
        coro_->cancel = &coro_->cancel_source.emplace();
        coro_->cancel->state = cancellation_state(coro_->cancel->slot = get_associated_cancellation_slot(handler));
        asio::dispatch(get_executor(), [h = handle(exec, std::forward<WaitHandler>(handler), std::integral_constant<bool, is_noexcept>{},
