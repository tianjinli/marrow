...
namespace asio {
  namespace experimental {
    ...
    public:
       ...
       */
      basic_channel(basic_channel&& other) : service_(other.service_), executor_(other.executor_) { service_->move_construct(impl_, other.impl_); }
       ...
       */
      template<typename Executor1>
      basic_channel(basic_channel<Executor1, Traits, Signatures...>&& other, constraint_t<is_convertible<Executor1, Executor>::value> = 0) :
          service_(other.service_),
       ...
       */
      template<typename Executor1>
      constraint_t<is_convertible<Executor1, Executor>::value, basic_channel&> operator=(basic_channel<Executor1, Traits, Signatures...>&& other) {
        if (this != &other) {
          service_->move_assign(impl_, *other.service_, other.impl_);
          executor_.~executor_type();
          new (&executor_) executor_type(other.executor_);
       ...
       */
      template<typename... Args, ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code)) CompletionToken ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
    ...
    private:
      ...
      public:
        ...
        void operator()(SendHandler&& handler, payload_type&& payload) const {
          ...
          self_->service_->async_send(self_->impl_, static_cast<payload_type&&>(payload), handler2.value, self_->get_executor());
        }
