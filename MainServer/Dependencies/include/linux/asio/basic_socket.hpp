...
#define ASIO_BASIC_SOCKET_FWD_DECL
  ...
  public:
    ...
    explicit basic_socket(ExecutionContext& context, constraint_t<is_convertible<ExecutionContext&, execution_context&>::value> = 0) :
        impl_(0, 0, context) {}
     ...
     * @throws asio::system_error Thrown on failure.
     */
    basic_socket(const executor_type& ex, const protocol_type& protocol, const native_handle_type& native_socket) : impl_(0, ex) {
      asio::error_code ec;
     ...
     * error occurs.
     */
    std::size_t available(asio::error_code& ec) const { return impl_.get_service().available(impl_.get_implementation(), ec); }
    ...
    auto async_connect(const endpoint_type& peer_endpoint, ConnectToken&& token = default_completion_token_t<executor_type>())
        -> decltype(async_initiate<ConnectToken, void(asio::error_code)>(declval<initiate_async_connect>(), token, peer_endpoint,
      ...
      return async_initiate<ConnectToken, void(asio::error_code)>(initiate_async_connect(this), token, peer_endpoint, open_ec);
    }
     ...
     * @endcode
     */
    endpoint_type remote_endpoint(asio::error_code& ec) const { return impl_.get_service().remote_endpoint(impl_.get_implementation(), ec); }
  ...
  private:
    ...
    public:
      ...
      void operator()(ConnectHandler&& handler, const endpoint_type& peer_endpoint, const asio::error_code& open_ec) const {
        ...
        if (open_ec) {
          asio::post(self_->impl_.get_executor(), asio::detail::bind_handler(static_cast<ConnectHandler&&>(handler), open_ec));
        } else {
          detail::non_const_lvalue<ConnectHandler> handler2(handler);
          self_->impl_.get_service().async_connect(self_->impl_.get_implementation(), peer_endpoint, handler2.value, self_->impl_.get_executor());
        }
      }
