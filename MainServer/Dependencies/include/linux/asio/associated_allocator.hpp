...
namespace asio {
  ...
  namespace detail {
    ...
    template<typename T, typename A>
    struct associated_allocator_impl<T, A, enable_if_t<!has_allocator_type<T>::value>, void_t<typename associator<associated_allocator, T, A>::type>>
  ...
  namespace detail {
    ...
    template<typename T, typename A>
    struct associated_allocator_forwarding_base<
        T, A, enable_if_t<is_same<typename associated_allocator<T, A>::asio_associated_allocator_is_unspecialised, void>::value>> {
      typedef void asio_associated_allocator_is_unspecialised;
    };
...
#endif // !defined(GENERATING_DOCUMENTATION)
  {
    ...
    /// Forwards the request to get the allocator to the associator specialisation
    /// for the unwrapped type @c T.
    static auto get(reference_wrapper<T> t, const Allocator& a) noexcept -> decltype(associated_allocator<T, Allocator>::get(t.get(), a)) {
