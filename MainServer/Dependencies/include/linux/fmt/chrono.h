...
#define FMT_NOMACRO
  ...
  using is_similar_arithmetic_type = bool_constant<(std::is_integral<T>::value && std::is_integral<U>::value) ||
...
public:
  FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return do_parse(ctx, detail::has_tm_gmtoff<std::tm>::value); }
