...
CPersonalRoomServiceManager::~CPersonalRoomServiceManager(void) {
  ...
  if (m_pIPersonalRoomItem != NULL) {
    SafeDelete(m_pIPersonalRoomItem);
  }
}
...
bool CPersonalRoomServiceManager::StartService() {
  ...
  // 启动通知
  if (m_pIPersonalRoomItem != NULL) {
    m_pIPersonalRoomItem->OnStartService();
  }

  return true;
}
// 接口查询
void* CPersonalRoomServiceManager::QueryInterface(const GUID& Guid, DWORD dwQueryVer) {
...
bool CPersonalRoomServiceManager::CreatePersonalRoom(BYTE cbPersonalRoomType) {
  ...
  m_pIPersonalRoomItem = new (std::nothrow) CPersonalRoomGame();
  if (m_pIPersonalRoomItem == NULL) {
    CLogger::Error(TEXT("创建约战房间失败！"));
  }
...
// 数据库事件
bool CPersonalRoomServiceManager::OnEventDataBase(WORD wRequestID, IServerUserItem* pIServerUserItem, VOID* pData, WORD wDataSize,
                                                  DWORD dwContextID) {
  ASSERT(m_pIPersonalRoomItem != NULL);
  if (m_pIPersonalRoomItem != NULL) {
...
// 命令消息
bool CPersonalRoomServiceManager::OnEventSocketPersonalRoom(WORD wSubCmdID, VOID* pData, WORD wDataSize, IServerUserItem* pIServerUserItem,
                                                            DWORD dwSocketID) {
  ASSERT(m_pIPersonalRoomItem != NULL);
  if (m_pIPersonalRoomItem != NULL) {
