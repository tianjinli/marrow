...
// 接口查询
void* CPersonalTableFrameHook::QueryInterface(const GUID& Guid, DWORD dwQueryVer) {
...
// 游戏开始
void CPersonalTableFrameHook::PersonalRoomWriteJoinInfo(DWORD dwUserID, WORD wTableID, WORD wChairID, DWORD dwKindID, TCHAR* szRoomID,
...
// 游戏结束
bool CPersonalTableFrameHook::OnEventGameEnd(WORD wTableID, WORD wChairCount, DWORD dwDrawCountLimit, DWORD& dwPersonalPlayCount, int nSpecialInfoLen,
                                             BYTE* cbSpecialInfo, SYSTEMTIME sysStartTime, tagPersonalUserScoreInfo* PersonalUserScoreInfo) {
  if (m_pPersonalRoomEventSink != NULL) {
    return m_pPersonalRoomEventSink->OnEventGameEnd(wTableID, wChairCount, dwDrawCountLimit, dwPersonalPlayCount, nSpecialInfoLen, cbSpecialInfo,
                                                    sysStartTime, PersonalUserScoreInfo);
  }
