...
#include "GameServer/AttemperEngineSink.h"
#include "GameServer/TableFrame.h"
...
bool CPersonalRoomGame::BindTableFrame(ITableFrame* pTableFrame, WORD wTableID) {
  ...
  // 创建钩子
  CPersonalTableFrameHook* pTableFrameHook = new (std::nothrow) CPersonalTableFrameHook();
...
bool CPersonalRoomGame::OnEventUserItemScore(IServerUserItem* pIServerUserItem, BYTE cbReason) {
  ...
  if ((CServerRule::IsImmediateWriteScore(m_pGameServiceOption->dwServerRule) == true) && (pIServerUserItem->IsVariation() == true)) {
    ...
    for (INT_PTR i = 0; i < m_pGameServiceOption->wTableCount; ++i) {
      ...
      if (pTableFrame) {
        if (pTableFrame->GetTableID() == pIServerUserItem->GetTableID()) {
          ...
          SnprintfT(WritePersonalScore.szPersonalRoomGUID, CountArray(WritePersonalScore.szPersonalRoomGUID), TEXT("%s"), pTableFrame->GetPersonalRoomGUID());
    ...
    if (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL) {
      m_pIDBCorrespondManager->PostDataBaseRequest(WritePersonalScore.dwUserID, DBR_GR_WRITE_PERSONAL_GAME_SCORE, 0L, &WritePersonalScore,
                                                   sizeof(WritePersonalScore), TRUE);
    }
  }
...
// 游戏开始
void CPersonalRoomGame::PersonalRoomWriteJoinInfo(DWORD dwUserID, WORD wTableID, WORD wChairID, DWORD dwKindID, TCHAR* szRoomID,
  ...
  if (pTableFrame) {
    SnprintfT(JoinInfo.szPeronalRoomGUID, CountArray(JoinInfo.szPeronalRoomGUID), TEXT("%s"), pTableFrame->GetPersonalRoomGUID());
  }
  ...
  // 			m_pIDBCorrespondManager->PostDataBaseRequest(PerformRoom.dwUserID, DBR_GR_WRITE_ROOM_TASK_PROGRESS,pBind->dwSocketID, &PerformRoom,
  // sizeof(PerformRoom));
}
...
// 游戏结束
bool CPersonalRoomGame::OnEventGameEnd(WORD wTableID, WORD wChairCount, DWORD dwDrawCountLimit, DWORD& dwPersonalPlayCount, int nSpecialInfoLen,
...
bool CPersonalRoomGame::OnTCPNetworkSubCreateTable(VOID* pData, WORD wDataSize, DWORD dwSocketID, IServerUserItem* pIServerUserItem) {
  ...
  if (wDataSize != sizeof(CMD_GR_CreateTable)) {
    return false;
  }
...
bool CPersonalRoomGame::OnTCPNetworkSubCancelRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID, IServerUserItem* pIServerUserItem) {
  ...
  if (wDataSize != sizeof(CMD_GR_CancelRequest)) {
    return false;
  }
  ...
  if (pTableFrame->CancelTableRequest(pCancelRequest->dwUserID, static_cast<WORD>(pCancelRequest->dwChairID)) == false) {
    return false;
  }
  ...
  for (int i = 0; i < wChairCount; ++i) {
    ...
    IServerUserItem* pUserItem = pTableFrame->GetTableUserItem(i);
    if (pUserItem == NULL || pUserItem == pIServerUserItem) {
      continue;
    }
...
bool CPersonalRoomGame::OnTCPNetworkSubRequestReply(VOID* pData, WORD wDataSize, DWORD dwSocketID, IServerUserItem* pIServerUserItem) {
  ...
  if (wDataSize != sizeof(CMD_GR_RequestReply)) {
    return false;
  }
  ...
  if (pTableFrame->CancelTableRequestReply(pRequestReply->dwUserID, pRequestReply->cbAgree) == false) {
    return false;
  }
...
bool CPersonalRoomGame::OnTCPNetworkSubHostDissumeTable(VOID* pData, WORD wDataSize, DWORD dwSocketID, IServerUserItem* pIServerUserItem) {
  ...
  if (wDataSize != sizeof(CMD_GR_HostDissumeGame)) {
    return false;
  }
  ...
  m_pIDBCorrespondManager->PostDataBaseRequest(CancelCreateTable.dwUserID, DBR_GR_HOST_CANCEL_CREATE_TABLE, dwSocketID, &CancelCreateTable,
...
bool CPersonalRoomGame::OnDBCurrenceRoomCardAndBeant(DWORD dwContextID, VOID* pData, WORD wDataSize, IServerUserItem* pIServerUserItem) {
  ...
  if (sizeof(DBO_GR_CurrenceRoomCardAndBeans) != wDataSize) {
    return false;
  }
  ...
  m_pITCPNetworkEngine->SendData(dwContextID, MDM_GR_PERSONAL_TABLE, SUB_GR_CURRECE_ROOMCARD_AND_BEAN, &CurrenceRoomCardAndBeans,
                                 sizeof(CMD_GR_CurrenceRoomCardAndBeans));
...
bool CPersonalRoomGame::OnDBDissumeTableResult(DWORD dwContextID, VOID* pData, WORD wDataSize, IServerUserItem* pIServerUserItem) {
  ...
  if (sizeof(DBO_GR_DissumeResult) != wDataSize) {
    return false;
  }
  ...
  m_pITCPNetworkEngine->SendData(pDissumeResult->dwSocketID, MDM_GR_PERSONAL_TABLE, SUB_GR_HOST_DISSUME_TABLE_RESULT, &DissumeTable,
                                 sizeof(CMD_GR_DissumeTable));
...
bool CPersonalRoomGame::OnDBLoadPersonalParameter(DWORD dwContextID, VOID* pData, WORD wDataSize, IServerUserItem* pIServerUserItem) {
  ...
  if (wDataSize % sizeof(tagPersonalTableParameter) != 0) {
    return false;
  }

  // 清楚缓存
  ASSERT(m_PersonalTableParameterArray.size() == 0);
  ...
  for (int i = 0; i < wCount; ++i) {
    ...
    m_PersonalTableParameterArray.emplace_back(pPersonalTableParameter);
  }
...
bool CPersonalRoomGame::OnDBCreateSucess(DWORD dwContextID, VOID* pData, WORD wDataSize, IServerUserItem* pIServerUserItem) {
  ...
  if (wDataSize != sizeof(DBO_GR_CreateSuccess)) {
    return false;
  }
  ...
  INT_PTR nSize = m_PersonalTableParameterArray.size();
  for (INT_PTR i = 0; i < nSize; ++i) {
    tagPersonalTableParameter* pTableParameter = m_PersonalTableParameterArray[i];
  ...
  SYSTEMTIME tm;
  GetLocalTime(&tm);
  SnprintfT(szInfo, CountArray(szInfo), TEXT("%d%d%d%d%d%d%d%d"), pIServerUserItem->GetUserID(), tm.wYear, tm.wMonth, tm.wDay, tm.wHour, tm.wMinute, tm.wSecond,
           tm.wMilliseconds);
  ...
  if (pCreateSuccess->cbGameRule[0] == SET_RULE) {
    if (pCreateSuccess->cbGameRule[1] != 0) {
      ...
      // 向客户端发送桌子上椅子数量改变的消息
      m_pITCPNetworkEngine->SendData(pBindParameter->dwSocketID, MDM_GR_PERSONAL_TABLE, SUB_GR_CHANGE_CHAIR_COUNT, &ChangeChairCount,
                                     sizeof(CMD_GR_ChangeChairCount));
    } else {
  ...
  if (pCreateSuccess->cbIsJoinGame) {
    ...
    if (pTableFrame->PerformSitDownAction(wChairID, pIServerUserItem, pCreateSuccess->szPassword) == FALSE) {
      ...
      // 投递数据
      m_pIDBCorrespondManager->PostDataBaseRequest(pCreateSuccess->dwUserID, DBR_GR_CANCEL_CREATE_TABLE, dwContextID, &CancelCreateTable,
                                                   sizeof(DBR_GR_CancelCreateTable));
...
bool CPersonalRoomGame::OnDBCreateFailure(DWORD dwContextID, VOID* pData, WORD wDataSize, IServerUserItem* pIServerUserItem) {
  ...
  if (wDataSize != sizeof(DBO_GR_CreateFailure)) {
    return false;
  }
...
bool CPersonalRoomGame::OnDBCancelCreateTable(DWORD dwContextID, VOID* pData, WORD wDataSize, IServerUserItem* pIServerUserItem) {
  ...
  if (wDataSize != sizeof(DBO_GR_CancelCreateResult)) {
    return false;
  }
