  ...
  if (pBindParameter->ServiceKind == ServiceKind_Game) {
    ...
    if (wBindIndex == m_wCollectItem) {
      ...
      if (m_WaitCollectItemArray.size() > 0) {
        // 提取变量
        // INT_PTR nWaitCount = m_WaitCollectItemArray.size();
        // m_wCollectItem = m_WaitCollectItemArray[nWaitCount - 1];
        m_wCollectItem = m_WaitCollectItemArray.back();

        // 删除数组
        // m_WaitCollectItemArray.erase(m_WaitCollectItemArray.begin() + nWaitCount - 1);
      ...
      // 删除等待
      // for (INT_PTR i = 0; i < m_WaitCollectItemArray.size(); i++) {
      //   if (wBindIndex == m_WaitCollectItemArray[i]) {
      //     m_WaitCollectItemArray.erase(m_WaitCollectItemArray.begin() + i);
      //     break;
      //   }
      // }
