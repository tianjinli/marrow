...
#define NANODBC_CALL_RC(FUNC, RC, ...)                                                                                   \
  do {                                                                                                                   \
    std::cerr << __FILE__ ":" NANODBC_STRINGIZE(__LINE__) " " NANODBC_STRINGIZE(FUNC) "(" #__VA_ARGS__ ")" << std::endl; \
    RC = FUNC(__VA_ARGS__);                                                                                              \
  } while (false) /**/
...
namespace {
  ...
  inline void convert(wide_char_t const* beg, size_t n, std::string& out) {
    ...
    out = converter.to_bytes(reinterpret_cast<unsigned short const*>(beg), reinterpret_cast<unsigned short const*>(beg + n));
#else
  ...
  inline std::string recent_error(SQLHANDLE handle, SQLSMALLINT handle_type, long& native, std::string& state) {
    ...
    do {
      NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDiagRec), rc, handle_type, handle, (SQLSMALLINT) i, sql_state, &native_error, nullptr, 0, &total_bytes);
      ...
      NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDiagRec), rc, handle_type, handle, (SQLSMALLINT) i, sql_state, &native_error, sql_message.data(),
                      (SQLSMALLINT) sql_message.size(), &total_bytes);
...
namespace nanodbc {
  ...
  programming_error::programming_error(std::string const& info) : std::runtime_error(info.c_str()) {}
...
#define NANODBC_THROW_DATABASE_ERROR(handle, handle_type) throw nanodbc::database_error(handle, handle_type, "ODBC database error: ") /**/
#else
...
namespace {
  ...
  template<typename T>
  using is_integral8 = std::integral_constant<bool, std::is_integral<T>::value && sizeof(T) == 1 && !std::is_same<T, char>::value>;

  template<typename T>
  using is_integral16 = std::integral_constant<bool, std::is_integral<T>::value && sizeof(T) == 2 && !std::is_same<T, wchar_t>::value>;

  template<typename T>
  using is_integral32 = std::integral_constant<bool, std::is_integral<T>::value && sizeof(T) == 4 && !std::is_same<T, wchar_t>::value>;
  ...
  template<typename T>
  struct sql_ctype<T, typename std::enable_if<is_integral16<T>::value && std::is_signed<T>::value>::type> {
    static const SQLSMALLINT value = SQL_C_SSHORT;
  };

  template<typename T>
  struct sql_ctype<T, typename std::enable_if<is_integral16<T>::value && std::is_unsigned<T>::value>::type> {
    static const SQLSMALLINT value = SQL_C_USHORT;
  };

  template<typename T>
  struct sql_ctype<T, typename std::enable_if<is_integral32<T>::value && std::is_signed<T>::value>::type> {
    static const SQLSMALLINT value = SQL_C_SLONG;
  };

  template<typename T>
  struct sql_ctype<T, typename std::enable_if<is_integral32<T>::value && std::is_unsigned<T>::value>::type> {
    static const SQLSMALLINT value = SQL_C_ULONG;
  };

  template<typename T>
  struct sql_ctype<T, typename std::enable_if<is_integral64<T>::value && std::is_signed<T>::value>::type> {
    static const SQLSMALLINT value = SQL_C_SBIGINT;
  };

  template<typename T>
  struct sql_ctype<T, typename std::enable_if<is_integral64<T>::value && std::is_unsigned<T>::value>::type> {
  ...
  struct bound_buffer {
    ...
    bound_buffer(T const* values, std::size_t size, std::size_t value_size = 0) : values_(values), size_(size), value_size_(value_size) {}
  ...
  inline void allocate_env_handle(SQLHENV& env) {
    ...
    try {
      NANODBC_CALL_RC(SQLSetEnvAttr, rc, env, SQL_ATTR_ODBC_VERSION, (SQLPOINTER) NANODBC_ODBC_VERSION, SQL_IS_UINTEGER);
      if (!success(rc)) {
        NANODBC_THROW_DATABASE_ERROR(env, SQL_HANDLE_ENV);
      }
    } catch (...) {
...
namespace nanodbc {
  attribute::attribute(long const& attribute, long const& string_length, attribute::variant const& resource) noexcept :
  ...
  void attribute::extractValuePtr() {
    std::visit(
        [this](auto&& arg) {
          ...
          if constexpr (std::is_same_v<T, string> || std::is_same_v<T, std::string> || std::is_same_v<T, std::vector<uint8_t>>) {
...
namespace nanodbc {
  ...
  public:
    ...
    connection_impl() : env_(nullptr), dbc_(nullptr), connected_(false), transactions_(0), rollback_(false) {}
...
#ifdef NANODBC_HAS_STD_VARIANT
    connection_impl(string const& dsn, string const& user, string const& pass, const std::list<attribute>& attributes) :
        env_(nullptr),
        dbc_(nullptr),
        connected_(false),
        transactions_(0),
    ...
    void enable_async(void* event_handle) {
      ...
      RETCODE rc;
      NANODBC_CALL_RC(SQLSetConnectAttr, rc, dbc_, SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE, (SQLPOINTER) SQL_ASYNC_DBC_ENABLE_ON, SQL_IS_INTEGER);
      if (!success(rc)) {
        NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
      }

      NANODBC_CALL_RC(SQLSetConnectAttr, rc, dbc_, SQL_ATTR_ASYNC_DBC_EVENT, event_handle, SQL_IS_POINTER);
      if (!success(rc)) {
        NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
      }
    }
    ...
    void async_complete() {
      ...
      NANODBC_CALL_RC(SQLSetConnectAttr, rc, dbc_, SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE, (SQLPOINTER) SQL_ASYNC_DBC_ENABLE_OFF, SQL_IS_INTEGER);
      if (!success(rc)) {
        NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
      }
    }
    ...
    RETCODE connect(string const& dsn, string const& user, string const& pass, long timeout, void* event_handle = nullptr) {
      ...
      if (event_handle != nullptr) {
        attributes.push_back({SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE, SQL_IS_UINTEGER, (std::uintptr_t) SQL_ASYNC_DBC_ENABLE_ON});
        attributes.push_back({SQL_ATTR_ASYNC_DBC_EVENT, SQL_IS_POINTER, (std::uintptr_t) event_handle});
      }
#endif
    ...
    RETCODE
    connect(string const& dsn, string const& user, string const& pass, std::list<attribute> const& attributes) {
      allocate_env_handle(env_);
      disconnect();
      ...
      for (const attribute& attr: attributes) {
        ...
        if (attr.attribute_ == SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE && attr.value_ptr_ == (void*) (std::intptr_t) SQL_ASYNC_DBC_ENABLE_ON) {
          is_async = true;
        }
#endif
      ...
      NANODBC_CALL_RC(NANODBC_FUNC(SQLConnect), rc, dbc_, (NANODBC_SQLCHAR*) dsn.c_str(), SQL_NTS,
                      !user.empty() ? (NANODBC_SQLCHAR*) user.c_str() : nullptr, SQL_NTS, !pass.empty() ? (NANODBC_SQLCHAR*) pass.c_str() : nullptr,
                      SQL_NTS);
    ...
    connect(string const& connection_string, std::list<attribute> const& attributes) {
      ...
      NANODBC_CALL_RC(NANODBC_FUNC(SQLDriverConnect), rc, dbc_, nullptr, (NANODBC_SQLCHAR*) connection_string.c_str(), SQL_NTS, nullptr, 0, nullptr,
                      SQL_DRIVER_NOPROMPT);
      if (!success(rc) && (!is_async || rc != SQL_STILL_EXECUTING)) {
        NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
      }
    ...
    string catalog_name() const {
      ...
      SQLINTEGER length(0);
      RETCODE rc;
      NANODBC_CALL_RC(NANODBC_FUNC(SQLGetConnectAttr), rc, dbc_, SQL_ATTR_CURRENT_CATALOG, name, sizeof(name) / sizeof(NANODBC_SQLCHAR), &length);
      if (!success(rc)) {
        NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
      }
  ...
  T connection::connection_impl::get_info_impl(short info_type) const {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetInfo), rc, dbc_, info_type, value, sizeof(value) / sizeof(NANODBC_SQLCHAR), &length);
    if (!success(rc)) {
      NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
    }
...
namespace nanodbc {
  ...
  public:
    ...
    explicit transaction_impl(class connection conn) : conn_(std::move(conn)), committed_(false) {
...
namespace nanodbc {
  ...
  public:
...
#ifndef NANODBC_DISABLE_MSSQL_TVP
    void open_tvp(table_valued_parameter& tvp, short param_index, bound_parameter& param, std::vector<null_type>** bind_len_or_null) {
      if (open_tvp_) {
    ...
    void enable_async(void* event_handle) {
      RETCODE rc;
      if (!async_enabled_) {
        NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_ASYNC_ENABLE, (SQLPOINTER) SQL_ASYNC_ENABLE_ON, SQL_IS_INTEGER);
        if (!success(rc)) {
          NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
        }
        async_enabled_ = true;
    ...
    void disable_async() const {
      if (async_enabled_) {
        RETCODE rc;
        NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_ASYNC_ENABLE, (SQLPOINTER) SQL_ASYNC_ENABLE_OFF, SQL_IS_INTEGER);
        if (!success(rc)) {
          NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
        }
        async_enabled_ = false;
    ...
    bool async_prepare(string const& query, void* event_handle, long timeout) { return async_helper(prepare(query, timeout, event_handle)); }

    bool async_execute_direct(class connection& conn, void* event_handle, string const& query, long batch_operations, long timeout,
                              statement& statement) {
...
#endif
    result execute_direct(class connection& conn, string const& query, long batch_operations, long timeout, statement& statement) {
      const batch_ops array_ops(batch_operations);
      return execute_direct(conn, query, array_ops, timeout, statement);
    }

    result execute_direct(class connection& conn, string const& query, batch_ops const& array_sizes, long timeout, statement& statement) {
    ...
    RETCODE just_execute_direct(class connection& conn, string const& query, batch_ops const& array_sizes, long timeout,
                                statement&, // statement
      ...
      if (array_sizes.parameter_array_length > 0) {
        NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_PARAMSET_SIZE, (SQLPOINTER) (std::intptr_t) array_sizes.parameter_array_length, 0);
        if (!success(rc)) {
          NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
        }
      }
    ...
    RETCODE just_execute(long batch_operations, long timeout, statement& /*statement*/, void* event_handle = nullptr) {
      ...
      NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_PARAMSET_SIZE, (SQLPOINTER) (std::intptr_t) batch_operations, 0);
    ...
    result procedure_columns(string const& catalog, string const& schema, string const& procedure, string const& column, statement& statement) {
      if (!open()) {
        throw programming_error("statement has no associated open connection");
      }
      ...
      RETCODE rc;
      NANODBC_CALL_RC(NANODBC_FUNC(SQLProcedureColumns), rc, stmt_, (NANODBC_SQLCHAR*) (catalog.empty() ? nullptr : catalog.c_str()),
                      (catalog.empty() ? 0 : SQL_NTS), (NANODBC_SQLCHAR*) (schema.empty() ? nullptr : schema.c_str()), (schema.empty() ? 0 : SQL_NTS),
                      (NANODBC_SQLCHAR*) procedure.c_str(), SQL_NTS, (NANODBC_SQLCHAR*) (column.empty() ? nullptr : column.c_str()),
    ...
    unsigned long parameter_size(short param_index) {
      ...
      NANODBC_ASSERT(param_size < static_cast<SQLULEN>(std::numeric_limits<unsigned long>::max()));
    ...
    void prepare_bind(short param_index, std::size_t batch_size, param_direction direction, bound_parameter& param) {
    ...
    template<class T>
    void bind(param_direction direction, short param_index, T const* values, std::size_t batch_size, bool const* nulls = nullptr,
              T const* null_sentry = nullptr);

    // handles multiple binary values
    void bind(param_direction direction, short param_index, std::vector<std::vector<uint8_t>> const& values, bool const* nulls = nullptr,
      ...
      for (std::size_t i = 0; i < batch_size; ++i) {
        std::copy(values[i].begin(), values[i].end(), binary_data_[param_index].data() + (i * max_length));
      }
    ...
    void bind_strings(param_direction direction, short param_index, T const* values, std::size_t value_size, std::size_t batch_size,
                      bool const* nulls = nullptr, T const* null_sentry = nullptr);

    template<class T, typename = enable_if_string<T>>
    void bind_strings(param_direction direction, short param_index, std::vector<T> const& values, bool const* nulls = nullptr,
    ...
    void describe_parameters(const short param_index) {
      ...
      disable_async();
#endif
      NANODBC_CALL_RC(SQLDescribeParam, rc, stmt_, static_cast<SQLUSMALLINT>(param_index + 1), &param_descr_data_[param_index].type_,
                      &param_descr_data_[param_index].size_, &param_descr_data_[param_index].scale_, &nullable);
      if (!success(rc)) {
    ...
    void describe_parameters(const std::vector<short>& idx, const std::vector<short>& type, const std::vector<unsigned long>& size,
  ...
  template<class T>
  void statement::statement_impl::bind(param_direction direction, short param_index, T const* values, std::size_t batch_size,
                                       bool const* nulls /*= nullptr*/, T const* null_sentry /*= nullptr*/) {
    bound_parameter param;
    prepare_bind(param_index, batch_size, direction, param);
  ...
  void statement::statement_impl::bind_strings(param_direction direction, short param_index, std::vector<T> const& values,
    ...
    bind_strings(direction, param_index, string_data.data(), max_length, batch_size, nulls, null_sentry);
  }

  template<class T, typename>
  void statement::statement_impl::bind_strings(param_direction direction, short param_index, T const* values, std::size_t value_size,
                                               std::size_t batch_size, bool const* nulls /*= nullptr*/, T const* null_sentry /*= nullptr*/) {
    bound_parameter param;
    prepare_bind(param_index, batch_size, direction, param);
  ...
  bool statement::statement_impl::equals(const timestamp& lhs, const timestamp& rhs) {
    return lhs.year == rhs.year && lhs.month == rhs.month && lhs.day == rhs.day && lhs.hour == rhs.hour && lhs.min == rhs.min && lhs.sec == rhs.sec &&
           lhs.fract == rhs.fract;
  }

  template<>
  std::vector<wide_string::value_type>& statement::statement_impl::get_bound_string_data(short param_index) {
    return wide_string_data_[param_index];
  }

  template<>
  std::vector<std::string::value_type>& statement::statement_impl::get_bound_string_data(short param_index) {
...
namespace nanodbc {
  ...
  public:
    table_valued_parameter_impl() : row_count_(0), param_index_(0), open_(false) {}
    ...
    void open(table_valued_parameter& tvp, statement& stmt, short param_index, std::size_t row_count) {
      ...
      *(SQLLEN*) bind_len_or_null->data() = 0 < row_count ? row_count : SQL_DEFAULT_PARAM;
      ...
      NANODBC_CALL_RC(SQLBindParameter, rc, hstmt, static_cast<SQLUSMALLINT>(param_index + 1), SQL_PARAM_INPUT, SQL_C_DEFAULT, SQL_SS_TABLE,
                      row_count, 0, (SQLPOINTER) tvp_name.data(), SQL_NTS, bind_len_or_null->data());
      ...
      // set param focus
      NANODBC_CALL_RC(NANODBC_FUNC(SQLSetStmtAttr), rc, hstmt, SQL_SOPT_SS_PARAM_FOCUS, (SQLPOINTER) (std::intptr_t) (param_index + 1),
                      SQL_IS_INTEGER);
      if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
        NANODBC_THROW_DATABASE_ERROR(hstmt, SQL_HANDLE_STMT);
      }
    ...
    void close() {
      ...
      if (open_) {
        ...
        if (0 < row_count_ && stmt_impl) {
          // reset param focus
          NANODBC_CALL_RC(NANODBC_FUNC(SQLSetStmtAttr), rc, stmt_impl->native_statement_handle(), SQL_SOPT_SS_PARAM_FOCUS, (SQLPOINTER) nullptr,
                          SQL_IS_INTEGER);
          if (!success(rc)) {
    ...
    void prepare_tvp_name() {
      ...
      // get ipd handle
      NANODBC_CALL_RC(NANODBC_FUNC(SQLGetStmtAttr), rc, hstmt, SQL_ATTR_IMP_PARAM_DESC, &hipd, SQL_IS_POINTER, &str_len);
      if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc) {
        NANODBC_THROW_DATABASE_ERROR(hstmt, SQL_HANDLE_STMT);
      }

      // get tvp name buffer length
      NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, hipd, param_index_ + 1, SQL_CA_SS_TYPE_NAME, nullptr, 0, &buf_len);
      if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc) {
      ...
      // get tvp name
      NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, hipd, param_index_ + 1, SQL_CA_SS_TYPE_NAME, &tvp_name_[0], buf_len, &buf_len);
      if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc) {
        NANODBC_THROW_DATABASE_ERROR(hipd, SQL_HANDLE_DESC);
      }
    }
    ...
    void prepare_tvp_param_all() {
      ...
      NANODBC_CALL_RC(NANODBC_FUNC(SQLSetStmtAttr), rc, hstmt, SQL_SOPT_SS_NAME_SCOPE, (SQLPOINTER) SQL_SS_NAME_SCOPE_TABLE_TYPE, SQL_IS_UINTEGER);
      if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc) {
        NANODBC_THROW_DATABASE_ERROR(hstmt, SQL_HANDLE_STMT);
      }

      NANODBC_CALL_RC(NANODBC_FUNC(SQLColumns), rc, hstmt, nullptr, 0, nullptr, 0, (NANODBC_SQLCHAR*) tvp_name_.data(), SQL_NTS, nullptr, 0);
      if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc) {
        NANODBC_THROW_DATABASE_ERROR(hstmt, SQL_HANDLE_STMT);
      }
    ...
    template<class T>
    void bind(short param_index, T const* values, std::size_t batch_size, bool const* nulls = nullptr, T const* null_sentry = nullptr);

    void bind(short param_index, std::vector<std::vector<uint8_t>> const& values, bool const* nulls = nullptr, uint8_t const* null_sentry = nullptr) {
    ...
    template<class T, typename = enable_if_character<T>>
    void bind_strings(short param_index, T const* values, std::size_t value_size, std::size_t batch_size, bool const* nulls = nullptr,
                      T const* null_sentry = nullptr);
  ...
  template<class T>
  void table_valued_parameter::table_valued_parameter_impl::bind(short param_index, T const* values, std::size_t batch_size,
                                                                 bool const* nulls /*= nullptr*/, T const* null_sentry /*= nullptr*/) {
  ...
  template<class T, typename>
  void table_valued_parameter::table_valued_parameter_impl::bind_strings(short param_index, std::vector<T> const& values,
  ...
  template<class T, typename>
  void table_valued_parameter::table_valued_parameter_impl::bind_strings(short param_index, T const* values, std::size_t value_size,
  ...
  template<>
  bool table_valued_parameter::table_valued_parameter_impl::equals(std::string const& lhs, std::string const& rhs) {
    return std::strncmp(lhs.c_str(), rhs.c_str(), lhs.size()) == 0;
  }

  template<>
  bool table_valued_parameter::table_valued_parameter_impl::equals(const wide_string& lhs, const wide_string& rhs) {
    // e6059ff3a79062f83256b9d1d3c9c8368798781e
    // Functions like `swprintf()`, `wcsftime()`, `wcsncmp()` can not be used
    // with `u16string` types. Instead, prefers to narrow unicode string to
    // work with them, and then widen them after work has been completed.
  ...
  template<>
  bool table_valued_parameter::table_valued_parameter_impl::equals(const timestamp& lhs, const timestamp& rhs) {
    return lhs.year == rhs.year && lhs.month == rhs.month && lhs.day == rhs.day && lhs.hour == rhs.hour && lhs.min == rhs.min && lhs.sec == rhs.sec &&
           lhs.fract == rhs.fract;
  }

  template<>
  std::vector<wide_string::value_type>& table_valued_parameter::table_valued_parameter_impl::get_bound_string_data(short param_index) {
    return wide_string_data_[param_index];
  }

  template<>
  std::vector<std::string::value_type>& table_valued_parameter::table_valued_parameter_impl::get_bound_string_data(short param_index) {
    return string_data_[param_index];
  }
...
namespace nanodbc {
  ...
  public:
    ...
    result_impl(statement stmt, long rowset_size) :
        ...
        has_unbound_(false) {
      RETCODE rc;
      NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_.native_statement_handle(), SQL_ATTR_ROW_ARRAY_SIZE, (SQLPOINTER) (std::intptr_t) rowset_size_, 0);
      if (!success(rc)) {
    ...
    unsigned long position() const {
      ...
      RETCODE rc;
      NANODBC_CALL_RC(SQLGetStmtAttr, rc, stmt_.native_statement_handle(), SQL_ATTR_ROW_NUMBER, &pos, SQL_IS_UINTEGER, nullptr);
      if (!success(rc)) {
        NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
      }
    ...
    string column_datatype_name(short column) const {
      ...
      RETCODE rc;
      NANODBC_CALL_RC(NANODBC_FUNC(SQLColAttribute), rc, stmt_.native_statement_handle(), static_cast<SQLUSMALLINT>(column + 1), SQL_DESC_TYPE_NAME,
                      type_name, sizeof(type_name), &len, nullptr);
      if (!success(rc)) {
        NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
      }
    ...
    string column_datatype_name(string const& column_name) const { return column_datatype_name(this->column(column_name)); }
  ...
  private:
    ...
    void auto_bind_columns() {
      ...
      for (SQLSMALLINT i = 0; i < n_columns; ++i) {
        NANODBC_CALL_RC(NANODBC_FUNC(SQLDescribeCol), rc, stmt_.native_statement_handle(), static_cast<SQLUSMALLINT>(i + 1),
                        (NANODBC_SQLCHAR*) column_name, sizeof(column_name) / sizeof(NANODBC_SQLCHAR), &len, &sqltype, &sqlsize, &scale, &nullable);
        if (!success(rc)) {
          NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
        }
    ...
    void set_current_position() {
      if (rowset_position_ < rowset_size_ && rowset_position_ < rows()) {
        RETCODE rc;
        NANODBC_CALL_RC(SQLSetPos, rc, stmt_.native_statement_handle(), static_cast<SQLSETPOSIROW>(rowset_position_) + 1, SQL_POSITION,
                        SQL_LOCK_NO_CHANGE);
        if (!success(rc)) {
          NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
        }
    ...
    switch (col.ctype_) {
      ...
      case SQL_C_WCHAR: {
        if (!is_bound(column)) {
          ...
          string::size_type const str_size = col.cbdata_[static_cast<size_t>(rowset_position_)] / sizeof(SQLWCHAR);
          auto const us = reinterpret_cast<wide_char_t const*>(s); // no-op or unsigned short to signed char16_t
      ...
      case SQL_C_DOUBLE: {
        ...
        const int bytes = std::snprintf(const_cast<char*>(buffer.data()), width + 1,
  ...
  inline void result::result_impl::get_ref_impl<_variant_t>(short column, _variant_t& result) const {
    ...
    switch (c_type) {
      ...
      case SQL_C_TYPE_DATE: {
        nanodbc::date v{0};
        get_ref_impl(column, v);
        ::SYSTEMTIME st{static_cast<WORD>(v.year), static_cast<WORD>(v.month), 0, static_cast<WORD>(v.day), 0, 0, 0, 0};
        ::DATE date{0};
      ...
      case SQL_C_TYPE_TIME: {
        nanodbc::time v{0};
        get_ref_impl(column, v);
        ::SYSTEMTIME st{0, 0, 0, 0, static_cast<WORD>(v.hour), static_cast<WORD>(v.min), static_cast<WORD>(v.sec), 0};
        ::DATE date{0};
        if (!::SystemTimeToVariantTime(&st, &date)) {
          throw type_incompatible_error();
        }
      ...
      case SQL_C_TYPE_TIMESTAMP: {
        ...
        get_ref_impl(column, v);
        SYSTEMTIME st{static_cast<WORD>(v.year), static_cast<WORD>(v.month),          0,
  ...
  namespace detail {
    ...
    auto from_string(std::string const& s, R) {
      auto integer = from_string(s, typename std::conditional<std::is_signed<R>::value, long long, unsigned long long>::type{});
  ...
  std::unique_ptr<T, std::function<void(T*)>> result::result_impl::ensure_pdata(short column) const {
    ...
    if (is_bound(column)) {
      ...
      return std::unique_ptr<T, std::function<void(T*)>>((T*) (col.pdata_ + rowset_position_ * col.clen_), [](T*) {
      });
    }
  ...
  std::list<datasource> list_datasources() {
    ...
    do {
      ...
      if (rc == SQL_SUCCESS) {
        ...
        static_assert(sizeof(NANODBC_SQLCHAR) == sizeof(char_type), "incompatible SQLCHAR and string::value_type");
  ...
  std::list<driver> list_drivers() {
    ...
    do {
      ...
      if (rc == SQL_SUCCESS) {
        ...
        for (auto pair_end = std::find(beg, end, NANODBC_TEXT('\0')); pair_end != end; pair_end = std::find(beg, end, NANODBC_TEXT('\0'))) {
...
namespace nanodbc {
  ...
  connection::connection(connection&& rhs) noexcept : impl_(std::move(rhs.impl_)) {}
  ...
  connection::connection(string const& connection_string, long timeout) : impl_(new connection_impl(connection_string, timeout)) {}

#ifdef NANODBC_HAS_STD_VARIANT
  connection::connection(string const& dsn, string const& user, string const& pass, std::list<attribute> const& attributes) :
...
#ifdef NANODBC_HAS_STD_VARIANT
  void connection::connect(string const& dsn, string const& user, string const& pass, std::list<attribute> const& attributes) {
...
#if !defined(NANODBC_DISABLE_ASYNC) && defined(SQL_ATTR_ASYNC_DBC_EVENT)
  bool connection::async_connect(string const& dsn, string const& user, string const& pass, void* event_handle, long timeout) {
...
namespace nanodbc {
  ...
  transaction::transaction(transaction&& rhs) noexcept : impl_(std::move(rhs.impl_)) {}
...
namespace nanodbc {
  ...
  statement::statement(class connection& conn, std::list<attribute> const& attributes) : impl_(new statement_impl(conn, attributes)) {}

  statement::statement(statement&& rhs) noexcept : impl_(std::move(rhs.impl_)) {}

  statement::statement(class connection& conn, string const& query, long timeout) : impl_(new statement_impl(conn, query, timeout)) {}
  ...
  result statement::execute_direct(class connection& conn, string const& query, long batch_operations, long timeout) {
    return impl_->execute_direct(conn, query, batch_operations, timeout, *this);
  }

  result statement::execute_direct(class connection& conn, string const& query, batch_ops const& array_sizes, long timeout) {
  ...
  bool statement::async_execute_direct(class connection& conn, void* event_handle, string const& query, long batch_operations, long timeout) {
  ...
  void statement::just_execute_direct(class connection& conn, string const& query, long batch_operations, long timeout) {
  ...
  result statement::procedure_columns(string const& catalog, string const& schema, string const& procedure, string const& column) {
...
#define NANODBC_INSTANTIATE_BIND_VECTOR_STRINGS(type)                                                               \
  ...
  template void statement::bind_strings(short, std::vector<type> const&, type::value_type const*, param_direction); \
  template void statement::bind_strings(short, std::vector<type> const&, bool const*, param_direction);

#define NANODBC_INSTANTIATE_BIND_STRINGS(type)                                                                                               \
  NANODBC_INSTANTIATE_BIND_VECTOR_STRINGS(type)                                                                                              \
  template void statement::bind_strings(short, const type::value_type*, std::size_t, std::size_t, param_direction);                          \
  template void statement::bind_strings(short, type::value_type const*, std::size_t, std::size_t, type::value_type const*, param_direction); \
  template void statement::bind_strings(short, type::value_type const*, std::size_t, std::size_t, bool const*, param_direction)
...
#undef NANODBC_INSTANTIATE_BIND_STRINGS
  ...
  template<class T>
  void statement::bind(short param_index, T const* values, std::size_t batch_size, param_direction direction) {
    impl_->bind(direction, param_index, values, batch_size);
  }

  template<class T>
  void statement::bind(short param_index, T const* values, std::size_t batch_size, T const* null_sentry, param_direction direction) {
    impl_->bind(direction, param_index, values, batch_size, nullptr, null_sentry);
  }

  template<class T>
  void statement::bind(short param_index, T const* values, std::size_t batch_size, bool const* nulls, param_direction direction) {
    impl_->bind(direction, param_index, values, batch_size, nulls);
  }

  void statement::bind(short param_index, std::vector<std::vector<uint8_t>> const& values, param_direction direction) {
    impl_->bind(direction, param_index, values);
  }

  void statement::bind(short param_index, std::vector<std::vector<uint8_t>> const& values, bool const* nulls, param_direction direction) {
    impl_->bind(direction, param_index, values, nulls);
  }

  void statement::bind(short param_index, std::vector<std::vector<uint8_t>> const& values, uint8_t const* null_sentry, param_direction direction) {
    impl_->bind(direction, param_index, values, nullptr, null_sentry);
  }

  template<class T, typename>
  void statement::bind_strings(short param_index, std::vector<T> const& values, param_direction direction) {
    impl_->bind_strings(direction, param_index, values);
  }

  template<class T, typename>
  void statement::bind_strings(short param_index, T const* values, std::size_t value_size, std::size_t batch_size, param_direction direction) {
    impl_->bind_strings(direction, param_index, values, value_size, batch_size);
  }

  template<class T, typename>
  void statement::bind_strings(short param_index, T const* values, std::size_t value_size, std::size_t batch_size, T const* null_sentry,
                               param_direction direction) {
    impl_->bind_strings(direction, param_index, values, value_size, batch_size, nullptr, null_sentry);
  }

  template<class T, typename>
  void statement::bind_strings(short param_index, T const* values, std::size_t value_size, std::size_t batch_size, bool const* nulls,
                               param_direction direction) {
    impl_->bind_strings(direction, param_index, values, value_size, batch_size, nulls);
  }

  template<class T, typename>
  void statement::bind_strings(short param_index, std::vector<T> const& values, typename T::value_type const* null_sentry,
                               param_direction direction) {
    impl_->bind_strings(direction, param_index, values, nullptr, null_sentry);
  }

  template<class T, typename>
  void statement::bind_strings(short param_index, std::vector<T> const& values, bool const* nulls, param_direction direction) {
  ...
  void statement::describe_parameters(const std::vector<short>& idx, const std::vector<short>& type, const std::vector<unsigned long>& size,
                                      const std::vector<short>& scale) {
...
namespace nanodbc {
  ...
  implementation_row_descriptor::sql_get_descr_field::sql_get_descr_field(implementation_row_descriptor const& ird, short record,
  ...
  implementation_row_descriptor::sql_get_descr_field::operator std::int64_t() const {
    ...
    SQLULEN value = 0;
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, ird_.descriptor_handle_, static_cast<SQLUSMALLINT>(record_ + 1), field_identifier_, &value, 0,
                    nullptr);
    if (!success(rc)) {
  ...
  implementation_row_descriptor::sql_get_descr_field::operator string() const {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, ird_.descriptor_handle_, static_cast<SQLUSMALLINT>(record_ + 1), field_identifier_, value,
  ...
  void implementation_row_descriptor::initialize_descriptor() {
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetStmtAttr), rc, statement_handle_, SQL_ATTR_IMP_ROW_DESC, &descriptor_handle_, 0, nullptr);
    if (!success(rc)) {
      NANODBC_THROW_DATABASE_ERROR(statement_handle_, SQL_HANDLE_STMT);
    }

    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, descriptor_handle_, 0, SQL_DESC_COUNT, &descriptor_records_count_, 0, nullptr);
    if (!success(rc)) {
      NANODBC_THROW_DATABASE_ERROR(statement_handle_, SQL_HANDLE_STMT);
    }
  }
...
namespace nanodbc {
  ...
  table_valued_parameter::table_valued_parameter() : impl_(new table_valued_parameter_impl()) {}

  table_valued_parameter::table_valued_parameter(const table_valued_parameter& rhs) : impl_(rhs.impl_) {}

  table_valued_parameter::table_valued_parameter(table_valued_parameter&& rhs) noexcept : impl_(std::move(rhs.impl_)) {}

  table_valued_parameter::table_valued_parameter(statement& stmt, short param_index, size_t row_count) : impl_(new table_valued_parameter_impl()) {
...
#define NANODBC_INSTANTIATE_TVP_BINDS(type)                                                                     \
  ...
  template void table_valued_parameter::bind(short, const type*, std::size_t, const type*); /* n-ary, sentry */ \
  template void table_valued_parameter::bind(short, const type*, std::size_t, const bool*) /* n-ary, flags */
...
#define NANODBC_INSTANTIATE_TVP_BIND_VECTOR_STRINGS(type)                                                       \
  ...
  template void table_valued_parameter::bind_strings(short, std::vector<type> const&, type::value_type const*); \
...
#define NANODBC_INSTANTIATE_TVP_BIND_STRINGS(type)                                                                                       \
  ...
  template void table_valued_parameter::bind_strings(short, const type::value_type*, std::size_t, std::size_t);                          \
  template void table_valued_parameter::bind_strings(short, type::value_type const*, std::size_t, std::size_t, type::value_type const*); \
  template void table_valued_parameter::bind_strings(short, type::value_type const*, std::size_t, std::size_t, bool const*)
  // The following are the only supported instantiations of statement::bind().
...
#undef NANODBC_INSTANTIATE_TVP_BIND_STRINGS
  ...
  template<class T>
  void table_valued_parameter::bind(short param_index, T const* values, std::size_t batch_size, T const* null_sentry) {
    impl_->bind(param_index, values, batch_size, nullptr, null_sentry);
  }

  template<class T>
  void table_valued_parameter::bind(short param_index, T const* values, std::size_t batch_size, bool const* nulls) {
    impl_->bind(param_index, values, batch_size, nulls);
  }

  void table_valued_parameter::bind(short param_index, std::vector<std::vector<uint8_t>> const& values) {
    impl_->bind(param_index, values);
  }

  void table_valued_parameter::bind(short param_index, std::vector<std::vector<uint8_t>> const& values, bool const* nulls) {
    impl_->bind(param_index, values, nulls);
  }

  void table_valued_parameter::bind(short param_index, std::vector<std::vector<uint8_t>> const& values, uint8_t const* null_sentry) {
  ...
  template<class T, typename>
  void table_valued_parameter::bind_strings(short param_index, T const* values, std::size_t value_size, std::size_t batch_size) {
  ...
  template<class T, typename>
  void table_valued_parameter::bind_strings(short param_index, T const* values, std::size_t value_size, std::size_t batch_size, bool const* nulls) {
    impl_->bind_strings(param_index, values, value_size, batch_size, nulls);
  }

  template<class T, typename>
  void table_valued_parameter::bind_strings(short param_index, std::vector<T> const& values, typename T::value_type const* null_sentry) {
    impl_->bind_strings(param_index, values, nullptr, null_sentry);
  }

  template<class T, typename>
  void table_valued_parameter::bind_strings(short param_index, std::vector<T> const& values, bool const* nulls) {
  ...
  void table_valued_parameter::describe_parameters(const std::vector<short>& idx, const std::vector<short>& type,
...
namespace nanodbc {
  ...
  catalog::procedures::procedures(result& find_result) : result_(find_result) {}
  ...
  catalog::procedure_columns::procedure_columns(result& find_result) : result_(find_result) {}
  ...
  catalog::tables catalog::find_tables(string const& table, string const& type, string const& schema, string const& catalog) {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(), (NANODBC_SQLCHAR*) (catalog.empty() ? nullptr : catalog.c_str()),
                    (catalog.empty() ? 0 : SQL_NTS), (NANODBC_SQLCHAR*) (schema.empty() ? nullptr : schema.c_str()), (schema.empty() ? 0 : SQL_NTS),
  ...
  catalog::procedures catalog::find_procedures(string const& procedure, string const& schema, string const& catalog) {
    // Passing a null pointer to a search pattern argument does not
    // constrain the search for that argument; that is, a null pointer and
    // the search pattern % (any characters) are equivalent.
    // However, a zero-length search pattern - that is, a valid pointer to
    ...
    statement stmt(conn_);
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLProcedures), rc, stmt.native_statement_handle(), (NANODBC_SQLCHAR*) (catalog.empty() ? nullptr : catalog.c_str()),
                    (catalog.empty() ? 0 : SQL_NTS), (NANODBC_SQLCHAR*) (schema.empty() ? nullptr : schema.c_str()), (schema.empty() ? 0 : SQL_NTS),
                    (NANODBC_SQLCHAR*) (procedure.empty() ? nullptr : procedure.c_str()), (procedure.empty() ? 0 : SQL_NTS));
    if (!success(rc)) {
      NANODBC_THROW_DATABASE_ERROR(stmt.native_statement_handle(), SQL_HANDLE_STMT);
    }
  ...
  catalog::procedure_columns catalog::find_procedure_columns(string const& column, string const& procedure, string const& schema,
    ...
    NANODBC_CALL_RC(NANODBC_FUNC(SQLProcedureColumns), rc, stmt.native_statement_handle(),
                    ...
                    (NANODBC_SQLCHAR*) (procedure.empty() ? nullptr : procedure.c_str()), (procedure.empty() ? 0 : SQL_NTS),
                    (NANODBC_SQLCHAR*) (column.empty() ? nullptr : column.c_str()), (column.empty() ? 0 : SQL_NTS));
    if (!success(rc)) {
      NANODBC_THROW_DATABASE_ERROR(stmt.native_statement_handle(), SQL_HANDLE_STMT);
    }
  ...
  catalog::table_privileges catalog::find_table_privileges(string const& catalog, string const& table, string const& schema) {
    // Passing a null pointer to a search pattern argument does not
    // constrain the search for that argument; that is, a null pointer and
    // the search pattern % (any characters) are equivalent.
    // However, a zero-length search pattern - that is, a valid pointer to
    ...
    statement stmt(conn_);
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLTablePrivileges), rc, stmt.native_statement_handle(),
                    (NANODBC_SQLCHAR*) (catalog.empty() ? nullptr : catalog.c_str()), (catalog.empty() ? 0 : SQL_NTS),
                    (NANODBC_SQLCHAR*) (schema.empty() ? nullptr : schema.c_str()), (schema.empty() ? 0 : SQL_NTS),
                    (NANODBC_SQLCHAR*) (table.empty() ? nullptr : table.c_str()), (table.empty() ? 0 : SQL_NTS));
    if (!success(rc)) {
  ...
  catalog::columns catalog::find_columns(string const& column, string const& table, string const& schema, string const& catalog) {
    statement stmt(conn_);
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLColumns), rc, stmt.native_statement_handle(), (NANODBC_SQLCHAR*) (catalog.empty() ? nullptr : catalog.c_str()),
                    (catalog.empty() ? 0 : SQL_NTS), (NANODBC_SQLCHAR*) (schema.empty() ? nullptr : schema.c_str()), (schema.empty() ? 0 : SQL_NTS),
                    (NANODBC_SQLCHAR*) (table.empty() ? nullptr : table.c_str()), (table.empty() ? 0 : SQL_NTS),
                    (NANODBC_SQLCHAR*) (column.empty() ? nullptr : column.c_str()), (column.empty() ? 0 : SQL_NTS));
    if (!success(rc)) {
  ...
  catalog::primary_keys catalog::find_primary_keys(string const& table, string const& schema, string const& catalog) {
    statement stmt(conn_);
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLPrimaryKeys), rc, stmt.native_statement_handle(),
                    (NANODBC_SQLCHAR*) (catalog.empty() ? nullptr : catalog.c_str()), (catalog.empty() ? 0 : SQL_NTS),
                    (NANODBC_SQLCHAR*) (schema.empty() ? nullptr : schema.c_str()), (schema.empty() ? 0 : SQL_NTS),
                    (NANODBC_SQLCHAR*) (table.empty() ? nullptr : table.c_str()), (table.empty() ? 0 : SQL_NTS));
    if (!success(rc)) {
  ...
  std::list<string> catalog::list_catalogs() {
    ...
    statement stmt(conn_);
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(), (NANODBC_SQLCHAR*) SQL_ALL_CATALOGS, 1,
                    (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0);
    if (!success(rc)) {
      NANODBC_THROW_DATABASE_ERROR(stmt.native_statement_handle(), SQL_HANDLE_STMT);
    }
  ...
  std::list<string> catalog::list_schemas() {
    ...
    statement stmt(conn_);
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(), (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0,
                    (NANODBC_SQLCHAR*) SQL_ALL_SCHEMAS, 1, (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0);
    if (!success(rc)) {
      NANODBC_THROW_DATABASE_ERROR(stmt.native_statement_handle(), SQL_HANDLE_STMT);
    }
  ...
  std::list<string> catalog::list_table_types() {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(), (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0,
                    (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR*) NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR*) SQL_ALL_TABLE_TYPES, 1);
    if (!success(rc)) {
      NANODBC_THROW_DATABASE_ERROR(stmt.native_statement_handle(), SQL_HANDLE_STMT);
    }
...
namespace nanodbc {
  ...
  result::result(statement stmt, long rowset_size) : impl_(new result_impl(std::move(stmt), rowset_size)) {}

  result::result(result&& rhs) noexcept : impl_(std::move(rhs.impl_)) {}
  ...
  template void result::get_ref(short, const std::string::value_type&, std::string::value_type&) const;
  template void result::get_ref(short, const wide_string::value_type&, wide_string::value_type&) const;
  ...
  template void result::get_ref(string const&, const std::string::value_type&, std::string::value_type&) const;
  template void result::get_ref(string const&, const wide_string::value_type&, wide_string::value_type&) const;
  ...
  template void result::get_ref(string const&, const unsigned long long int&, unsigned long long int&) const;
  ...
  template void result::get_ref(string const&, const std::vector<std::uint8_t>&, std::vector<std::uint8_t>&) const;
#if defined(_MSC_VER)
  ...
  template std::vector<std::uint8_t> result::get(string const&, const std::vector<std::uint8_t>&) const;
#if defined(_MSC_VER)
