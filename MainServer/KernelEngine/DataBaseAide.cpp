...
// 插入参数
VOID CDataBaseAide::AddParameter(LPCTSTR pszItem, StringViewT pszString, nanodbc::statement::param_direction ParameterDirection) {
  ASSERT(db_engine_ != nullptr);
...
// 插入参数
VOID CDataBaseAide::AddParameter(LPCTSTR pszItem, std::u16string_view pszString, nanodbc::statement::param_direction ParameterDirection){
  ASSERT(db_engine_ != nullptr);
  StringT DBVarValue = ToSimpleString(pszString);
  db_engine_->AddParameter(pszItem, ParameterDirection, DBVarValue);
}
...
// 插入参数
VOID CDataBaseAide::AddParameterOutput(LPCTSTR pszItem, TCHAR* pszString, UINT uSize, nanodbc::statement::param_direction ParameterDirection) {
  ASSERT(db_engine_ != nullptr);
