...
// 接口版本
#define INTERFACE_VERSION(cbMainVer, cbSubVer) (DWORD)((((BYTE) (PRODUCT_VER)) << 24) + (((BYTE) (cbMainVer)) << 16) + ((BYTE) (cbSubVer) << 8))
...
#define QUERYINTERFACE(Interface, Guid, dwQueryVer)                                                                          \
  do {                                                                                                                       \
    if ((std::memcmp(&IID_##Interface, &Guid, sizeof(GUID)) == 0) && (InterfaceVersionCompare(VER_##Interface, dwQueryVer))) \
...
#define QUERYINTERFACE_IUNKNOWNEX(BaseInterface, Guid, dwQueryVer)                                                         \
  do {                                                                                                                     \
    if ((std::memcmp(&IID_IUnknownEx, &Guid, sizeof(GUID)) == 0) && (InterfaceVersionCompare(VER_IUnknownEx, dwQueryVer))) \
...
CTemplateHelper<IModuleInterface>::CTemplateHelper(REFGUID guid, DWORD version, LPCTSTR module_dll_name, const char* create_proc_name) :
    guid_(guid),
    version_(version),
    create_proc_name_(create_proc_name),
    module_dll_name_(module_dll_name) {}
...
#ifdef _UNICODE
#define ToProcNameT(name)             \
  ([&]() -> std::wstring {            \
    const std::string& _s = (name);   \
    std::wstring _t;                  \
    _t.reserve(_s.size());            \
    for (char _c: _s) {               \
      _t += static_cast<wchar_t>(_c); \
    }                                 \
    return _t;                        \
  }())
#else
...
bool CTemplateHelper<IModuleInterface>::CreateInstance() {
  ...
  if (dll_instance_ == nullptr) {
    error_describe_ = std::format(TEXT("“{}”模块加载失败"), module_dll_name_);
    return false;
  }
  ...
  if (create_proc == nullptr) {
    error_describe_ = std::format(TEXT("找不到组件创建函数“{}”: {}"), ToProcNameT(create_proc_name_), DYNLIB_ERROR());
    return false;
  }
  ...
  if (module_interface_ == nullptr) {
    error_describe_ = std ::format(TEXT("调用函数“{}”生成对象失败"), ToProcNameT(create_proc_name_));
    return false;
  }
...
// 获取描述
template<typename IModuleInterface>
const StringT& CTemplateHelper<IModuleInterface>::GetErrorDescribe() const {
  return error_describe_;
}

// 指针重载
template<typename IModuleInterface>
IModuleInterface* CTemplateHelper<IModuleInterface>::operator->() const {
  return GetInterface();
}

// 获取接口
template<typename IModuleInterface>
IModuleInterface* CTemplateHelper<IModuleInterface>::GetInterface() const {
...
#define DECLARE_CREATE_MODULE(OBJECT_NAME)                                                 \
  extern "C" DYNLIB_EXPORT VOID* Create##OBJECT_NAME(REFGUID Guid, DWORD dwInterfaceVer) { \
    C##OBJECT_NAME* raw_object = new (std::nothrow) C##OBJECT_NAME();                      \
    if (raw_object == nullptr)                                                             \
      return nullptr;                                                                      \
    std::unique_ptr<C##OBJECT_NAME> pin_guard(raw_object);                                 \
    void* known_object = raw_object->QueryInterface(Guid, dwInterfaceVer);                 \
    if (known_object == nullptr)                                                           \
      return nullptr;                                                                      \
    /* 4. 🔥 奇迹时刻：只有在这里成功了，我们才让 unique_ptr 放弃所有权！ */               \
    /* release() 会让 pin_guard 撤销托管，把完好无损的对象完整移交给外部系统 */            \
    pin_guard.release();                                                                   \
    return known_object;                                                                   \
  }
