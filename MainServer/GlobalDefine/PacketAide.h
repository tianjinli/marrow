...
protected:
  ...
  LPBYTE m_pcbBuffer = nullptr; // 缓冲指针

  // 函数定义
public:
  // 构造函数
  CSendPacketHelper(VOID* pcbBuffer, WORD wMaxBytes) : m_pcbBuffer((BYTE*) pcbBuffer), m_wMaxBytes(wMaxBytes) {}
...
public:
  // 构造函数
  inline CRecvPacketHelper(VOID* pcbBuffer, WORD wDataSize) : m_pcbBuffer((BYTE*) pcbBuffer), m_wDataSize(wDataSize) {}
...
bool CSendPacketHelper::AddPacket(const wchar_t* pszString, WORD wDataType) {
  ...
  // 插入数据
  return AddPacket((VOID*) pszString, CountStringBuffer(pszString), wDataType);
}
