...
CTableFrame::CTableFrame() {
  ...
  m_MapPersonalTableScoreInfo.reserve(MAX_CHAIR);
      ...
      if (pIServerUserItem != NULL) {
        ...
        if (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL && m_szPersonalTableID != NULL) {
          if (m_pIPersonaTableFrameHook) {
            m_pIPersonaTableFrameHook->PersonalRoomWriteJoinInfo(pIServerUserItem->GetUserID(), m_wTableID, i, m_pGameServiceOption->wKindID,
                                                                 m_szPersonalTableID, m_szRoomGUID);
          }
...
bool CTableFrame::ConcludeGame(BYTE cbGameStatus) {
    ...
    for (WORD i = 0; i < m_wChairCount; i++) {
      ...
      if ((m_pGameServiceOption->lMinTableScore != 0L) && (pIServerUserItem->GetUserScore() < m_pGameServiceOption->lMinTableScore) &&
        ...
        if (m_pGameServiceOption->wServerType & GAME_GENRE_GOLD) {
          SnprintfT(szDescribe, CountArray(szDescribe), TEXT("您的游戏币少于 ") SCORE_STRING TEXT("，不能继续游戏！"),
                    m_pGameServiceOption->lMinTableScore);
        } else {
          SnprintfT(szDescribe, CountArray(szDescribe), TEXT("您的游戏积分少于 ") SCORE_STRING TEXT("，不能继续游戏！"),
                    m_pGameServiceOption->lMinTableScore);
        }
      ...
      if (((m_pGameServiceOption->lMinTableScore != 0L) &&
           pIServerUserItem->GetUserScore() < m_pGameServiceOption->lMinTableScore * m_lCellScore / m_pGameServiceOption->lCellScore) &&
          ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0)) {
        // 构造提示
        Personal_Room_Message personalRoomMessage;
        if (lstrcmp(m_pGameServiceOption->szDataBaseName, szTreasureDB) == 0) {
          auto text = std::format(TEXT("您的游戏币少于 {}，不能继续游戏！"),
                                  m_pGameServiceOption->lMinTableScore * m_lCellScore / m_pGameServiceOption->lCellScore);
          ...
          lstrcpyn(personalRoomMessage.szMessage, text.c_str(), CountArray(personalRoomMessage.szMessage));
          SendGameMessage(pIServerUserItem, text.c_str(), SUB_GF_PERSONAL_MESSAGE);
          m_bBeOut[i] = true;
          continue;
        }
      }
      ...
      if (pIServerUserItem->GetUserStatus() != US_OFFLINE && (m_pGameServiceOption->wServerType & GAME_GENRE_MATCH) == 0) {
        ...
        if (pIServerUserItem->IsAndroidUser() == true) {
          ...
          // 查找机器
          IAndroidUserItem* pIAndroidUserItem =
              m_pIAndroidUserManager->SearchAndroidUserItem(pIServerUserItem->GetUserID(), pBindParameter->dwSocketID);
  ...
  if ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) {
    ...
    // if(m_pIPersonaTableFrameHook!=NULL && m_pIPersonaTableFrameHook->OnEventGameEnd(m_wTableID,  m_wChairCount, m_dwDrawCountLimit,
    // m_dwPersonalPlayCount, m_nSpecialInfoLen, m_cbSpecialInfo, m_PersonalUserScoreInfo))
    if (m_pIPersonaTableFrameHook != NULL &&
        m_pIPersonaTableFrameHook->OnEventGameEnd(m_wTableID, m_wChairCount, m_dwDrawCountLimit, m_dwPersonalPlayCount, m_nSpecialInfoLen,
                                                  m_cbSpecialInfo, m_sysStartTime, m_PersonalUserScoreInfo)) {
      // 桌子设置
      m_dwPersonalPlayCount = 0;
      m_dwTableOwnerID = 0;
      m_dwDrawCountLimit = 0;
      ...
      m_dwReplyCount = 0;
      ZeroMemory(m_bRequestReply, sizeof(m_bRequestReply));
      m_MapPersonalTableScoreInfo.clear();
...
bool CTableFrame::WriteUserScore(WORD wChairID, tagScoreInfo& ScoreInfo, DWORD dwGameMemal, DWORD dwPlayGameTime) {
  ...
  if (pIServerUserItem != NULL) {
    ...
    SCORE lRevenueScore = std::min(m_lFrozenedScore[wChairID], m_pGameServiceOption->lServiceScore);

    // 扣服务费
    if (m_pGameServiceOption->lServiceScore > 0L && m_pGameServiceOption->wServerType == GAME_GENRE_GOLD &&
      ...
      if (ScoreInfo.lScore > 0L) {
        ...
        if (pPropertyBuff != NULL) {
          ...
          if (pPropertyBuff->dwScoreMultiple == 4) {
            SnprintfT(szMessage, CountArray(szMessage), TEXT("[ %s ] 使用了[ 四倍积分卡 ]，得分翻四倍！)"), pIServerUserItem->GetNickName());
          } else if (pPropertyBuff->dwScoreMultiple == 2) {
            SnprintfT(szMessage, CountArray(szMessage), TEXT("[ %s ] 使用了[ 双倍积分卡 ]，得分翻倍！"), pIServerUserItem->GetNickName());
          }
        }
      } else {
        // 附身符
        ...
        if (pPropertyBuff != NULL) {
          // 积分不减
          ScoreInfo.lScore = 0;
          SnprintfT(szMessage, CountArray(szMessage), TEXT("[ %s ] 使用了[ 护身符卡 ]，积分不变！"), pIServerUserItem->GetNickName());
        }
      }
    }
    ...
    // 用户昵称
    lstrcpyn(m_PersonalUserScoreInfo[wChairID].szUserNicname, pIServerUserItem->GetNickName(),
             sizeof(m_PersonalUserScoreInfo[wChairID].szUserNicname));
    ...
    if (pIServerUserItem->IsAndroidUser() == false && CServerRule::IsRecordGameScore(m_pGameServiceOption->dwServerRule) == true) {
      ...
      // 查询库存
      if (m_GameScoreRecordBuffer.size() > 0L) {
        // 获取对象
        INT_PTR nCount = m_GameScoreRecordBuffer.size();
        pGameScoreRecord = m_GameScoreRecordBuffer[nCount - 1];

        // 删除对象
        m_GameScoreRecordBuffer.erase(m_GameScoreRecordBuffer.begin() + nCount - 1);
      }
      ...
      if (pGameScoreRecord != NULL) {
        ...
        // 插入数据
        m_GameScoreRecordActive.emplace_back(pGameScoreRecord);
      }
    }
...
IServerUserItem* CTableFrame::EnumLookonUserItem(WORD wEnumIndex) {
  if (wEnumIndex >= m_LookonUserItemArray.size()) {
    return NULL;
  }
...
bool CTableFrame::SetTableFrameHook(IUnknownEx* pIUnknownEx) {
  ...
  // 类型判断
  if ((m_pGameServiceOption->wServerType & GAME_GENRE_MATCH) == 0 && (m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) == 0) {
    return false;
  }
...
bool CTableFrame::SendChatMessage(IServerUserItem* pIServerUserItem, IServerUserItem* pITargetServerUserItem, DWORD dwChatColor,
  ...
  // 字符过滤
  StringT chat_string = m_pIMainServiceFrame->SensitiveWordFilter(lpszChatString);
  lstrcpyn(UserChat.szChatString, chat_string.c_str(), wChatLength);
...
bool CTableFrame::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter) {
  ...
  switch (dwTimerID) {
    case IDI_OFF_LINE: // 断线事件
    {
      ...
      if (m_bGameStarted == false && m_pGameServiceOption->wServerType != GAME_GENRE_PERSONAL) {
        return false;
      }
      ...
      DWORD dwElapse = TIME_OFF_LINE;
      if ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) {
        dwElapse = TIME_OFF_LINE_PERSONAL;
      }
      ...
      if (wOffLineChair != (WORD) dwBindParameter) {
        ...
        // 设置时间
        dwLapseTime = std::min(dwLapseTime, dwElapse - 2000);
  ...
  if (dwTimerID == IDI_PERSONAL_BEFORE_BEGAIN_TIME || dwTimerID == IDI_PERSONAL_AFTER_CREATE_ROOM_TIME) {
    // 桌子状态
    if (m_bGameStarted == true && m_bPersonalLock == true) {
      return true;
    }
    ...
    m_bIsDissumGame = true;
    // 退还费用
    m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_NOT_START, m_wTableID,
                                            m_szPersonalTableID); // CANCELTABLE_REASON_ENFOCE
    return true;
  }
...
bool CTableFrame::OnEventSocketFrame(WORD wSubCmdID, VOID* pData, WORD wDataSize, IServerUserItem* pIServerUserItem) {
  ...
  switch (wSubCmdID) {
    case SUB_GF_GAME_OPTION: // 游戏配置
    {
      ...
      if (((m_pGameServiceOption->lMinTableScore != 0L) &&
        ...
        if (lstrcmp(m_pGameServiceOption->szDataBaseName, szTreasureDB) == 0) {
          ...
          m_pIMainServiceFrame->SendData(pIServerUserItem, MDM_GR_PERSONAL_TABLE, SUB_GF_PERSONAL_MESSAGE, &personalRoomMessage,
                                         sizeof(personalRoomMessage));
          return true;
        }
      }
      ...
      m_pIMainServiceFrame->SendData(pIServerUserItem, MDM_GR_PERSONAL_TABLE, SUB_GR_PERSONAL_TABLE_TIP, &PersonalTableTip,
                                     sizeof(CMD_GR_PersonalTableTip));

      // 发送消息
      TCHAR szMessage[128] = TEXT("");
      SnprintfT(szMessage, CountArray(szMessage), TEXT("欢迎您进入“%s”游戏，祝您游戏愉快！"), m_pGameServiceAttrib->szGameName);
    ...
    case SUB_GF_USER_CHAT: // 用户聊天
    {
      ...
      ASSERT(wDataSize ==
             (sizeof(CMD_GF_C_UserChat) - sizeof(pUserChat->szChatString) + pUserChat->wChatLength * sizeof(pUserChat->szChatString[0])));
      ...
      // 字符过滤
      StringT src = StringUtils::ToString(pUserChat->szChatString);
      auto chat_string = m_pIMainServiceFrame->SensitiveWordFilter(src.c_str());
    ...
    case SUB_GF_USER_VOICE: // 用户语音
    {
      ...
      ASSERT(wDataSize ==
             (sizeof(CMD_GF_C_UserVoice) - sizeof(pUserVoice->byVoiceData) + pUserVoice->dwVoiceLength * sizeof(pUserVoice->byVoiceData[0])));

      // 效验参数
      if (wDataSize > sizeof(CMD_GF_C_UserVoice)) {
        return false;
      }
      if (wDataSize < (sizeof(CMD_GF_C_UserVoice) - sizeof(pUserVoice->byVoiceData))) {
        return false;
      }
      if (wDataSize !=
          (sizeof(CMD_GF_C_UserVoice) - sizeof(pUserVoice->byVoiceData) + pUserVoice->dwVoiceLength * sizeof(pUserVoice->byVoiceData[0]))) {
    ...
    case SUB_GF_LOOKON_CONFIG: // 旁观配置
    {
      ...
      if (pLookonConfig->dwUserID != 0L) {
        for (INT_PTR i = 0; i < m_LookonUserItemArray.size(); i++) {
          // 获取用户
...
WORD CTableFrame::GetLookonUserCount() {
  // 获取数目
  INT_PTR nLookonCount = m_LookonUserItemArray.size();
...
bool CTableFrame::InitializationFrame(WORD wTableID, tagTableFrameParameter& TableFrameParameter) {
  ...
  SnprintfT(szIniFile, CountArray(szIniFile), TEXT("%s\\PersonalTable.ini"), szWorkDir);
...
bool CTableFrame::PerformStandUpAction(IServerUserItem* pIServerUserItem, bool bInitiative /*=false*/) {
  ...
  if (((m_bGameStarted == true) && ((cbUserStatus == US_PLAYING) || (cbUserStatus == US_OFFLINE)) &&
       m_pGameServiceOption->wServerType != GAME_GENRE_PERSONAL) ||
      (((m_pGameServiceOption->lMinTableScore != 0L) &&
            (pIServerUserItem->GetUserScore() >= m_pGameServiceOption->lMinTableScore * m_lCellScore / m_pGameServiceOption->lCellScore &&
             bTreasureDB) ||
        !bTreasureDB) &&
       (m_PersonalTableParameter.cbIsJoinGame || m_dwPersonalPlayCount != 0 || cbUserStatus >= US_SIT) &&
       !m_bIsDissumGame && // 如果房主参与游戏,且不是强制解散游戏
       (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL &&
        (m_dwPersonalPlayCount != 0 ||
         (m_dwTableOwnerID == pIServerUserItem->GetUserID() && ((m_dwDrawCountLimit != 0 && m_dwDrawCountLimit <= m_dwPersonalPlayCount))))))) {
    // 比赛类型，约战房间类型
    if ((m_pGameServiceOption->wServerType == GAME_GENRE_MATCH && m_pGameMatchOption->cbMatchType == MATCH_TYPE_IMMEDIATE) ||
        m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL &&
            (m_PersonalTableParameter.cbIsJoinGame || cbUserStatus >= US_SIT ||
             (m_dwPersonalPlayCount != 0 && (m_dwDrawCountLimit != 0 && m_dwDrawCountLimit >= m_dwPersonalPlayCount)))) {
      // 设置托管
      pIServerUserItem->SetTrusteeUser(true);
  ...
  if (pIServerUserItem == pITableUserItem) {
    ...
    if (m_pGameServiceOption->wServerType == GAME_GENRE_MATCH && m_pGameMatchOption->cbMatchType == MATCH_TYPE_IMMEDIATE) {
      ...
      if (cbUserStatus == US_OFFLINE && cbUserMatchStatus == MUS_PLAYING) {
        CLogger::Error(TEXT("即时赛中出现玩家: {}起立时状态为US_OFFLINE的情况"), pIServerUserItem->GetUserID());
      }
    }
    ...
    if (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL) {
      ...
      if ((((m_pGameServiceOption->lMinTableScore != 0L) &&
        ...
        KillGameTimer(IDI_PERSONAL_BEFORE_BEGAIN_TIME);
        m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_PLAYER, m_wTableID,
                                                m_szPersonalTableID);
      }
    ...
    for (INT_PTR i = 0; i < m_LookonUserItemArray.size(); i++) {
      if (pIServerUserItem == m_LookonUserItemArray[i]) {
        // 删除子项
        m_LookonUserItemArray.erase(m_LookonUserItemArray.begin() + i);
...
bool CTableFrame::PerformLookonAction(WORD wChairID, IServerUserItem* pIServerUserItem) {
  ...
  if (m_pGameServiceAttrib->wChairCount < MAX_CHAIR && pIServerUserItem->IsAndroidUser() == false) {
    ...
    for (WORD i = 0; i < m_pGameServiceAttrib->wChairCount; i++) {
      ...
      // 模拟判断
      if ((pAndroidParameter->dwServiceMode & ANDROID_SIMULATE) != 0 && (pAndroidParameter->dwServiceMode & ANDROID_PASSIVITY) == 0 &&
  ...
  // 设置用户
  m_LookonUserItemArray.emplace_back(pIServerUserItem);
...
bool CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem* pIServerUserItem, LPCTSTR lpszPassword) {
  ...
  if (((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) && m_dwPersonalPlayCount != 0) {
    ...
    if (!bUserExist) {
      ...
      CLogger::Debug("userid = {}  m_wChairCount = {}  GetSitUserCount() = {}  m_dwPersonalPlayCount = {}  m_wTableID = {} 约战已经开始",
                     pIServerUserItem->GetUserID(), m_wChairCount, GetSitUserCount(), m_dwPersonalPlayCount, m_wTableID);
    }
  ...
  if (pITableUserItem != NULL) {
    ...
    // 构造信息
    TCHAR szDescribe[128] = TEXT("");
    SnprintfT(szDescribe, CountArray(szDescribe), TEXT("椅子已经被 [ %s ] 捷足先登了，下次动作要快点了！"), pITableUserItem->GetNickName());
  ...
  if ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) {
    ...
    if (!bTreasureDB && !bIsExsit) {
      if (ZJH_KIND_ID == m_pGameServiceOption->wKindID || NN_KIND_ID == m_pGameServiceOption->wKindID ||
          TBZ_KIND_ID == m_pGameServiceOption->wKindID) {
        LONGLONG lMinScore = m_pGameServiceOption->lMinTableScore * m_dwDrawCountLimit * m_lCellScore / m_pGameServiceOption->lCellScore;
        LONGLONG lScore = std::max(m_lIniScore, lMinScore);
  ...
  if ((IsGameStarted() == false) || (m_cbStartMode != START_MODE_TIME_CONTROL)) {
    if (CServerRule::IsAllowAvertCheatMode(m_pGameServiceOption->dwServerRule) == false &&
        (m_pGameServiceOption->wServerType & GAME_GENRE_MATCH) == 0) {
      pIServerUserItem->SetClientReady(false);
...
bool CTableFrame::EfficacyIPAddress(IServerUserItem* pIServerUserItem) {
  ...
  if (bCheckSameIP == true) {
    ...
    for (WORD i = 0; i < m_wChairCount; i++) {
      // 获取用户
      IServerUserItem* pITableUserItem = GetTableUserItem(i);
      if ((pITableUserItem != NULL) && (pITableUserItem != pIServerUserItem) && (!pITableUserItem->IsAndroidUser()) &&
          (pITableUserItem->GetMasterOrder() == 0) && (pITableUserItem->GetClientAddr() == dwUserIP)) {
    ...
    for (WORD i = 0; i < m_wChairCount - 1; i++) {
      ...
      if (pITableUserItem != NULL && (!pITableUserItem->IsAndroidUser()) && (pITableUserItem->GetMasterOrder() == 0)) {
        for (WORD j = i + 1; j < m_wChairCount; j++) {
          ...
          if ((pITableNextUserItem != NULL) && (!pITableNextUserItem->IsAndroidUser()) && (pITableNextUserItem->GetMasterOrder() == 0) &&
              (pITableUserItem->GetClientAddr() == pITableNextUserItem->GetClientAddr())) {
  ...
  for (WORD i = 0; i < m_wChairCount; i++) {
    ...
    if (pITableUserItem != NULL) {
      ...
      if ((pTableUserRule->bLimitFleeRate) && (wFleeRate > pTableUserRule->wMaxFleeRate)) {
        // 构造信息
        TCHAR szDescribe[128] = TEXT("");
        SnprintfT(szDescribe, CountArray(szDescribe), TEXT("您的逃跑率太高，与 %s 设置的设置不符，不能加入游戏！"), pITableUserItem->GetNickName());
      ...
      if ((pTableUserRule->bLimitWinRate) && (wWinRate < pTableUserRule->wMinWinRate)) {
        // 构造信息
        TCHAR szDescribe[128] = TEXT("");
        SnprintfT(szDescribe, CountArray(szDescribe), TEXT("您的胜率太低，与 %s 设置的设置不符，不能加入游戏！"), pITableUserItem->GetNickName());
      ...
      if (pTableUserRule->bLimitGameScore == true) {
        ...
        if (pIServerUserItem->GetUserScore() > pTableUserRule->lMaxGameScore) {
          // 构造信息
          TCHAR szDescribe[128] = TEXT("");
          SnprintfT(szDescribe, CountArray(szDescribe), TEXT("您的积分太高，与 %s 设置的设置不符，不能加入游戏！"), pITableUserItem->GetNickName());
        ...
        if (pIServerUserItem->GetUserScore() < pTableUserRule->lMinGameScore) {
          // 构造信息
          TCHAR szDescribe[128] = TEXT("");
          SnprintfT(szDescribe, CountArray(szDescribe), TEXT("您的积分太低，与 %s 设置的设置不符，不能加入游戏！"), pITableUserItem->GetNickName());
...
bool CTableFrame::EfficacyEnterTableScoreRule(WORD wChairID, IServerUserItem* pIServerUserItem) {
  ...
  if (((lMinTableScore != 0L) && (lUserScore < lMinTableScore)) || ((lLessEnterScore != 0L) && (lUserScore < lLessEnterScore))) {
    ...
    TCHAR szDescribe[128] = TEXT("");
    if (m_pGameServiceOption->wServerType == GAME_GENRE_GOLD) {
      SnprintfT(szDescribe, CountArray(szDescribe), TEXT("加入游戏至少需要 ") SCORE_STRING TEXT(" 的游戏币，您的游戏币不够，不能加入！"),
                std::max(lMinTableScore, lLessEnterScore));
    } else if (m_pGameServiceOption->wServerType == GAME_GENRE_MATCH) {
      SnprintfT(szDescribe, CountArray(szDescribe), TEXT("加入游戏至少需要 ") SCORE_STRING TEXT(" 的比赛币，您的比赛币不够，不能加入！"),
                std::max(lMinTableScore, lLessEnterScore));
    } else {
      if (m_pGameServiceOption->wServerType != GAME_GENRE_PERSONAL) {
        SnprintfT(szDescribe, CountArray(szDescribe), TEXT("加入游戏至少需要 ") SCORE_STRING TEXT(" 的游戏积分，您的积分不够，不能加入！"),
                  std::max(lMinTableScore, lLessEnterScore));
  ...
  if (bDrawStarted == true) {
    ...
    if (CServerRule::IsRecordGameScore(m_pGameServiceOption->dwServerRule) == true) {
      ...
      if (dwStartGameTime != INVALID_DWORD) {
        GetLocalTime(&GameScoreRecord.SystemTimeStart);
      } else {
      ...
      // 用户积分
      for (INT_PTR i = 0; i < m_GameScoreRecordActive.size(); i++) {
        // 获取对象
    ...
    // 清理记录
    if (m_GameScoreRecordActive.size() > 0L) {
      m_GameScoreRecordBuffer.insert(m_GameScoreRecordBuffer.end(), m_GameScoreRecordActive.begin(), m_GameScoreRecordActive.end());
      m_GameScoreRecordActive.clear();
    }
  }
}
...
bool CTableFrame::CancelTableRequest(DWORD dwUserID, WORD dwChairID) {
  ...
  if (m_wUserCount == 1 && pUserItem->GetUserID() == m_dwTableOwnerID) // m_PersonalTableParameter.cbIsJoinGame)
  {
    ...
    m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_ENFOCE, m_wTableID,
                                            m_szPersonalTableID);
    m_dwPersonalPlayCount = 0;
...
bool CTableFrame::CancelTableRequestReply(DWORD dwUserID, BYTE cbAgree) {
  ...
  if (wAgreeCount >= wOnlineCount || wAgreeCount >= ((m_wChairCount + 1) / 2) || wAgreeCount >= ((m_nPersonalRoomPlayerCount + 1) / 2)) {
    bCancel = true;
  }
  ...
  if (bCancel == true) {
    ...
    } else // 游戏开始
    {
      ...
      for (int i = 0; i < m_wChairCount; ++i) {
        ...
        m_pIMainServiceFrame->SendData(pTableUserItem, MDM_GR_PERSONAL_TABLE, SUB_GR_PERSONAL_TABLE_END, &PersonalTableEnd,
                                       sizeof(CMD_GR_PersonalTableEnd));
...
bool CTableFrame::GetPersonalScore(DWORD dwUserID, LONGLONG& lScore) {
  if (auto itr = m_MapPersonalTableScoreInfo.find(dwUserID); itr != m_MapPersonalTableScoreInfo.end()) {
    lScore = itr->second;
    return true;
  }
  lScore = 0;
...
void CTableFrame::HostDissumeGame(BOOL bIsEnforce) {
  ...
  if (!bIsEnforce) {
    if (m_dwPersonalPlayCount == 0) {
      ...
      m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_OVER_TIME, m_wTableID,
                                              m_szPersonalTableID);
    }
  }
