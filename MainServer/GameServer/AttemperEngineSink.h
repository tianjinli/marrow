...
using CSystemMessageArray = std::vector<tagSystemMessage*>; // 系统消息
using CTaskParameterArray = std::vector<tagTaskParameter*>; // 任务数组
using CKickUserItemMap = std::unordered_map<DWORD, DWORD>; // 踢人映射
using CTaskParameterMap = std::unordered_map<WORD, tagTaskParameter*>; // 任务参数映射

using CPersonalTableParameterArray = std::vector<tagPersonalTableParameter*>; // 私人配置


//////////////////////////////////////////////////////////////////////////////////

// 调度钩子
class CAttemperEngineSink : IMainServiceFrame,
                            IServerUserItemSink,
                            IAttemperEngineEvent,
                            ITimerEngineEvent,
                            IControlEngineEvent,
...
public:
  ...
  // 关闭事件
  virtual bool OnEventTCPNetworkShut(DWORD dwSocketID, DWORD dwClientAddr, time_t dwActiveTime);
  // 读取事件
...
public:
  ...
  // 敏感词过滤
  virtual StringT SensitiveWordFilter(StringViewT msg);
  // 敏感词过滤
  virtual std::u16string SensitiveWordFilter(std::u16string_view msg);
  // 解锁机器人
...
protected:
  ...
  // 推进任务
  bool PerformTaskProgress(IServerUserItem* pIServerUserItem, tagUserTaskEntry* pUserTaskEntry, DWORD dwWinCount, DWORD dwLostCount,
                           DWORD dwDrawCount);
  // 切换连接
