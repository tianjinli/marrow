...
// 投递请求
bool CServiceUnits::PostControlRequest(WORD wIdentifier, VOID* cbBuffer, WORD wDataSize) {
  // ⚡直接处理消息
  switch (wIdentifier) {
    ...
    case UI_PERSONAL_ROOM_CORRESPOND_RESULT: {
      ...
      // 失败处理
      if (pControlResult->cbSuccess == ER_FAILURE) {
        if (m_PrsnlRmTCPSocketService.GetInterface() != NULL) {
          m_PrsnlRmTCPSocketService->ConcludeService();
        }
      }
      return true;
    }
...
bool CServiceUnits::InitializeService() {
  ...
  if (m_RecordDataBaseEngine->SetDataBaseEngineSink(&m_RecordDataBaseSink) == false) {
    return false;
  }
  if (m_KernelDataBaseEngine->SetDataBaseEngineSink(&m_KernelDataBaseSink) == false) {
    return false;
  }
  ...
  if (m_TCPSocketService->SetTCPSocketEvent(&m_AttemperEngineSink) == false) {
    return false;
  }


  // 约战服务器
  if (m_PrsnlRmTCPSocketService->SetServiceID(NETWORK_PERSONAL_ROOM_CORRESPOND) == false) {
    return false;
  }
  if (m_PrsnlRmTCPSocketService->SetTCPSocketEvent(&m_AttemperEngineSink) == false) {
    return false;
  }
  ...
  if (m_PersonalRoomServiceManager.GetInterface() != NULL) {
    if (m_PersonalRoomServiceManager->CreatePersonalRoom(0) == false) {
      return false;
    }
  }
  ...
  m_AttemperEngineSink.m_pIDBCorrespondManager =
      (IDBCorrespondManager*) m_DBCorrespondManager.QueryInterface(IID_IDBCorrespondManager, VER_IDBCorrespondManager);
  ...
  m_RecordDataBaseSink.m_pIDataBaseEngineEvent = &m_AttemperEngineSink;
  ...
  m_KernelDataBaseSink.m_pIDataBaseEngineEvent = &m_AttemperEngineSink;
  m_KernelDataBaseSink.m_pIDBCorrespondManager =
      (IDBCorrespondManager*) m_DBCorrespondManager.QueryInterface(IID_IDBCorrespondManager, VER_IDBCorrespondManager);
...
bool CServiceUnits::StartKernelService() {
  ...
  // 约战引擎
  if (m_PrsnlRmTCPSocketService->StartService(io_context_) == false) {
    ASSERT(FALSE);
    return false;
  }

  // 数据引擎
  if (m_RecordDataBaseEngine->StartService(io_context_) == false) {
    ASSERT(FALSE);
    return false;
  }

  // 数据引擎
  if (m_KernelDataBaseEngine->StartService(io_context_) == false) {
    ASSERT(FALSE);
    return false;
  }

  // 数据协调
  if (m_DBCorrespondManager.StartService(io_context_) == false) {
    ASSERT(FALSE);
    return false;
  }
...
bool CServiceUnits::RectifyServiceParameter() {
  ...
  if (m_GameServiceManager.module_dll_name_ != m_GameServiceAttrib.szServerDLLName) {
    CLogger::Error(TEXT("游戏模块的绑定 DLL/SO 名字与配置模块 DLL/SO 名字不一致"));
    return false;
  }
  ...
  if ((m_GameServiceOption.cbDistributeRule & DISTRIBUTE_ALLOW) != 0) {
    ...
    // 最少人数
    m_GameServiceOption.wMinDistributeUser = std::max((WORD) 2, m_GameServiceOption.wMinDistributeUser);
    ...
    if (m_GameServiceOption.wMinPartakeGameUser != 0) {
      m_GameServiceOption.wMinPartakeGameUser = std::max(m_GameServiceOption.wMinPartakeGameUser, (WORD) 2);
      m_GameServiceOption.wMinPartakeGameUser = std::min(m_GameServiceOption.wMinPartakeGameUser, m_GameServiceAttrib.wChairCount);
    } else {
    ...
    if (m_GameServiceOption.wMaxPartakeGameUser != 0) {
      m_GameServiceOption.wMaxPartakeGameUser = std::min(m_GameServiceOption.wMaxPartakeGameUser, m_GameServiceAttrib.wChairCount);
      m_GameServiceOption.wMaxPartakeGameUser = std::max(m_GameServiceOption.wMaxPartakeGameUser, m_GameServiceOption.wMinPartakeGameUser);
    } else {
  ...
  m_GameServiceOption.wMaxPlayer = std::max(m_GameServiceOption.wMaxPlayer, (WORD) (wMaxPlayer + RESERVE_USER_COUNT));

  // 最小积分
  if (m_GameServiceOption.wServerType & GAME_GENRE_GOLD) {
    m_GameServiceOption.lMinTableScore += m_GameServiceOption.lServiceScore;
    m_GameServiceOption.lMinTableScore = std::max(m_GameServiceOption.lMinTableScore, m_GameServiceOption.lServiceScore);
  }

  // 限制调整
  if (m_GameServiceOption.lMaxEnterScore != 0L) {
    m_GameServiceOption.lMaxEnterScore = std::max(m_GameServiceOption.lMaxEnterScore, m_GameServiceOption.lMinTableScore);
  }
...
bool CServiceUnits::LoadDataBaseParameter(LPCTSTR pszDataBaseAddr, LPCTSTR pszDataBaseName, tagDataBaseParameter& DataBaseParameter) {
  ...
  // 设置连接
  PlatformDBModule->SetConnectionInfo(pPlatformDBParameter->szDataBaseAddr, pPlatformDBParameter->wDataBasePort, pPlatformDBParameter->szDataBaseName,
                                      pPlatformDBParameter->szDataBaseUser, pPlatformDBParameter->szDataBasePass);
...
void CServiceUnits::WriteSeverCount(bool bStartOrClose) {
  ...
  // 构造路径
  const auto szIniFile = std::filesystem::path(StringT(szWorkDir)) / std::format(TEXT("{}.ini"), m_GameServiceOption.wKindID);
  ...
  // 是否允许配置私人房间参数
  auto cbEnable = ini_reader.GetLongValue(TEXT("Enable"), TEXT("Enabled"), 0);
  ...
  ini_reader.SetLongValue(TEXT("Enable"), TEXT("Enabled"), cbEnable);
}
