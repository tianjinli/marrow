...
int CToken::isChinese(character* _char) {
  if ((unsigned short) _char->val_word.high > 127 && (unsigned short) _char->val_word.low > 127) {
    return CH;
  }
  return EN;
}
...
CToken* tokenArray::insertToken(unsigned short code, character& _char) {
  ...
  if (it != alltokens.end()) {
    ...
    CToken* tok = new (std::nothrow) CToken(code, _char);
...
//////////////////////////////////////////////////////////////////////////
void CFilterTree::BuildWord(StringViewT msg) {}

// 过滤脏词
StringT CFilterTree::Filtrate(StringViewT msg) {
  // 变量定义
  StringT strChat;
  ...
  // 过滤脏词
  strChat = msg;
  for (INT_PTR i = 0; i < m_ForbidArray.size(); i++) {
    strChat = StringUtils::Replace(strChat, m_ForbidArray[i], TEXT("*"));
  }

  //
  StringT pLimit[] = {TEXT("25qp"), TEXT("25ＱＰ"), TEXT("２５ＱＰ"), TEXT("25qＰ"), TEXT("25Ｑp")};
  for (INT_PTR i = 0; i < CountArray(pLimit); i++) {
    strChat = StringUtils::Replace(strChat, pLimit[i], TEXT("foxuc"));
  }
...
void CFilterTree::BuildTree() {
  INT_PTR nCount = m_ForbidArray.size();
...
// 添加禁词
bool CSensitiveWordsFilter::AddSensitiveWords(StringT& pSensitiveWords) {
  // 添加记录
  m_FilterTree.m_ForbidArray.emplace_back(pSensitiveWords);
