...
// 数组定义
using CTableUserItemArray = std::array<IServerUserItem*, MAX_CHAIR>; // 游戏数组
using CLookonUserItemArray = std::vector<IServerUserItem*>; // 旁观数组
using CGameScoreRecordArray = std::vector<tagGameScoreRecord*>; // 记录数组
using CMapPersonalTabelScoreInfo = std::unordered_map<DWORD, LONGLONG>;
...
public:
  ...
  // 聊天接口
public:
  virtual bool SendChatMessage(IServerUserItem* pIServerUserItem, IServerUserItem* pITargetServerUserItem, DWORD dwChatColor, LPCTSTR lpszChatString,
                               LPTSTR lpszDescribeString);
  // 获取信息
public:
  // 获取信息
