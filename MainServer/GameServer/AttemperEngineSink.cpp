...
// 构造函数
CAttemperEngineSink::CAttemperEngineSink() : random_engine_(random_device_()) {
  // 状态变量
  ...
  DYNLIB_HANDLE hInstLibrary = DYNLIB_LOAD(PERSONAL_ROOM_SERVICE_DLL_NAME);
...
CAttemperEngineSink::~CAttemperEngineSink() {
  ...
  // 删除桌子
  for (INT_PTR i = 0; i < m_TableFrameArray.size(); i++) {
    SafeRelease(m_TableFrameArray[i]);
  }

  // 删除消息
  m_SystemMessageBuffer.insert(m_SystemMessageBuffer.end(), m_SystemMessageActive.begin(), m_SystemMessageActive.end());
  for (INT_PTR i = 0; i < m_SystemMessageBuffer.size(); i++) {
    SafeDelete(m_SystemMessageBuffer[i]);
  }

  // 清空数组
  m_SystemMessageActive.clear();
  m_SystemMessageBuffer.clear();

  // 清理数据
  m_KickUserItemMap.clear();
...
VOID* CAttemperEngineSink::QueryInterface(REFGUID Guid, DWORD dwQueryVer) {
  ...
  QUERYINTERFACE(IAttemperEngineEvent, Guid, dwQueryVer);
...
bool CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx* pIUnknownEx) {
  // 绑定信息
  m_pAndroidParameter = new (std::nothrow) tagBindParameter[MAX_ANDROID];
  ZeroMemory(m_pAndroidParameter, sizeof(tagBindParameter) * MAX_ANDROID);

  // 绑定信息
  m_pNormalParameter = new (std::nothrow) tagBindParameter[m_pGameServiceOption->wMaxPlayer];
...
bool CAttemperEngineSink::OnAttemperEngineConclude(IUnknownEx* pIUnknownEx) {
  ...
  // 私人配置
  if (m_PersonalTableParameterArray.size() > 0) {
    INT_PTR nSize = m_PersonalTableParameterArray.size();
    for (INT_PTR i = 0; i < nSize; ++i) {
      tagPersonalTableParameter* pPersonalTableParameter = m_PersonalTableParameterArray[i];
      SafeDelete(pPersonalTableParameter);
    }
    m_PersonalTableParameterArray.clear();
  }
...
bool CAttemperEngineSink::OnEventControl(WORD wIdentifier, VOID* pData, WORD wDataSize) {
  switch (wIdentifier) {
    case CT_CONNECT_CORRESPOND: // 连接协调
    {
      ...
      SnprintfT(szString, CountArray(szString), TEXT("正在连接协调服务器 [ %s:%d ]..."), pCorrespondAddress->szAddress,
                m_pInitParameter->m_wCorrespondPort);
    ...
    case CT_CONNECT_PERSONAL_ROOM_CORRESPOND: {
      ...
      // 构造提示
      TCHAR szString[512] = TEXT("");
      SnprintfT(szString, CountArray(szString), TEXT("正在连接约战服务器 [ %s:%d ]"), pPrsnlRmCorrespondAddress->szAddress,
                m_pInitParameter->m_wPrsnlRmCorrespondPort);
...
bool CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam) {
  try {
      ...
      switch (dwTimerID) {
        ...
        case IDI_DISTRIBUTE_ANDROID: // 分配机器
        {
          ...
          if (m_AndroidUserManager.GetAndroidCount() > 0 && m_pGameServiceOption->wServerType != GAME_GENRE_MATCH) {
            ...
            bool bAllowAvertCheatMode =
                (CServerRule::IsAllowAvertCheatMode(m_pGameServiceOption->dwServerRule) && (m_pGameServiceAttrib->wChairCount < MAX_CHAIR));
            ...
            if (bAllowAndroidSimulate == true && bAllowAvertCheatMode == false) {
              ...
              if (AndroidSimulate.wFreeUserCount > 0) {
                for (WORD i = 0; i < 8; i++) {
                  // 随机桌子
                  WORD wTableID = random_engine_() % (std::max(m_pGameServiceOption->wTableCount / 3, 1));
                  ...
                  if (AndroidSimulate.wFreeUserCount >= TableUserInfo.wMinUserCount) {
                    ...
                    if (m_pGameServiceAttrib->wChairCount > TableUserInfo.wMinUserCount) {
                      ...
                      WORD wOffUserCount = std::min(wChairCount, wFreeUserCount) - TableUserInfo.wMinUserCount;
                      wWantAndroidCount += ((wOffUserCount > 0) ? (random_engine_() % (wOffUserCount + 1)) : 0);
                    }
                ...
                if (AndroidPassivity.wFreeUserCount > 0) {
                  ...
                  if (m_pGameServiceAttrib->wChairCount >= MAX_CHAIR) {
                    ...
                    for (INT_PTR i = 0; i < m_pGameServiceOption->wTableCount; i++) {
                      ...
                      if ((wUserSitCount >= TableUserInfo.wMinUserCount) && (random_engine_() % 100 > 50)) {
                        continue;
                      }
              ...
              if (bAllowAvertCheatMode) {
                ...
                if (AndroidInitiative.wFreeUserCount > 0) {
                  for (INT_PTR i = 0; i < m_pGameServiceOption->wTableCount; i++) {
                    ...
                    // 分配判断
                    if ((m_pGameServiceAttrib->wChairCount < MAX_CHAIR) && wUserSitCount > (TableUserInfo.wMinUserCount - 1) && (random_engine_() % 100 > 50)) {
                      continue;
                    }
            ...
            if (bSinglePlayerGame) {
              ...
              m_dwIntervalTime = MIN_INTERVAL_TIME + random_engine_() % (MAX_INTERVAL_TIME - MIN_INTERVAL_TIME);
            }
            ...
            WORD wRandCount = ((random_engine_() % 3) + 1);
            INT_PTR nIndex = random_engine_() % (std::max(m_pGameServiceOption->wTableCount, (WORD) 1));
            for (INT_PTR i = nIndex; i < m_pGameServiceOption->wTableCount + nIndex; ++i) {
              ...
              // 用户过虑
              bool bRand = ((random_engine_() % 100) > 50);
            ...
            // 起立处理
            nIndex = random_engine_() % std::max(m_pGameServiceOption->wTableCount, (WORD) 1);
            for (INT_PTR i = nIndex; i < m_pGameServiceOption->wTableCount + nIndex; ++i) {
              // 获取桌子
              INT_PTR nTableIndex = i % m_pGameServiceOption->wTableCount;
              CTableFrame* pTableFrame = m_TableFrameArray[nTableIndex];
        ...
        {
          // 数量判断
          if (m_SystemMessageActive.size() == 0) {
            return true;
          }

          // 时效判断
          DWORD dwCurrTime = (DWORD) time(NULL);
          for (size_t nIndex = m_SystemMessageActive.size(); nIndex-- > 0;) {
            ...
            if (pTagSystemMessage->SystemMessage.tConcludeTime < dwCurrTime) {
              m_SystemMessageActive.erase(m_SystemMessageActive.begin() + nIndex);
        ...
        case IDI_ANDROID_JION_MATCH: // 机器人参赛
        {
          ...
          if (m_pGameServiceOption->wServerType == GAME_GENRE_MATCH) {
            ...
            if (bAllowAndroidAttend == true) {
              ...
              // 坐下处理
              WORD wIndex = random_engine_() % (AndroidPassivity.wFreeUserCount + 1);
    ...
    TCHAR szMessage[128];
    SnprintfT(szMessage, CountArray(szMessage), TEXT("定时器异常：TimerID=%d"), dwTimerID);
...
bool CAttemperEngineSink::OnEventDataBaseResult(WORD wRequestID, DWORD dwContextID, VOID* pData, WORD wDataSize) {
  switch (wRequestID) {
    ...
    case DBO_GR_LOAD_PERSONAL_ROOM_OPTION: {
      ...
      if (sizeof(tagPersonalRoomOption) != wDataSize) {
        return false;
      }
...
bool CAttemperEngineSink::OnEventTCPSocketShut(WORD wServiceID, BYTE cbShutReason) {
  ...
  if (wServiceID == NETWORK_CORRESPOND) {
    ...
    if (m_bNeekCorrespond == true) {
      ...
      SnprintfT(szDescribe, CountArray(szDescribe), TEXT("与协调服务器的连接关闭了，%ld 秒后将重新连接"), m_pInitParameter->m_wConnectTime);
    ...
    if (m_bNeekCorrespond == true) {
      // 构造提示
      TCHAR szDescribe[128] = TEXT("");
      SnprintfT(szDescribe, CountArray(szDescribe), TEXT("与约战服务器的连接关闭了，%ld 秒后将重新连接"), m_pInitParameter->m_wConnectTime);
...
bool CAttemperEngineSink::OnEventTCPSocketLink(WORD wServiceID, INT nErrorCode) {
  ...
  if (wServiceID == NETWORK_CORRESPOND) {
    ...
    if (nErrorCode != 0) {
      // 构造提示
      TCHAR szDescribe[128] = TEXT("");
      SnprintfT(szDescribe, CountArray(szDescribe), TEXT("协调服务器连接失败 [ %ld ]，%ld 秒后将重新连接"), nErrorCode,
                m_pInitParameter->m_wConnectTime);
    ...
    // 桌子状态
    for (WORD i = 0; i < m_TableFrameArray.size(); i++) {
      CTableFrame* pTableFrame = m_TableFrameArray[i];
      // 如果使用的是金币数据库
      if (bIsTreasureDB) {
    ...
    if (nErrorCode != 0) {
      // 构造提示
      TCHAR szDescribe[128] = TEXT("");
      SnprintfT(szDescribe, CountArray(szDescribe), TEXT("约战服务器连接失败 [ %ld ]，%ld 秒后将重新连接"), nErrorCode,
                m_pInitParameter->m_wConnectTime);
...
// 关闭事件
bool CAttemperEngineSink::OnEventTCPNetworkShut(DWORD dwSocketID, DWORD dwClientAddr, time_t dwActiveTime) {
  // 变量定义
  WORD wBindIndex = LOWORD(dwSocketID);
  tagBindParameter* pBindParameter = GetBindParameter(wBindIndex);
  if (pBindParameter == NULL) {
  ...
  try {
    ...
    SnprintfT(szMessage, CountArray(szMessage), TEXT("关闭连接异常: wTableID=%d"), wTableID);
...
// 读取事件
bool CAttemperEngineSink::OnEventTCPNetworkRead(DWORD dwSocketID, TCP_Command Command, VOID* pData, WORD wDataSize) {
  switch (Command.wMainCmdID) {
    ...
    case MDM_GF_GAME: // 游戏命令
    {
      ...
      if (!bReturnVal) {
        CLogger::Info(TEXT("游戏命令返回 false   wMainCmdID = {}  wSubCmdID = {}"), Command.wMainCmdID, Command.wSubCmdID);
      }
      return bReturnVal;
    }
    ...
    if (!bViewModeAll) {
      ...
      if (wCmdTable != INVALID_TABLE) {
        ...
        if (wCmdTable == 0) {
          ...
          if ((wTagerDeskPos < wCmdTable - wEverySendPage * wTagerDeskCount) ||
              (wTagerDeskPos > wCmdTable + wEverySendPage * wTagerDeskCount + wTagerDeskCount - 1)) {
            continue;
          }
        }
      }
...
bool CAttemperEngineSink::OnEventUserItemScore(IServerUserItem* pIServerUserItem, BYTE cbReason) {
  ...
  if ((CServerRule::IsImmediateWriteScore(m_pGameServiceOption->dwServerRule) == true) && pIServerUserItem->IsVariation() == true) {
    ...
    // 推进任务
    if (pUserTaskEntry != NULL) {
      PerformTaskProgress(pIServerUserItem, pUserTaskEntry, WriteGameScore.VariationInfo.dwWinCount, WriteGameScore.VariationInfo.dwLostCount,
                          WriteGameScore.VariationInfo.dwDrawCount);
    }
  }
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  // 被踢判断
  if (auto itr = m_KickUserItemMap.find(pDBOLogonSuccess->dwUserID); itr != m_KickUserItemMap.end()) {
    DWORD dwKickTime = itr->second;
    // 时效判断
    DWORD dwCurrTime = (DWORD) time(NULL);
  ...
  if (pIServerUserItem != NULL) {
    // 切换用户
    SwitchUserItemConnect(pIServerUserItem, pDBOLogonSuccess->szMachineID, wBindIndex, pDBOLogonSuccess->cbDeviceType,
                          pDBOLogonSuccess->wBehaviorFlags, pDBOLogonSuccess->wPageTableCount);
  ...
  if (bAndroidUser == true && m_pGameServiceOption->wServerType != GAME_GENRE_MATCH) {
    ...
    if (pIAndroidUserItem != NULL) {
      ...
      if ((pAndroidParameter->lTakeMinScore != 0L) && (pAndroidParameter->lTakeMaxScore != 0L)) {
        ...
        SCORE lTakeMaxScore = (SCORE) std::max(pAndroidParameter->lTakeMaxScore, pAndroidParameter->lTakeMinScore);

        // 调整积分
        if ((lTakeMaxScore - lTakeMinScore) > 0L) {
          SCORE lTakeScore = std::max((lTakeMaxScore - lTakeMinScore) / 10, (SCORE) 1);
          pDBOLogonSuccess->lScore = (SCORE) (lTakeMinScore + (random_engine_() % 10) * lTakeScore + random_engine_() % lTakeScore);
        } else {
  ...
  if ((m_pGameServiceOption->lMinEnterScore != 0L) && (pDBOLogonSuccess->lScore < m_pGameServiceOption->lMinEnterScore)) {
    ...
    SnprintfT(szMsg, CountArray(szMsg), TEXT("抱歉，您的游戏成绩低于当前游戏房间的最低进入成绩%lld，不能进入当前游戏房间！"),
              m_pGameServiceOption->lMinEnterScore);
  ...
  if ((m_pGameServiceOption->lMaxEnterScore != 0L) && (pDBOLogonSuccess->lScore > m_pGameServiceOption->lMaxEnterScore)) {
    // 发送失败
    TCHAR szMsg[128] = TEXT("");
    SnprintfT(szMsg, CountArray(szMsg), TEXT("抱歉，您的游戏成绩高于当前游戏房间的最高进入成绩%lld，不能进入当前游戏房间！"),
              m_pGameServiceOption->lMaxEnterScore);
    SendLogonFailure(szMsg, 0, pBindParameter->dwSocketID);
  ...
  if (m_pGameServiceOption->cbMinEnterMember != 0 && pDBOLogonSuccess->cbMemberOrder < m_pGameServiceOption->cbMinEnterMember &&
      (pDBOLogonSuccess->cbMasterOrder == 0)) {
    // 发送失败
  ...
  if ((pDBOLogonSuccess->cbMemberOrder == 0) && (pDBOLogonSuccess->cbMasterOrder == 0) &&
      (dwOnlineCount > (DWORD) (wMaxPlayer - RESERVE_USER_COUNT))) {
    // 发送失败
...
bool CAttemperEngineSink::OnDBGameColumnInfo(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  if (pGameColumnInfo->cbColumnCount == 0) {
    ...
    CopyMemory(m_DataConfigColumn.ColumnItem, pGameColumnInfo->ColumnItemInfo,
               pGameColumnInfo->cbColumnCount * sizeof(pGameColumnInfo->ColumnItemInfo[0]));
  }
...
bool CAttemperEngineSink::OnDBGameAndroidParameter(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  ASSERT((wDataSize >= wHeadSize) &&
         (wDataSize == (wHeadSize + pGameAndroidParameter->wParameterCount * sizeof(pGameAndroidParameter->AndroidParameter[0]))));
...
bool CAttemperEngineSink::OnDBGameSendTrumpet(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  if (pSendTrumpetResult->bSuccful) {
    ...
    // 字符过滤
    auto trumpet_content = SensitiveWordFilter(pSendTrumpetResult->szTrumpetContent);
    lstrcpyn(SendTrumpet.szTrumpetContent, trumpet_content.c_str(), CountArray(SendTrumpet.szTrumpetContent));
...
bool CAttemperEngineSink::OnDBPropertySuccess(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  if (pPropertySuccess->wPropKind == PROP_KIND_GIFT) {
    ...
    SendPropertyMessage(PropertySuccess.dwSourceUserID, PropertySuccess.dwTargetUserID, PropertySuccess.wPropertyIndex, PropertySuccess.wItemCount);
  }
...
bool CAttemperEngineSink::OnDBQueryPropertyPresent(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  if (pServerUserItem != NULL) {
    ...
    // 发送数据
    m_pIDBCorrespondManager->PostDataBaseRequest(pServerUserItem->GetUserID(), DBR_GR_QUERY_SEND_PRESENT, pBindParameter->dwSocketID,
                                                 &QuerySendPresentRequest, sizeof(QuerySendPresentRequest));
  }
...
bool CAttemperEngineSink::OnDBUserTaskList(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  // 拷贝数据
  WORD wTaskCount = std::min(pTaskListInfo->wTaskCount, (WORD) (CountArray(m_TaskParameter) - m_wTaskCount));
...
bool CAttemperEngineSink::OnDBSystemMessage(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  for (size_t nIndex = m_SystemMessageBuffer.size(); nIndex-- > 0;) {
    ...
    if (pTagSystemMessage && pTagSystemMessage->SystemMessage.dwMessageID == pSystemMessage->dwMessageID) {
      ...
      m_SystemMessageActive.emplace_back(pTagSystemMessage);
      m_SystemMessageBuffer.erase(m_SystemMessageBuffer.begin() + nIndex);
  ...
  // 定义变量
  tagSystemMessage* pSendMessage = new (std::nothrow) tagSystemMessage;
  ...
  // 记录消息
  m_SystemMessageActive.emplace_back(pSendMessage);
...
bool CAttemperEngineSink::OnDBSystemMessageFinish(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  // 处理临时消息
  if (m_SystemMessageBuffer.size() > 0) {
    // 变量定义
...
bool CAttemperEngineSink::OnDBSensitiveWords(DWORD dwContextID, VOID* pData, WORD wDataSize) {
  ...
  // 加载敏感词
  StringT pWords = (const TCHAR*) pData;
  ...
  switch (wSubCmdID) {
    case SUB_CS_S_REGISTER_FAILURE: // 注册失败
    {
      ...
      if (lstrlen(pRegisterFailure->szDescribeString) > 0) {
        StringT szDescribeString = ToSimpleString(pRegisterFailure->szDescribeString);
        CLogger::Error(TEXT("{}: {}"), TEXT(__FUNCTION__), szDescribeString);
      }
...
bool CAttemperEngineSink::OnTCPSocketMainServiceInfo(WORD wSubCmdID, VOID* pData, WORD wDataSize) {
  switch (wSubCmdID) {
    ...
    case SUB_CS_S_CREATE_TABLE_RESULT: // 创建结果
    {
      ...
      if (wDataSize != sizeof(CMD_CS_S_CreateTableResult)) {
        return false;
      }
      ...
      if (pCreateTableResult->PersonalTable.dwDrawCountLimit == 0 && pCreateTableResult->PersonalTable.dwDrawTimeLimit == 0) {
        ...
        // 发送数据
        m_pITCPNetworkEngine->SendData(pCreateTableResult->dwSocketID, MDM_GR_PERSONAL_TABLE, SUB_GR_CREATE_FAILURE, &CreateFailure,
                                       sizeof(CMD_GR_CreateFailure));
      ...
      m_pITCPNetworkEngine->SendData(pCreateTableResult->dwSocketID, MDM_GR_PERSONAL_TABLE, SUB_GR_CREATE_SUCCESS, &CreateSuccess,
                                     sizeof(CMD_GR_CreateSuccess));
    ...
    case SUB_CS_C_DISMISS_TABLE_RESULT: // 解散结果
    {
      ...
      if (wDataSize != sizeof(CMD_CS_C_DismissTableResult)) {
        return false;
      }
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonUserID(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (((m_pGameServiceOption->wServerKind & SERVER_GENRE_PASSWD) != 0) && (LOWORD(dwSocketID) < INDEX_ANDROID)) {
    // 密码比较
    StringT server_password = StringUtils::ToString(pLogonUserID->szServerPasswd);
    if (server_password != m_pGameServiceOption->szServerPasswd) {
      // 发送失败
  ...
  if (pIServerUserItem != NULL) {
    if ((pIServerUserItem->IsAndroidUser() && (pBindParameter->dwClientAddr != 0L)) ||
      ...
      SendRoomMessage(dwSocketID, TEXT("该帐号已在此房间游戏，且不允许踢出，请咨询管理员！"), SMT_CHAT | SMT_EJECT | SMT_GLOBAL | SMT_CLOSE_ROOM,
                      (pBindParameter->dwClientAddr == 0L));
      return true;
    }
  }

  // 密码判断
  StringT strPassword = StringUtils::ToString(pLogonUserID->szPassword);
  strPassword = StringUtils::Trim(strPassword);
  if (pBindParameter->dwClientAddr != 0L && strPassword.empty()) {
    // 发送失败
  ...
  // 切换判断
  StringT password = StringUtils::ToString(pLogonUserID->szPassword);
  TCHAR machine_id[LEN_MACHINE_ID] = {};
  lstrcpyn(machine_id, pLogonUserID->szMachineID, LEN_MACHINE_ID);
  if ((pIServerUserItem != NULL) && (pIServerUserItem->ContrastLogonPass(password.c_str()) == true)) {
    SwitchUserItemConnect(pIServerUserItem, machine_id, wBindIndex);
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonMobile(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (((m_pGameServiceOption->wServerKind & SERVER_GENRE_PASSWD) != 0) && (LOWORD(dwSocketID) < INDEX_ANDROID)) {
    // 密码比较
    StringT server_password = StringUtils::ToString(pLogonMobile->szServerPasswd);
    if (server_password != m_pGameServiceOption->szServerPasswd) {
      // 发送失败
      SendLogonFailure(TEXT("抱歉，您输入的房间密码不正确，请重新输入！"), 0, dwSocketID);
  ...
  // 密码判断
  StringT strPassword = StringUtils::ToString(pLogonMobile->szPassword);
  strPassword = StringUtils::Trim(strPassword);
  if (pBindParameter->dwClientAddr != 0L && strPassword.empty()) {
  ...
  // 切换判断
  StringT password = StringUtils::ToString(pLogonMobile->szPassword);
  TCHAR machine_id[LEN_MACHINE_ID] = {};
  lstrcpyn(machine_id, pLogonMobile->szMachineID, LEN_MACHINE_ID);
  if ((pIServerUserItem != NULL) && (pIServerUserItem->ContrastLogonPass(password.c_str()) == true)) {
    CTraceService::TraceString(TEXT(" 切换判断"), TraceLevel_Debug);
    SwitchUserItemConnect(pIServerUserItem, machine_id, wBindIndex, pLogonMobile->cbDeviceType, pLogonMobile->wBehaviorFlags,
                          pLogonMobile->wPageTableCount);
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonAccounts(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 机器人和真人不许互踢
  StringT accounts = StringUtils::ToString(pLogonAccounts->szAccounts);
  IServerUserItem* pIServerUserItem = m_ServerUserManager.SearchUserItem(accounts.c_str());
  if (pIServerUserItem != NULL) {
    if ((pIServerUserItem->IsAndroidUser() && (pBindParameter->dwClientAddr != 0L)) ||
        (!pIServerUserItem->IsAndroidUser() && (pBindParameter->dwClientAddr == 0L))) {
      SendRoomMessage(dwSocketID, TEXT("该帐号已在此房间游戏，且不允许踢出，请咨询管理员！"), SMT_CHAT | SMT_EJECT | SMT_GLOBAL | SMT_CLOSE_ROOM,
  ...
  // 密码判断
  StringT strPassword = StringUtils::ToString(pLogonAccounts->szPassword);
  strPassword = StringUtils::Trim(strPassword);
  if (pBindParameter->dwClientAddr != 0L && strPassword.empty()) {
    // 发送失败
    SendLogonFailure(TEXT("很抱歉，您的登录密码错误，不允许继续进入！"), LOGON_FAIL_SERVER_INVALIDATION, dwSocketID);
  ...
  // 切换判断
  StringT password = StringUtils::ToString(pLogonAccounts->szPassword);
  TCHAR machine_id[LEN_MACHINE_ID] = {};
  lstrcpyn(machine_id, pLogonAccounts->szMachineID, LEN_MACHINE_ID);
  if ((pIServerUserItem != NULL) && (pIServerUserItem->ContrastLogonPass(password.c_str()) == true)) {
    SwitchUserItemConnect(pIServerUserItem, machine_id, wBindIndex);
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserLookon(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pUserLookon->wTableID >= (WORD) m_TableFrameArray.size()) {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserSitDown(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (wRequestTableID >= m_TableFrameArray.size()) {
    ...
    // 寻找位置
    for (WORD i = wStartTableID; i < m_TableFrameArray.size(); i++) {
      // 游戏状态
  ...
  StringT password = StringUtils::ToString(pUserSitDown->szPassword);
  pTableFrame->PerformSitDownAction(wRequestChairID, pIServerUserItem, password.c_str());
...
bool CAttemperEngineSink::OnTCPNetworkSubUserStandUp(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pUserStandUp->wTableID >= (WORD) m_TableFrameArray.size()) {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserChat(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 权限判断
  if (pUserChat->dwSendUserID != pISendUserItem->GetUserID() &&
      (pISendUserItem->GetMasterOrder() == 0 || CMasterRight::CanManagerAndroid(pISendUserItem->GetMasterRight()) == false)) {
    return false;
  }
  ...
  // 字符过滤
  auto chat_string = SensitiveWordFilter(pUserChat->szChatString);
  lstrcpyn(UserChat.szChatString, chat_string.c_str(), CountArray(UserChat.szChatString));
...
bool CAttemperEngineSink::OnTCPNetworkSubUserExpression(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 权限判断
  if (pUserExpression->dwSendUserID != pISendUserItem->GetUserID() &&
      (pISendUserItem->GetMasterOrder() == 0 || CMasterRight::CanManagerAndroid(pISendUserItem->GetMasterRight()) == false)) {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubWisperChat(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  ASSERT(wDataSize ==
         (sizeof(CMD_GR_C_WisperChat) - sizeof(pWisperChat->szChatString) + pWisperChat->wChatLength * sizeof(pWisperChat->szChatString[0])));
  ...
  // 权限判断
  if (pWisperChat->dwSendUserID != pISendUserItem->GetUserID() &&
      (pISendUserItem->GetMasterOrder() == 0 || CMasterRight::CanManagerAndroid(pISendUserItem->GetMasterRight()) == false)) {
    return false;
  }
  ...
  // 字符过滤
  auto chat_string = SensitiveWordFilter(pWisperChat->szChatString);
  lstrcpyn(WisperChat.szChatString, chat_string.c_str(), CountArray(WisperChat.szChatString));
  ...
  if (pIRecvUserItem->IsAndroidUser() == true) {
    ...
    do {
      ...
      if (pTempServerUserItem != NULL) {
        ...
        if (pTempServerUserItem->GetMasterOrder() > 0 && CMasterRight::CanManagerAndroid(pTempServerUserItem->GetMasterRight()) == true) {
          SendData(pTempServerUserItem, MDM_GR_USER, SUB_GR_WISPER_CHAT, &WisperChat,
                   wHeadSize + WisperChat.wChatLength * sizeof(WisperChat.szChatString[0]));
        }
      }
...
bool CAttemperEngineSink::OnTCPNetworkSubWisperExpression(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 权限判断
  if (pWisperExpression->dwSendUserID != pISendUserItem->GetUserID() &&
      (pISendUserItem->GetMasterOrder() == 0 || CMasterRight::CanManagerAndroid(pISendUserItem->GetMasterRight()) == false)) {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubGamePropertyBuy(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (wDataSize != sizeof(CMD_GR_PropertyBuy)) {
    return false;
  }
  ...
  // 投递数据
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_GAME_PROPERTY_BUY, dwSocketID, &PropertyRequest,
                                               sizeof(PropertyRequest));
...
bool CAttemperEngineSink::OnTCPNetworkSubPropertyBackpack(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递数据
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_QUERY_BACKPACK, dwSocketID, &QueryBackpack,
                                               sizeof(QueryBackpack));
  return true;
}
...
bool CAttemperEngineSink::OnTCPNetworkSubPropertyUse(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递数据
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_PROPERTY_USE, dwSocketID, &PropertyUseRequest,
                                               sizeof(PropertyUseRequest));
  return true;
}
...
bool CAttemperEngineSink::OnTCPNetworkSubQuerySendPresent(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_QUERY_SEND_PRESENT, dwSocketID, &QuerySendPresentRequest,
                                               sizeof(QuerySendPresentRequest));
  return true;
}
...
bool CAttemperEngineSink::OnTCPNetworkSubPropertyPresent(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递数据
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_PROPERTY_PRESENT, dwSocketID, &PropertyPresentRequest,
                                               sizeof(PropertyPresentRequest));
  return true;
}
...
bool CAttemperEngineSink::OnTCPNetworkSubGetSendPresent(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_GET_SEND_PRESENT, dwSocketID, &GetSendPresentRequest,
                                               sizeof(GetSendPresentRequest));
...
bool CAttemperEngineSink::OnTCPNetwordSubSendTrumpet(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递数据
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_SEND_TRUMPET, dwSocketID, &SendTrumpetRequest,
                                               sizeof(SendTrumpetRequest));
...
bool CAttemperEngineSink::OnTCPNetworkSubMemberKickUser(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pITargetUserItem->GetUserStatus() == US_PLAYING) {
    ...
    SnprintfT(szMessage, CountArray(szMessage), TEXT("由于玩家 [ %s ] 正在游戏中,您不能将它踢出游戏！"), pITargetUserItem->GetNickName());
  ...
  if (pPropertyBuff != NULL) {
    // 变量定义
    TCHAR szMessage[256] = TEXT("");
    SnprintfT(szMessage, CountArray(szMessage), TEXT("由于玩家 [ %s ] 正在使用防踢卡,您无法将它踢出游戏！"), pITargetUserItem->GetNickName());
    // 发送消息
    SendRoomMessage(pIServerUserItem, szMessage, SMT_EJECT);
    return true;
  }
  ...
  if (wTargerTableID != INVALID_TABLE) {
    ...
    SnprintfT(szMessage, CountArray(szMessage), TEXT("你已被%s请离桌子！"), pIServerUserItem->GetNickName());
...
bool CAttemperEngineSink::OnTCPNetworkSubUserChairReq(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 查找桌子
  INT nStartIndex = random_engine_() % m_pGameServiceOption->wTableCount;
  for (INT_PTR i = 0; i < (m_pGameServiceOption->wTableCount); i++) {
    // 定义变量
  ...
  // 查找桌子
  nStartIndex = random_engine_() % m_pGameServiceOption->wTableCount;
  for (INT_PTR i = 0; i < (m_pGameServiceOption->wTableCount); i++) {
    // 定义变量
    INT nTableIndex = ((INT) i + nStartIndex) % (INT) m_pGameServiceOption->wTableCount;
...
bool CAttemperEngineSink::OnTCPNetworkSubGrowLevelQuery(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pGrowLevelQueryInfo->dwUserID, DBR_GR_GROWLEVEL_QUERY_IFNO, dwSocketID, &GrowLevelQueryInfo,
                                               sizeof(GrowLevelQueryInfo));
...
bool CAttemperEngineSink::OnTCPNetworkSubExchangeScoreByIngot(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(ExchangeScore.dwUserID, DBR_GR_EXCHANGE_SCORE_INGOT, dwSocketID, &ExchangeScore,
                                               sizeof(ExchangeScore));
...
bool CAttemperEngineSink::OnTCPNetworkSubExchangeScoreByBeans(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(ExchangeScore.dwUserID, DBR_GR_EXCHANGE_SCORE_BEANS, dwSocketID, &ExchangeScore,
                                               sizeof(ExchangeScore));
...
bool CAttemperEngineSink::OnTCPNetworkSubQueryInsureInfo(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(QueryInsureInfo.dwUserID, DBR_GR_QUERY_INSURE_INFO, dwSocketID, &QueryInsureInfo,
                                               sizeof(QueryInsureInfo));
...
bool CAttemperEngineSink::OnTCPNetworkSubEnableInsureRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(UserEnableInsure.dwUserID, DBR_GR_USER_ENABLE_INSURE, dwSocketID, &UserEnableInsure,
                                               sizeof(UserEnableInsure));
...
bool CAttemperEngineSink::OnTCPNetworkSubSaveScoreRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  if (pSaveScoreRequest->lSaveScore > lConsumeQuota) {
    if (lConsumeQuota < lUserWholeScore) {
      // 构造提示
      TCHAR szDescribe[128] = TEXT("");
      SnprintfT(szDescribe, CountArray(szDescribe), TEXT("由于您正在游戏中，游戏币可存入额度为 %I64d，存入操作失败！"), lConsumeQuota);
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_USER_SAVE_SCORE, dwSocketID, &UserSaveScore,
                                               sizeof(UserSaveScore));
...
bool CAttemperEngineSink::OnTCPNetworkSubTakeScoreRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_USER_TAKE_SCORE, dwSocketID, &UserTakeScore,
                                               sizeof(UserTakeScore));
...
bool CAttemperEngineSink::OnTCPNetworkSubTransferScoreRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_USER_TRANSFER_SCORE, dwSocketID, &UserTransferScore,
                                               sizeof(UserTransferScore));
...
bool CAttemperEngineSink::OnTCPNetworkSubQueryUserInfoRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_QUERY_TRANSFER_USER_INFO, dwSocketID, &QueryTransferUserInfo,
                                               sizeof(QueryTransferUserInfo));
...
bool CAttemperEngineSink::OnTCPNetworkSubCheckInQueryRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_CHECKIN_QUERY_INFO, dwSocketID, &CheckInQueryInfo,
                                               sizeof(CheckInQueryInfo));
...
bool CAttemperEngineSink::OnTCPNetworkSubLoadTaskInfoRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_TASK_QUERY_INFO, dwSocketID, &TaskQueryInfo,
                                               sizeof(TaskQueryInfo));
...
bool CAttemperEngineSink::OnTCPNetworkSubBaseEnsureTakeRequest(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_BASEENSURE_TAKE, dwSocketID, &TakeBaseEnsure,
                                               sizeof(TakeBaseEnsure));
...
bool CAttemperEngineSink::OnTCPNetworkSubOptionServer(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  SnprintfT(szBuffer, CountArray(szBuffer), TEXT("远程修改房间配置通知 管理员 %s [ %ld ]"), pIServerUserItem->GetNickName(),
...
bool CAttemperEngineSink::OnTCPNetworkSubKickAllUser(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  while (pITargerUserItem != NULL) {
    if (pITargerUserItem != pIServerUserItem) {
      // 发送消息
      StringT kick_message = StringUtils::ToString(pKillAllUser->szKickMessage);
      SendRoomMessage(pITargerUserItem, kick_message.c_str(), SMT_CHAT | SMT_EJECT | SMT_CLOSE_LINK | SMT_CLOSE_ROOM);
    }
...
bool CAttemperEngineSink::OnTCPNetworkSubDismissGame(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 效验数据
  if (pDismissGame->wDismissTableNum >= m_TableFrameArray.size()) {
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubWarningUser(VOID* pData, WORD wDataSize, DWORD dwSocketID) {
  ...
  // 发送警告
  TCHAR szMessage[128] = TEXT("");
  SnprintfT(szMessage, CountArray(szMessage), TEXT("警告：%s"), pSendWarning->szWarningMessage);
...
VOID CAttemperEngineSink::OnEventUserLogon(IServerUserItem* pIServerUserItem, bool bAlreadyOnLine) {
  ...
  if (pIServerUserItem->IsMobileUser() == false) {
    ...
    if (bAndroidUser == false) {
      // 构造提示
      TCHAR szMessage[128] = TEXT("");
      SnprintfT(szMessage, CountArray(szMessage), TEXT("欢迎您进入“%s”游戏房间，祝您游戏愉快！"), m_pGameServiceAttrib->szGameName);
...
VOID CAttemperEngineSink::OnEventUserLogout(IServerUserItem* pIServerUserItem, DWORD dwLeaveReason) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_LEAVE_GAME_SERVER, 0L, &LeaveGameServer, sizeof(LeaveGameServer),
                                               TRUE);
  ...
  // 推进任务
  if (pUserTaskEntry != NULL) {
    PerformTaskProgress(pIServerUserItem, pUserTaskEntry, LeaveGameServer.VariationInfo.dwWinCount, LeaveGameServer.VariationInfo.dwLostCount,
                        LeaveGameServer.VariationInfo.dwDrawCount);
  }
...
// 推进任务
bool CAttemperEngineSink::PerformTaskProgress(IServerUserItem* pIServerUserItem, tagUserTaskEntry* pUserTaskEntry, DWORD dwWinCount,
                                              DWORD dwLostCount, DWORD dwDrawCount) {
  // 参数校验
  ...
  dwSpanSecond = std::min(dwSpanSecond, pUserTaskEntry->dwResidueTime);
...
bool CAttemperEngineSink::PerformRoomTaskProgress(IServerUserItem* pIServerUserItem, tagUserTaskEntry* pUserTaskEntry, int iRoom, int iRoomNum,
                                                  int iPlayCount) {
  // 参数校验
  ASSERT(pIServerUserItem != NULL && pUserTaskEntry != NULL);
  if (pIServerUserItem == NULL || pUserTaskEntry == NULL) {
  ...
  // 投递请求
  m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(), DBR_GR_WRITE_ROOM_TASK_PROGRESS, dwSocketID, &PerformRoom,
                                               sizeof(PerformRoom));
...
// 道具事件
bool CAttemperEngineSink::OnEventPropertyBuyPrep(WORD cbRequestArea, WORD wPropertyIndex, IServerUserItem* pISourceUserItem,
                                                 IServerUserItem* pTargetUserItem) {
  // 目标玩家
  ...
  if ((m_pGameServiceOption->wServerType == GAME_GENRE_GOLD || m_pGameServiceOption->wServerType == GAME_GENRE_MATCH) &&
      (wPropertyIndex == PROPERTY_ID_SCORE_CLEAR || wPropertyIndex == PROPERTY_ID_TWO_CARD || wPropertyIndex == PROPERTY_ID_FOUR_CARD ||
       wPropertyIndex == PROPERTY_ID_POSSESS)) {
    // 发送消息
  ...
  switch (wPropertyIndex) {
    case PROPERTY_ID_SCORE_CLEAR: // 负分清零
    {
      ...
      if (lCurrScore >= 0) {
        ...
        if (pISourceUserItem == pTargetUserItem) {
          SnprintfT(szMessage, CountArray(szMessage), TEXT("您现在的积分已经是非负数，不需要使用负分清零道具！"));
        } else {
          SnprintfT(szMessage, CountArray(szMessage), TEXT("[ %s ]现在的积分已经是非负数，不需要使用负分清零道具！"), pTargetUserItem->GetNickName());
        }
    ...
    case PROPERTY_ID_ESCAPE_CLEAR: // 逃跑清零
    {
      ...
      if (dwCurrFleeCount == 0) {
        ...
        TCHAR szMessage[128] = TEXT("");
        if (pISourceUserItem == pTargetUserItem) {
          SnprintfT(szMessage, CountArray(szMessage), TEXT("您现在的逃跑率已经为0，不需要使用逃跑清零道具！"));
        } else {
          SnprintfT(szMessage, CountArray(szMessage), TEXT("[ %s ]现在的逃跑率已经为0，不需要使用逃跑清零道具！"), pTargetUserItem->GetNickName());
        }
...
bool CAttemperEngineSink::PerformDistribute() {
  ...
  while (true) {
    ...
    try {
      ...
      // 获取用户
      srand(GetTickCount());
      WORD wRandCount = std::max(m_pGameServiceOption->wMaxPartakeGameUser - m_pGameServiceOption->wMinPartakeGameUser + 1, 1);
      WORD wChairCount = m_pGameServiceOption->wMinPartakeGameUser + random_engine_() % wRandCount;
      ...
      if (bSitSuccess) {
        ...
        // 移除结点
        while (DistributeInfoArray.size() > 0) {
          m_DistributeManage.RemoveDistributeNode((tagDistributeNode*) DistributeInfoArray.front().pPertainNode);
          DistributeInfoArray.erase(DistributeInfoArray.begin());
        }
...
bool CAttemperEngineSink::InitTableFrameArray() {
  ...
  ZeroMemory(m_TableFrameArray.data(), m_pGameServiceOption->wTableCount * sizeof(CTableFrame*));

  // 创建桌子
  for (WORD i = 0; i < m_pGameServiceOption->wTableCount; i++) {
    // 创建对象
    m_TableFrameArray[i] = new (std::nothrow) CTableFrame;
...
bool CAttemperEngineSink::DisconnectCorrespond() {
  ...
  // 关闭约战服务连接
  if (m_pPrsnlRmITCPSocketService != NULL) {
    m_pPrsnlRmITCPSocketService->CloseSocket();
  }
...
// 敏感词过滤
StringT CAttemperEngineSink::SensitiveWordFilter(StringViewT msg) {
  return m_WordsFilter.Filtrate(msg);
}

// 敏感词过滤
std::u16string CAttemperEngineSink::SensitiveWordFilter(std::u16string_view msg) {
  return m_WordsFilter.Filtrate(msg);
}
...
// 取消创建
VOID CAttemperEngineSink::CancelCreateTable(DWORD dwUserID, DWORD dwDrawCountLimit, DWORD dwDrawTimeLimit, DWORD dwReason, WORD wTableID,
                                            TCHAR* szRoomID) {
  // 退还费用
  ...
  if (pServerUserItem != NULL) {
    ...
    // 投递数据
    m_pIDBCorrespondManager->PostDataBaseRequest(dwUserID, DBR_GR_CANCEL_CREATE_TABLE, dwSocketID, &CancelCreateTable,
                                                 sizeof(DBR_GR_CancelCreateTable));
  } else {
    // 投递数据
    DBR_GR_CancelCreateTable CancelCreateTable;
    ZeroMemory(&CancelCreateTable, sizeof(DBR_GR_CancelCreateTable));
    ...
    m_pIDBCorrespondManager->PostDataBaseRequest(CancelCreateTable.dwUserID, DBR_GR_HOST_CANCEL_CREATE_TABLE, 0, &CancelCreateTable,
                                                 sizeof(DBR_GR_CancelCreateTable));
  }
  ...
  while (wEnumIndex < m_ServerUserManager.GetUserItemCount()) {
    ...
    if (wTablePos != INVALID_WORD && !bViewModeAll) {
      ...
      if (wTablePos == 0) {
        ...
        if ((wUserPagePos < wTablePos - wEverySendPage * wTagerDeskCount) ||
            (wUserPagePos > wTablePos + wEverySendPage * wTagerDeskCount + wTagerDeskCount - 1)) {
          continue;
        }
      }
    }
  ...
  while (wEnumIndex < m_ServerUserManager.GetUserItemCount()) {
    ...
    if (!bViewModeAll) {
      ...
      if (wTagerDeskPos == 0) {
        ...
        if ((wDestPos < wTagerDeskPos - wEverySendPage * wTagerDeskCount) ||
            (wDestPos > wTagerDeskPos + wEverySendPage * wTagerDeskCount + wTagerDeskCount - 1)) {
          continue;
        }
      }
    }
...
bool CAttemperEngineSink::SendSystemMessage(CMD_GR_SendMessage* pSendMessage, WORD wDataSize) {
  ...
  if (pSendMessage->cbAllRoom == TRUE) {
    ...
    // 发送系统消息
    StringT system_message = StringUtils::ToString(pSendMessage->szSystemMessage);
    if (pSendMessage->cbGame == TRUE) {
      SendGameMessage(system_message.c_str(), SMT_CHAT);
    }
    if (pSendMessage->cbRoom == TRUE) {
      SendRoomMessage(system_message.c_str(), SMT_CHAT);
    }
  }
...
// 开始游戏写入参与信息
VOID CAttemperEngineSink::PersonalRoomWriteJoinInfo(DWORD dwUserID, WORD wTableID, WORD wChairID, DWORD dwKindID, TCHAR* szRoomID,
                                                    TCHAR* szPersonalRoomGUID) {
  // 退还费用
