...
bool CDataBaseEngineSink::OnDataBaseEngineStart(IUnknownEx* pIUnknownEx) {
  ...
  try {
    ...
    // 设置连接
    m_GameDBModule->SetConnectionInfo(m_pDataBaseParameter->szDataBaseAddr, m_pDataBaseParameter->wDataBasePort, m_pDataBaseParameter->szDataBaseName,
                                      m_pDataBaseParameter->szDataBaseUser, m_pDataBaseParameter->szDataBasePass);
    ...
    m_pIGameDataBaseEngineSink =
        (IGameDataBaseEngineSink*) m_pIGameServiceManager->CreateGameDataBaseEngineSink(IID_IGameDataBaseEngineSink, VER_IGameDataBaseEngineSink);
    ...
    // 错误信息
    CLogger::Error(TEXT("{}: {}"), TEXT(__FUNCTION__), pIException->GetExceptionDescribe());
...
bool CDataBaseEngineSink::OnRequestLogonUserID(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID, BOOL bMatch) {
  ...
  try {
    ...
    SnprintfT(szClientAddr, CountArray(szClientAddr), TEXT("%d.%d.%d.%d"), pClientAddr[0], pClientAddr[1], pClientAddr[2], pClientAddr[3]);
    ...
    // 输出参数
    m_GameDBAide.AddParameterOutput(TEXT("@strErrorDescribe"), m_LogonFailure.szDescribeString, sizeof(m_LogonFailure.szDescribeString),
                                    adParamOutput);
    ...
    OnLogonDisposeResult(dwContextID, lResultCode, DBVarValue.c_str(), false);
...
bool CDataBaseEngineSink::OnRequestLogonMobile(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    StringT DBVarValue;
    m_GameDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
    OnLogonDisposeResult(dwContextID, lResultCode, DBVarValue.c_str(), true, pLogonMobile->cbDeviceType, pLogonMobile->wBehaviorFlags,
...
bool CDataBaseEngineSink::OnRequestWritePersonalGameScore(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    if (wDataSize != sizeof(DBR_GR_WritePersonalGameScore)) {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestGameScoreRecord(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    ASSERT(wDataSize == (sizeof(DBR_GR_GameScoreRecord) - sizeof(pGameScoreRecord->GameScoreRecord) +
                         pGameScoreRecord->wRecordCount * sizeof(pGameScoreRecord->GameScoreRecord[0])));
...
bool CDataBaseEngineSink::OnRequestLoadGameColumn(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    for (WORD i = 0; i < CountArray(GameColumnInfo.ColumnItemInfo); i++) {
      ...
      m_GameDBAide.GetValue_String(TEXT("ColumnName"), GameColumnInfo.ColumnItemInfo[i].szColumnName,
                                   CountArray(GameColumnInfo.ColumnItemInfo[i].szColumnName));
...
bool CDataBaseEngineSink::OnRequestLoadAndroidUser(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    for (WORD i = 0; i < CountArray(GameAndroidInfo.AndroidAccountsInfo); i++) {
      ...
      m_TreasureDBAide.GetValue_String(TEXT("LogonPass"), GameAndroidInfo.AndroidAccountsInfo[i].szPassword,
                                       CountArray(GameAndroidInfo.AndroidAccountsInfo[i].szPassword));
...
bool CDataBaseEngineSink::OnRequestUserEnableInsure(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    lstrcpyn(UserEnableInsureResult.szDescribeString, DBVarValue.c_str(), CountArray(UserEnableInsureResult.szDescribeString));
    ...
    lstrcpyn(UserEnableInsureResult.szDescribeString, TEXT("由于数据库操作异常，请您稍后重试！"),
             CountArray(UserEnableInsureResult.szDescribeString));
...
bool CDataBaseEngineSink::OnRequestUserSaveScore(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    StringT DBVarValue;
    m_TreasureDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
    OnInsureDisposeResult(dwContextID, lResultCode, pUserSaveScore->lSaveScore, DBVarValue.c_str(), false, pUserSaveScore->cbActivityGame);
...
bool CDataBaseEngineSink::OnRequestUserTakeScore(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    StringT DBVarValue;
    m_TreasureDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
    OnInsureDisposeResult(dwContextID, lResultCode, 0L, DBVarValue.c_str(), false, pUserTakeScore->cbActivityGame);
...
bool CDataBaseEngineSink::OnRequestUserTransferScore(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    StringT DBVarValue;
    m_TreasureDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
    OnInsureDisposeResult(dwContextID, lResultCode, 0L, DBVarValue.c_str(), false, pUserTransferScore->cbActivityGame);
...
bool CDataBaseEngineSink::OnRequestQueryInsureInfo(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_QueryUserInsureInfo"), true) == DB_SUCCESS) {
      ...
      // 错误处理
      OnInsureDisposeResult(dwContextID, m_TreasureDBAide.GetReturnValue(), 0L, DBVarValue.c_str(), false, pQueryInsureInfo->cbActivityGame);
    }
...
bool CDataBaseEngineSink::OnRequestQueryTransferUserInfo(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_QueryTransferUserInfo"), true) == DB_SUCCESS) {
      ...
      // 错误处理
      OnInsureDisposeResult(dwContextID, m_TreasureDBAide.GetReturnValue(), 0L, DBVarValue.c_str(), false, pQueryTransferUserInfo->cbActivityGame);
    }
...
bool CDataBaseEngineSink::OnRequestTaskGiveUp(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    lstrcpyn(TaskResult.szNotifyContent, DBVarValue.c_str(), CountArray(TaskResult.szNotifyContent));
...
bool CDataBaseEngineSink::OnRequestPurchaseMember(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    lstrcpyn(PurchaseResult.szNotifyContent, DBVarValue.c_str(), CountArray(PurchaseResult.szNotifyContent));
...
bool CDataBaseEngineSink::OnRequestExchangeScoreByIngot(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    StringT DBVarValue;
    m_TreasureDBModule->GetParameter(TEXT("@strNotifyContent"), DBVarValue);
    lstrcpyn(ExchangeResult.szNotifyContent, DBVarValue.c_str(), CountArray(ExchangeResult.szNotifyContent));
...
bool CDataBaseEngineSink::OnRequestCreateTable(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (wDataSize != sizeof(DBR_GR_CreateTable)) {
      return false;
    }
    ...
    if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_CreateTableFee"), true) == DB_SUCCESS) {
      ...
      lstrcpyn(CreateFailure.szDescribeString, DBVarValue.c_str(), CountArray(CreateFailure.szDescribeString));
...
bool CDataBaseEngineSink::OnRequsetInsertCreateRecord(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (wDataSize != sizeof(DBR_GR_InsertCreateRecord)) {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestRoomTaskProgress(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (wDataSize != sizeof(DBR_GR_PerformRoomTaskProgress)) {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestCancelCreateTable(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (wDataSize != sizeof(DBR_GR_CancelCreateTable)) {
      return false;
    }
    ...
    if (pCancelCreateTable->dwReason == CANCELTABLE_REASON_ENFOCE || CANCELTABLE_REASON_NOT_START == pCancelCreateTable->dwReason) {
      ...
      lstrcpyn(CancelCreateResult.szDescribeString, DBVarValue.c_str(), CountArray(CancelCreateResult.szDescribeString));
    }
  ...
  try {
    ...
    if (pCancelCreateTable->dwReason == 0) {
      ...
      if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_CreateTableQuit"), true) == DB_SUCCESS) {
        ...
        // 发送参数
        m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_CURRENCE_ROOMCARD_AND_BEAN, dwContextID, &CurrenceRoomCardAndBeans,
                                                       sizeof(DBO_GR_CurrenceRoomCardAndBeans));
      }
    }
...
bool CDataBaseEngineSink::OnRequsetDissumeRoom(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    std::vector<uint8_t> cbPersonalRoomInfo;
    cbPersonalRoomInfo.reserve(PERSONAL_ROOM_CHAIR * sizeof(cbPersonalRoomInfo));
    for (int i = 0; i < PERSONAL_ROOM_CHAIR; i++) {
      auto data = (uint8_t*) &pDissumCreateTable->PersonalUserScoreInfo[i];
      cbPersonalRoomInfo.insert(cbPersonalRoomInfo.end(), data, data + sizeof(tagPersonalUserScoreInfo));
    }
    ...
    // 头像信息
    pIDataBase->AddParameter(TEXT("@cbPersonalRoomInfo"), adParamInput, cbPersonalRoomInfo);
...
bool CDataBaseEngineSink::OnRequestLoadPersonalParameter(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (lResultCode == DB_SUCCESS) {
      ...
      // 错误信息
      CLogger::Error(TEXT("{}   GSP_GS_LoadPersonalRoomParameter 错误 = {}"), TEXT(__FUNCTION__), lResultCode);
    }

    m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_LOAD_PERSONAL_ROOM_OPTION, dwContextID, &pPersonalRoomOption,
                                                   sizeof(tagPersonalRoomOption));
...
void Hex2Char(char* szHex, uint8_t& rch) {
  rch = 0;
  int i;
  for (i = 0; i < 2; i++) {
    if (i == 0) {
...
bool CDataBaseEngineSink::OnRequestBuyGameProperty(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  if (wDataSize != sizeof(DBR_GR_PropertyBuy)) {
    return false;
  }
  ...
  try {
    ...
    if (lResultCode == DB_SUCCESS) {
      ...
      lstrcpyn(PropertyBuyResult.szNotifyContent, DBVarValue.c_str(), CountArray(PropertyBuyResult.szNotifyContent));
      ...
      StringT DBVarValue;
      m_PlatformDBAide.GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
      lstrcpyn(PropertyFailure.szDescribeString, DBVarValue.c_str(), CountArray(PropertyFailure.szDescribeString));
...
bool CDataBaseEngineSink::OnRequestPropertyUse(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    StringT DBVarValue;
    m_PlatformDBAide.GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
    lstrcpyn(DBOPropertyUseResult.szNotifyContent, DBVarValue.c_str(), CountArray(DBOPropertyUseResult.szNotifyContent));
    ...
    if (lResultCode == 0) {
      ...
      for (WORD i = 0;; i++) {
        ...
        m_PlatformDBAide.GetValue_String(TEXT("Name"), QuerySendPresenResult.Present[i].szPropName,
                                         CountArray(QuerySendPresenResult.Present[i].szPropName));
        SYSTEMTIME st;
...
bool CDataBaseEngineSink::OnRequestPropertyPresent(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    StringT DBVarValue;
    m_PlatformDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
    lstrcpyn(PropertyPresent.szNotifyContent, DBVarValue.c_str(), CountArray(PropertyPresent.szNotifyContent));
    ...
    if (lResultCode == 0) {
      ...
      for (WORD i = 0;; i++) {
        ...
        m_PlatformDBAide.GetValue_String(TEXT("Name"), GetSendPresenResult.Present[i].szPropName,
                                         CountArray(GetSendPresenResult.Present[i].szPropName));
        SYSTEMTIME st;
        m_PlatformDBAide.GetValue_SystemTime(TEXT("SendTime"), st);
        struct tm gm = {st.wSecond, st.wMinute, st.wHour, st.wDay, st.wMonth - 1, st.wYear - 1900, st.wDayOfWeek, 0, 0};
...
bool CDataBaseEngineSink::OnRequestMemberDayPresent(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    lstrcpyn(pDBOMemberInfoResult.szNotifyContent, DBVarValue.c_str(), CountArray(pDBOMemberInfoResult.szNotifyContent));
...
bool CDataBaseEngineSink::OnRequestCheckInQueryInfo(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GR_CheckInQueryInfo"), true) == DB_SUCCESS) {
      ...
      lstrcpyn(CheckInResult.szNotifyContent, DBVarValue.c_str(), CountArray(CheckInResult.szNotifyContent));
...
bool CDataBaseEngineSink::OnRequestTakeBaseEnsure(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    lstrcpyn(BaseEnsureResult.szNotifyContent, DBVarValue.c_str(), CountArray(BaseEnsureResult.szNotifyContent));
...
bool CDataBaseEngineSink::OnRequestMatchSignup(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    if (MatchSignupResult.bResultCode == true) {
      ...
      StringT DBVarValue;
      m_GameDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
      lstrcpyn(MatchSignupResult.szDescribeString, DBVarValue.c_str(), CountArray(MatchSignupResult.szDescribeString));
    }
...
bool CDataBaseEngineSink::OnRequestMatchBuySafeCard(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    if (MatchBuyResult.bSuccessed == true) {
      ...
      StringT DBVarValue;
      m_GameDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
      lstrcpyn(MatchBuyResult.szDescribeString, DBVarValue.c_str(), CountArray(MatchBuyResult.szDescribeString));
    }
...
bool CDataBaseEngineSink::OnRequestMatchUserRevive(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  ...
  try {
    ...
    if (MatchReviveResult.bSuccessed == true) {
      ...
      StringT DBVarValue;
      m_GameDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
      lstrcpyn(MatchReviveResult.szDescribeString, DBVarValue.c_str(), CountArray(MatchReviveResult.szDescribeString));
    }
...
bool CDataBaseEngineSink::OnRequestQueryGrowLevelParameter(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (lResultCode == DB_SUCCESS) {
      ...
      lstrcpyn(GrowLevelUpgrade.szNotifyContent, DBVarValue.c_str(), CountArray(GrowLevelUpgrade.szNotifyContent));
...
bool CDataBaseEngineSink::OnRequestLoadSystemMessage(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (lReturnValue == 0) {
      ...
      SnprintfT(szServerID, CountArray(szServerID), TEXT("%d"), m_pGameServiceOption->wServerID);
      ...
      while (true) {
        ...
        StringT strServerRange = szServerRange;

        std::vector<StringT> vecServerID = StringUtils::Split(strServerRange, TEXT(','));
        for (auto& strID: vecServerID) {
          WORD wServerID = (WORD) (std::stoi(strID));
          // 范围判断
          bAllRoom = wServerID == 0;
          bSendMessage = (bAllRoom || wServerID == m_pGameServiceOption->wServerID);
          if (bSendMessage) {
            break;
          }
        }
        ...
        if (bSendMessage) {
          ...
          SystemMessage.tStartTime = GetUnixTime(&systime);
          // 结束时间
          m_GameDBAide.GetValue_SystemTime(TEXT("ConcludeTime"), systime);
          SystemMessage.tConcludeTime = GetUnixTime(&systime);
...
// 登录结果
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient, BYTE cbDeviceType,
                                               WORD wBehaviorFlags, WORD wPageTableCount) {
  if (dwErrorCode == DB_SUCCESS) {
    ...
    try {
      ...
      CLogger::Error(TEXT("%s   %s 出错"), TEXT(__FUNCTION__), TEXT("GSP_GR_QueryTaskInfo"));
    }
    ...
    try {
      ...
      CLogger::Error(TEXT("%s   %s 出错"), TEXT(__FUNCTION__), TEXT("GSP_GR_QueryUserGameData"));
    }
...
// 银行结果
VOID CDataBaseEngineSink::OnInsureDisposeResult(DWORD dwContextID, DWORD dwErrorCode, SCORE lFrozenedScore, LPCTSTR pszErrorString,
                                                bool bMobileClient, BYTE cbActivityGame) {
  if (dwErrorCode == DB_SUCCESS) {
    // 变量定义
...
bool CDataBaseEngineSink::OnRequestWriteJoinInfo(DWORD dwContextID, VOID* pData, WORD wDataSize, DWORD& dwUserID) {
  try {
    ...
    if (wDataSize != sizeof(DBR_GR_WriteJoinInfo)) {
      return false;
    }
    ...
    CLogger::Error(TEXT("OnRequestWriteJoinInfo 函数中有错误"));
  }
