...
#define SUB_GR_USER_ONLOOK 2           // 旁观请求
...
#define SUB_GR_USER_ROOM_CHEAT 13      // 房间配置
...
#define SUB_GR_ROOMCHEAT 13       // 作弊信息
//////////////////////////////////////////////////////////////////////////////////
...
struct CMD_GR_Match_Info {
  ...
  BYTE cbTotalPlayCount; // 当轮总局数
  BYTE cbCurPlayCount;   // 当前局数
  BYTE cbCurMatchRank;   // 当前排名
  BYTE cbStartUserCount; // 开赛人数
  BYTE cbPromotion;      // 晋级人数
};
...
#define SUB_GF_ONLOOK_CONFIG 3 // 旁观配置
...
#define SUB_GF_ONLOOK_STATUS 102 // 旁观状态
...
// 游戏预留范围（游戏处理）
#define SUB_GF_FRAME_MESSAG_GAME_MIN 600 // 游戏最小命令
#define SUB_GF_FRAME_MESSAG_GAME_MAX 700 // 游戏最小命令
#define SUB_GET_ROMM_ALL_USER 766        // 获取房间用户
#define SUB_GET_ROMM_ALL_USER_RESULT 767 // 获取结果
//////////////////////////////////////////////////////////////////////////////////
// 用户信息
struct CMD_CF_UserInfo {
  DWORD wUserID[MAX_CHAIR];
  DWORD wGameID[MAX_CHAIR];
  LONGLONG lUserScore[MAX_CHAIR];
  LONGLONG lInsureScore[MAX_CHAIR];
  TCHAR szNickName[MAX_CHAIR][LEN_NICKNAME];

  // 用户属性
  bool bIsAndroid[MAX_CHAIR]; // 机器标识
  // 用户状态
  WORD wTableID[MAX_CHAIR];     // 桌子索引
  WORD wChairID[MAX_CHAIR];     // 椅子索引
  BYTE cbUserStatus[MAX_CHAIR]; // 用户状态

  WORD wUserCount;
};
// 游戏配置
struct CMD_GF_GameOption {
  BYTE cbAllowOnlook;    // 旁观标志
  DWORD dwFrameVersion;  // 框架版本
  DWORD dwClientVersion; // 游戏版本
};

// 旁观配置
struct CMD_GF_OnlookConfig {
  DWORD dwUserID;     // 用户标识
  BYTE cbAllowOnlook; // 允许旁观
};

// 旁观状态
struct CMD_GF_OnlookStatus {
  BYTE cbAllowOnlook; // 允许旁观
};
...
#define SUB_GR_DEAL_CANCEL_INFO 18 // 处理解散房间信息

#define SUB_GF_DEAL_CANCEL_REPLY 19  // 通知请求解散玩家请求成功
#define SUB_GF_DEAL_CANCEL_FINISH 20 // 通知请求解散玩家请求成功

#define SUB_GF_ALREADY_CANCLE 21 // 约战房间已经被解散
#define SUB_GF_CANCLE_ERROR 22   // 约战房间解散出错

#define SUB_GR_QUERY_JOIN_INFO 30 // 查询加入信息
#define SUB_GR_QUERY_JOIN_INFO_RESULT 301

#define SUB_GR_QUERY_JOIN_CLUB_INFO_RESULT 302 // 对应 SUB_GR_QUERY_JOIN_INFO 的房间是俱乐部房间的时候的结果

struct CMD_GR_QueryJoinClubRoomInfoResult {
  BYTE cbResult; // 0成功 1不是该俱乐部成员
};
...
// 请求解散
struct CMD_GR_CancelRequest_Result {
  DWORD dwUserID;   // 用户 I D
  DWORD dwTableID;  // 桌子 I D
  DWORD dwChairID;  // 椅子 I D
  WORD wCancelTime; // 取消时间
};
// 房主强制解散游戏
...
// 请求加入信息
struct CMD_GR_QueryJoinInfo {
  DWORD dwUserID;  // 用户 I D
  DWORD dwTableID; // 桌子 I D
};

// 请求加入信息结果
struct CMD_GR_QueryJoinInfoResult {
  DWORD dwTableOwnerID;      // 房主ID
  BYTE cbPayMode;            // 支付方式
  DWORD dwDrawCountLimit;    // 局数限制
  LONG lDiamondFee;          // 钻石费用
  int bRoomPwd;              //(BOOL)房间密码
  BYTE cbGameRule[RULE_LEN]; // 游戏规则 第0位标识 是否设置规则 0 代表未设置 1 代表设置
};
...
struct CMD_GR_DealCancelInfo {
  BYTE cbUserDealInfo[CHAIR_COUNT]; // 申请解散玩家处理方式 0 未处理 1 同意 2 不同意
  BYTE cbIsDeal;                    // 操作是否结束 0 结束 1 未结束
  BYTE cbRequestUser;               // 申请解散玩家的座位号
  DWORD dwRemainTime;               // 剩余多少时间
};
//////////////////////////////////////////////////////////////////////////////////
#pragma pack()
