...
struct tagGetPersonalRule {
  BYTE cbPersonalRule[1024]; // 私人定制配置
};
// 密码状态
...
struct tagHostCreatRoomInfo {
  ...
  tagHostCreatRoomInfo() { memset(&hRoomCreateInfo, 0, sizeof(tagRoomCreateInfo) * MAX_CREATE_PERSONAL_ROOM); }
};
...
struct tagPersonalUserScoreInfo {
  ...
  tagPersonalUserScoreInfo() { memset(this, 0, sizeof(tagPersonalUserScoreInfo)); }
};

// 用于请求单个约战房间信息
struct tagPersonalRoomInfo {
  DWORD dwPersonalRoomID = 0; // 约战房间ID
  DWORD dwGameKindID = 0;     // 游戏类型

  BYTE cbPlayTurnCount = 0;  // 私人放进行游戏的最大局数
  DWORD dwPlayTimeLimit = 0; // 私人房进行游戏的最大时间 单位秒

  BYTE cbIsDisssumRoom = 0;       // 房间状态 0 表示未开始 1 表示 游戏中 2 已解散
...
struct tagPersonalGeneralRule {
  ...
  int nExTwo;       // 扩展配置2
  int nExThree;     // 扩展配置3
  tagPersonalGeneralRule() { memset(this, 0, sizeof(tagPersonalGeneralRule)); }
};
...
struct tagPersonalRule {
  ...
  tagPersonalRule() { memset(nCellScore, 0, sizeof(nCellScore)); }
};
//////////////////////////////////////////////////////////////////////////////////
