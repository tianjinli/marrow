...
class CDispatchEngineSink : public IDispatchEngineSink {
  ...
  // 状态变量
protected:
  bool m_bNeekCorrespond = true; // 协调标志
  bool m_bShowServerStatus = false; // 显示服务器状态
  bool m_bNeekPersonal = true; // 约战标志
  bool m_bHasPrsnlRM = false; // 有约战
...
protected:
  ...
  IDataBaseEngine* database_engine_ = nullptr; // 数据引擎
  ITCPNetworkEngine* network_engine_ = nullptr; // 网络引擎
  ITCPSocketService* correspond_service_ = nullptr; // 协调服务
  ITCPSocketService* personal_service_ = nullptr; // 约战服务
public:
  // 视频配置
protected:
  WORD m_wAVServerPort = 0; // 视频端口
  DWORD m_dwAVServerAddr = 0; // 视频地址
