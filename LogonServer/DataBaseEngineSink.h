...
protected:
  ...
  //IDataBaseEngineEvent* m_pIDataBaseEngineEvent = nullptr; // 数据事件

  // 函数定义
public:
  // 构造函数
  CDataBaseEngineSink() = default;
  // 析构函数
  virtual ~CDataBaseEngineSink() = default;
...
public:
  ...
  // 发布结果事件
  virtual bool EmitDataBaseEngineResult(WORD wRequestID, DWORD dwContextID, VOID* pData, WORD wDataSize);
  // 请求事件
...
protected:
  ...
  // 绑定推广
  bool OnRequestBindSpreader(DWORD dwContextID, VOID* pData, WORD wDataSize);
...
protected:
  ...
  // 请求私人房间信息
  bool OnRequestQueryPersonalRule(DWORD dwContextID, VOID* pData, WORD wDataSize);
  // 俱乐部
protected:
  bool OnRequestCreateClub(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestGetClubList(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestGetClubUserList(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestGetClubName(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestQueryJoinClub(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestQueryQuitClub(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestQueryClubJoinUserList(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestDealUserJoinClub(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestMoveJewelToClub(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestDeleteClubUser(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestCreateClubRoomRule(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestGetClubAllRoomRule(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestDeleteClubRoomRule(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestGetClubRoomList(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestModifyClubSummary(DWORD dwContextID, VOID* pData, WORD wDataSize);
  bool OnRequestJoinUserToClub(DWORD dwContextID, VOID* pData, WORD wDataSize);
  // 帐号绑定
protected:
  /// 帐号绑定
...
protected:
  // 视频信息
  bool OnRequestVideoInfo(DWORD dwContextID, VOID* pData, WORD wDataSize);
  // 视频详情
  bool OnRequestVideoDetails(DWORD dwContextID, VOID* pData, WORD wDataSize);
  // 视频详情
  bool OnRequestVideoDetailsByRoomID(DWORD dwContextID, VOID* pData, WORD wDataSize);
  // 查询回放码
  bool OnRequestPlayBackCodeYZ(DWORD dwContextID, VOID* pData, WORD wDataSize);
  // 查询回放码
  bool OnRequestPlayBackCode(DWORD dwContextID, VOID* pData, WORD wDataSize);
  // 结果处理
protected:
  // 登录结果
